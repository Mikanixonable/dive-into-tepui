...
また、これらの分割されていないときは、分割するようにパターンを統一する。例えばgame.tsには毎フレーム呼ばれる関数がいくつかに分割されて存在するが、それらの分割方針に一貫性が見られない。pipRendererとかは何をしているのか、何のために分けてあるのか謎な関数になっている。こういうのが生まれないようにする。
