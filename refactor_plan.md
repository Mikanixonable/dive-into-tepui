...
- ctx注入パターンの見直し。
全てが過剰なctxを受け取っている可能性が高い。密結合なので避ける。

そもそも、幅広いctxが必要になる時点で、責務分離が不十分である可能性が高い。現在挙がっているメソッドを確認し、ctxのなかで必要としているフィールドが異なるメソッド群が一つのモジュールに混在していないかを確認する。

- フォルダ階層、類似命名の整理　physicsとあるが、orbitとcollisionに大別できるはず。
- 描画系は、大きく
  - 3D描画系
  - pip描画系
  - hud描画系
  に分けられるはず。pipはカメラは3D、表示は専用のhudという感じだろうか？

- 類似のクラスを継承またはインターフェイスパターンで整理できるか検討。
  - 例えば、rとvとmを個別に受け渡している例などはデータの構造化が悪い。orbitPositionなどの便利な型を作って、一元化する。
  - 例えば、物理演算対象を種別ごとにすべて別の配列に突っ込んでいるが、colliderObjectやOrbitObjectなどの taggedTypeなどで共通化できないか。
