...
### 描画の移譲が中途半端。
renderDynamicsに一元化されているが、playerのように単にmodelを描画すればいいというものでもないものがあるのだから、
...
renderBeltはcontextを丸ごともらうべきではない。必要なものだけをもらうべき。


### player内の配線
onfireあたりの配線が変。その他にも、分割する必要のない関数が分割されている。一度しか参照されていないような関数は、あるていど、責務が肥大化しない範囲で、インライン展開してから再度分割の見通しを立てる。
