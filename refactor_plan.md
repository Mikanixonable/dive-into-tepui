...
```
などとし、実装は別のメソッドに切り出すこと。コメントと空行で区切られた数行のブロック（つまり意味を持つコード単位）は適切に引数と返り値を設計すればすべてサブルーチン化すべき。

- 関数としてそもそもそのクラス・モジュールに存在すべきでないものは、適切なモジュールに移動する。
  - たとえば、プレイヤーの挙動はplayer.tsを新設し、playerクラスにまとめる。
  - ベルトの挙動に関するものはbelt.tsにまとめる。
  - 物理演算に関連する事物がGameにあるべきでない。必要な事物をまとめたクラスを新設する。
  - ステージの初期設定、説明文などのデータはコード内に埋め込むのではなく、データベース化・定数化する。
  - その他、まとめるべき事象はどんどん新規モジュールを作成し、まとめる。
判断基準のヒントとして、Game.tsのあるメソッド内で、例えばthis.player.brabraPropertyのように、this.playerのプロパティを直接参照するようなコードが多数ある場合、その操作はPlayerの責務であり、Playerのメソッドとして実装されるべき。その他のプロパティについても同様。
