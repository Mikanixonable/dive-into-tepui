...
// ------------------------------------------------------------- ブースター
// 再使用可能な一段ブースター。機体の長手方向をローカル Z 軸とし、機首(前端)が +Z、ノズル(船尾)が
// -Z で、前端カプラーのおおよその面を z=0 に置く。段間カバーは別モデルにし、被せる段にだけ足す。
