...
export const METEOROLOGICAL_CASES: Readonly<Record<MeteorologicalCaseId, MeteorologicalCaseFixture>> = {
  ...
  C7: { id: 'C7', label: '波源と物質風', ...CONTROLS.c7, atmosphericLayers: STANDARD_LAYERS,
    measurementWindow: COMMON_WINDOW, measurements: [
      ...
      measurement('directional-spectrum', '方向別パワー', '1', 'directional spatial spectrum', 'valid spectral band', 1e-3, 'fixture 場で方向別スペクトルを測定するが量的許容域が未固定のため blocked'),
    ] },
  C8: { id: 'C8', label: '海洋境界層セル', ...CONTROLS.c8, atmosphericLayers: STANDARD_LAYERS,
    measurementWindow: { startMinutes: 0, endMinutes: 360, sampleIntervalMinutes: 10 }, measurements: [
      measurement('hole-fraction', 'セルの穴率', '1', 'clear-area share within cell mask', 'ocean boundary-layer mask', 1e-3, 'fixture 場で測定するが量的許容域が未固定のため blocked', 'observationSeries'),
      measurement('cell-size', 'セル径', 'km', 'connected-cell equivalent diameter', 'ocean boundary-layer mask', 0, 'fixture 場で測定するが量的許容域が未固定のため blocked', 'observationSeries'),
      measurement('cell-lifetime', 'セル寿命', 'min', 'connected-cell persistence', 'ocean boundary-layer mask', 0, 'fixture 系列で測定するが量的許容域が未固定のため blocked', 'observationSeries'),
    ] },
  C9: { id: 'C9', label: '二雲層の空隙と視差', ...CONTROLS.c9, atmosphericLayers: STANDARD_LAYERS,
    measurementWindow: COMMON_WINDOW, measurements: [
      measurement('tau-vertical-liquid', '中心鉛直光路の液水光学厚', '1',
        'same-rule CPU ray integration through the two-disc local field', 'disc center column', 0,
        '解析値 0.4 に対し絶対誤差 0.005 以下', 'analytic'),
      measurement('tau-vertical-ice', '中心鉛直光路の氷光学厚', '1',
        'same-rule CPU ray integration through the two-disc local field', 'disc center column', 0,
        '解析値 0.2 に対し絶対誤差 0.005 以下', 'analytic'),
      measurement('tau-vertical', '中心鉛直光路の合計光学厚', '1',
        'same-rule CPU ray integration through the two-disc local field', 'disc center column', 0,
        '解析値 0.6 に対し絶対誤差 0.005 以下', 'analytic'),
      measurement('transmittance-vertical', '中心鉛直光路の透過率', '1',
        'exp of the integrated total optical depth', 'disc center column', 0,
        'exp(-0.6) = 0.5488116361。許容値は光学厚ゲートに従う', 'analytic'),
      measurement('tau-slant-45', '45°中心光路の合計光学厚', '1',
        'same-rule CPU ray integration through the two-disc local field', 'disc center column', 0,
        '解析値 0.8485281374 に対し絶対誤差 0.005 以下', 'analytic'),
      measurement('transmittance-slant-45', '45°中心光路の透過率', '1',
        'exp of the integrated total optical depth', 'disc center column', 0,
        'exp(-0.8485281374) = 0.4280444912。許容値は光学厚ゲートに従う', 'analytic'),
      measurement('phase-isolation-liquid-only', '液水層のみの対照で氷の光学厚', '1',
        'same-rule CPU ray integration of the liquid-only control field', 'disc center column', 0,
        '厳密に 0', 'analytic'),
      measurement('phase-isolation-ice-only', '氷層のみの対照で液水の光学厚', '1',
        'same-rule CPU ray integration of the ice-only control field', 'disc center column', 0,
        '厳密に 0', 'analytic'),
      measurement('layer-gap', '雲層間空隙', 'm', 'zero-extinction band thickness between layer supports', 'between-layer mask', 1e-3, '解析値 3000 m。空隙内の非零消散は許さない'),
      measurement('parallax', '高度による視差', 'px', 'projected centroid separation by layer', 'both layer masks', 0, '45°投影で 5 km = 0.5 km 格子の 10 px。それぞれの重心誤差 0.5 km 以下'),
      measurement('parallax-centroid-liquid', '45°投影の液水層重心誤差', 'm', 'projected centroid error of the liquid layer', 'liquid layer mask', 0, '0.5 km 以下'),
      measurement('parallax-centroid-ice', '45°投影の氷層重心誤差', 'm', 'projected centroid error of the ice layer', 'ice layer mask', 0, '0.5 km 以下'),
      measurement('wind-displacement-liquid', '1時間の液水層中心変位', 'm', 'field centroid displacement after layer wind transport', 'liquid layer mask', 0, '東 36 km ± 0.5 km'),
      measurement('wind-displacement-ice', '1時間の氷層中心変位', 'm', 'field centroid displacement after layer wind transport', 'ice layer mask', 0, '北 36 km ± 0.5 km'),
      measurement('wind-cross-axis', '風交差軸の偽変位', 'm', 'cross-axis displacement of both layer centers', 'both layer masks', 0, '0.5 km 以下'),
      measurement('shadow-support', '影の支持域', 'm2', 'shadow support from shared 3D field', 'shadow-valid mask', 1e-3, '支持域面積を測定するが量的許容域が未固定のため blocked'),
    ] },
} as const;
