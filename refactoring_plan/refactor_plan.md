...
renderにphysicsのvec3を持ってきてほしくない（floatingOrigin補正はgame/フォルダ側で終わらせてほしい…）
↑これは徹底されてなさそう。要検証。

逆に、これらの条件を守る限りにおいて、gameフォルダを極力スリム化したい。純粋状態だけであればphysicsに、描画問題はrenderに移したい。たとえば、belt-physicsは適切に純粋化したらphysicsに置くべきで、gameから追い出せる（後述）
