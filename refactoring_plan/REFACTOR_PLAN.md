...
次に、その経験を踏まえて、同様の問題を抱えた箇所がないか、洗い出しをします。

predictにしてもplanEditorにしてもその他のモジュールにしても、自身の挙動・状態に関するGUIは原則自身で管理し、外部からの配線を最小化したい。そのために多少GUIが変更されても構わない（モジュール整理前のGUIを維持しようということが歴史的しがらみになっていてリファクタリングが最適化されていない）
