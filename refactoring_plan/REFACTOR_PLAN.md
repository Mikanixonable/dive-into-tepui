...
## 方向マーカーのインフラ
現状、marker-for-gameやmap-markerなどで利用されている「マーカー」機能（網羅的にはmarker-managerの参照範囲を調べてください）には、大きく2種類があって、3D空間上の「位置」を示すマーカーと、3D空間上の「方向」を示すマーカーである。（「視界外の敵機マーカー」は2Dの方向を表すなど、どちらでもない例外はあるが）

このうち後者、3Dの方向を指し示すマーカーは、
- 「方向ベクトルの大きさを無限遠（const DIST = 5e4;となっている。これconst.tsに移すべきだ）に拡大
- Player座標を足す（あるいはカメラ座標、あるいはfloatingOrigin。実コードは統一できていないかもしれないが、Player座標で統一すべき）
- それをprojectionでスクリーン座標に変換して表示する。

という変換手順になっていて、この変換がmarker-for-game.ts内などで何か所にも存在している。
（変数定義が離れたりしていて見づらい箇所もある。また、方向系マーカーでこの座標変換を経ていないものがあったら、実装の方が壊れている。発見したら報告してほしい）

これを関数に括りだして一元化するべき。marker-manager内にあると便利だろう。
同様に、3D空間上の位置を示すマーカーを表示するための関数も、marker-manager内に欲しい。これは
- 3Dの座標を、projectionでスクリーン座標に変換して表示

だけなので比較的シンプルだが、存在した方が便利であることには変わりない。

どれが3D位置を示すマーカーで、どれが方向を示すマーカーであるかは、処理（壊れている可能性はある）、コメント（書いてない可能性はある）、SPEC.mdの記述（書いてない可能性はある）、および文脈から判断してほしい。迷ったら聞いてください。

もし実装方針にわからない点があれば、実装の前に質問してください。
