...
## orbitLineの集約 vs 分散
分散させるんだか集中させるんだかがやや曖昧。
GameのsyncEntityOrbitLinesが個別にorbitLineの表示を面倒を見ていて、責務漏出っぽい。
少なくとも、playerのはPlayer.sync、targetとenemyのはTargeter.syncに責務を持たせるべき。

enemyのうち「target以外」を表示したいという設計になっているため、orbitLineを一括で表示するということができなくなっている。これは生成した後から色を変えることに対応していないので、ターゲット切り替え時にはEnemyのorbitLineを非表示にして代わりにtargetを表示して…という処理になっている。orbitLineの描画ごとに色（マテリアル）を指定するようにすれば改善するのでは。

そもそも、orbitLineごとにlineBasicMaterialが再生成されていて、これは描画パフォーマンスを悪くしていそう（drawCallが増えそう）。外部でマテリアルを保持するべきなんだけどどこの責務にしたものか……
...
不要なクロージャ注入に見える。trajLine（planに従った予測キャッシュ）の配置を再検討すべき
- editorからはクリック判定が欲しいのと、planを更新したら一緒にキャッシュも更新したい。
- predictからは、軌道線やプレイヤーの未来マーカーのために必要。
predict側の方が近いので、trajLineはpredict側に置いた方がいいかも…？　クリック判定や更新通知の配線が必要になる……
...
## パフォーマンス改善
リファクタリングでは可読性、疎結合化を重視した。パフォーマンスの問題はあとでweb-workerにでも括りだせばよかろう
predictが本当に延長分だけ計算しているか確認。predictはかなり重そう（直感）なので、測定してなんとかする。
