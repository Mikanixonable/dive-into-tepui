...
宇宙船は固定モデルではなく、モジュール実体と接続辺 からなる **ShipAssembly** である。`cockpit`、`tank`、`thruster`、`RCS`、`weapon`、`dock`、`solar_panel`、`radiator`、`booster` などが個別状態を持ち、総質量・重心・慣性・能力は船体構造から導出する。

ドッキングでは二隻を「親子のまま追従」させず、接続後の一つのアセンブリ として質量特性を再計算する。分離では接続グラフを切り、連結成分ごとに船体を再構成する。描画メッシュは衝突形状や保存形式の正本ではない。
