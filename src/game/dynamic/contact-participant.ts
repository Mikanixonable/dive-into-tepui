...
import type {
  EntityContactParticipant, KinematicParticipant, SurfaceContactParticipant,
} from './dynamic-simulation-participant';

// 区間の両端の位置・速度と半径が有限で、接触用の質量が負でないか。1つでも欠けた
// 参加者を解決へ入れる前に落とす — 非有限座標はセル添字を壊し、区間変位は
// セル一辺の算出を通じて全参加者へ伝播する。質量は 0(試験粒子)と無限大(不動)を通し、
// NaN だけを落とす — `contactMass < 0` と書くと NaN が通り抜ける。
function hasFiniteKinematics(e: KinematicParticipant): boolean {
  const { r, v } = e.state;
...
export function isFiniteParticipant(e: EntityContactParticipant): boolean {
  return hasFiniteKinematics(e) && e.contactMass >= 0;
}

export function isFiniteSurfaceParticipant(e: SurfaceContactParticipant): boolean {
  return hasFiniteKinematics(e);
}

// TOI(prevState→state 区間内の割合)を接触時刻へ変換する。重なりフォールバックでは
// toi=1(区間終端)なので、その場合は state.t にそのまま一致する。
export function contactTime(e: KinematicParticipant, toi: number): number {
