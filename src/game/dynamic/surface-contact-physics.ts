...
export class SurfaceContactPhysics {
  ...
  resolveOne(e: DynamicEntity, activeStage: StageOutcome, registry: EntityRegistry): void {
  ...
  resolveShared(
    entities: readonly DynamicEntity[], activeStage: StageOutcome, registry: EntityRegistry,
  ): void {
    ...
    for (const e of this.participantScratch) this.resolveAgainstCandidates(e, activeStage, registry);
  }

  // 個体1つが区間内で最も早く触れる天体を1体だけ解き、反発を当ててから
  // collideWithCelestialBody を呼ぶ。
  private resolveAgainstCandidates(
    e: DynamicEntity, activeStage: StageOutcome, registry: EntityRegistry,
  ): void {
