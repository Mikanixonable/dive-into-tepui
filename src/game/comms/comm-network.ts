...
import { commStationRange, communicationRange } from '../vessel/capabilities';
...
// 機体が持つ中継能力。通信基地区画を積んでいれば網の起点になり、通信モジュールだけなら
// 既に有効な中継点と繋がったときにだけ中継点になる。どちらも無ければ中継しない。
function relayOf(vessel: CommVessel): CommRelay | null {
  if (!vessel.alive) return null;
  const ground = commStationRange(vessel);
  const range = Math.max(ground, communicationRange(vessel));
  if (range <= 0) return null;
  return { id: vessel.id, pos: vessel.state.r, range, isGround: ground > 0 };
}
