...
import { StageWinCtx } from './stages/stage-definition';
...
export function showWinScreen(hud: Hud, sfx: Sfx, ctx: StageWinCtx, title?: string): void {
  const { shots, hits } = ctx.killCounter;
...
export function showScoreAttackResultScreen(hud: Hud, sfx: Sfx, killCounter: KillCounter, title?: string): void {
  const { shots, hits, kills } = killCounter;
  const acc = shots > 0 ? ((hits / shots) * 100).toFixed(1) : '0.0';
  const detailHtml =
    `撃墜数 ${kills} 機<br>` +
    `発射 ${shots} 発 / 命中 ${hits} 発 (命中率 ${acc}%)`;
