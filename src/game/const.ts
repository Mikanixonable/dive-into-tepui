...
// 消費されない弧(計画の区間・mu ≠ 0 の実体の予測)の積分ステップ数・保持サンプル数の上限。
...
// 消費されない弧(計画の区間・mu ≠ 0 の実体の予測)の刻みの下限 [s]。消費されない弧は状態を
...
// 消費されない弧(mu ≠ 0 の実体)を作り直す間隔 [s](sim 時間)。その弧は状態を決めず線としてだけ
// 読まれるので、実体から離れていくのは起点の張り直しで戻すだけでよい。伸び切った弧しか対象に
// しないので、短くしても伸長中の弧を振り出しへ戻すことはない。
export const ARC_REANCHOR_INTERVAL = 600;
