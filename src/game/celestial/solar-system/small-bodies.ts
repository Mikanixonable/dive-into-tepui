// 彗星核・小惑星・太陽系外縁天体。静的事実・運動・見た目を1体につき1箇所で組む。
import { SatelliteMotion, StarMotion } from '../../../physics/celestial-motion';
import { PlanetDef, planetDefForSimZero, SatelliteDef, satelliteDefForSimZero } from '../../../physics/celestial-body-def';
import { planetSystem } from '../../../physics/planet-system';
import { keplerPeriod } from '../../../physics/elements';
import { JULIAN_CENTURY, KeplerOrbit, planetOrbit } from '../../../physics/kepler-orbit';
import { AU } from '../../../physics/astronomical-unit';
import { GRAVITATIONAL_CONSTANT, MU_SUN } from './constants';
import { CelestialSurface } from '../../../render/celestial/celestial-surface';
import { CelestialEntity } from '../celestial-entity/celestial-entity';
import { SphereCelestialView } from '../../../render/celestial/celestial-entity/sphere-celestial-view';
import { CHARIKLO_RINGS, QUAOAR_RINGS } from './rings';
import { jplSatelliteOrbit } from './satellite-orbit-builders';

// 小天体に登録された天体の id。表示名も構築の網羅性もこの集合が決める。
export type SmallBodyId =
  | 'halley' | 'encke'
  | 'sedna' | 'quaoar' | 'weywot' | 'chariklo' | 'hygiea' | 'eros' | 'ryugu' | 'bennu'
  | 'orcus' | 'vanth' | 'gonggong' | 'salacia' | 'varuna' | 'ixion' | 'arrokoth' | 'chiron'
  | 'interamnia' | 'europa52' | 'davida' | 'juno' | 'psyche' | 'eunomia' | 'sylvia' | 'apophis'
  | 'didymos' | 'tempel1' | 'wild2' | 'hartley2' | 'cruithne' | 'kamooalewa' | 'tk7' | 'eureka';

// 長半径 a [m] から、周回天体の平均運動をケプラー第3法則で世紀あたりの度へ換算する。
// SBDB の公開周期を別途転記すると a と食い違いうるため、常にこれで導く。
function lRateFromSemiMajorAxis(a: number): number {
  return (360 * JULIAN_CENTURY) / keplerPeriod(a, MU_SUN);
}

// JPL Small-Body Database の単一元期の接触要素をそのまま受ける日心軌道。
function sbdbOrbit(p: {
  aAu: number;
  e: number;
  incDeg: number;
  raanDeg: number;
  lonPeriDeg: number;
  l0Deg: number;
}): KeplerOrbit {
  const a = p.aAu * AU;
  return planetOrbit({
    a,
    e: p.e,
    incDeg: p.incDeg,
    raanDeg: p.raanDeg,
    lonPeriDeg: p.lonPeriDeg,
    l0Deg: p.l0Deg,
    // 平均運動は長半径から導き、永年変化率は 0(SBDB は単一元期の要素しか公開しない)。
    lRateDegPerCentury: lRateFromSemiMajorAxis(a),
    raanRateDegPerCentury: 0,
    incRateDegPerCentury: 0,
    lonPeriRateDegPerCentury: 0,
    eRatePerCentury: 0,
    aRatePerCenturyAu: 0,
  });
}

// 彗星核の μ/半径は観測が乏しく粗い推定値。
const HALLEY: PlanetDef = {
  id: 'halley',
  mu: GRAVITATIONAL_CONSTANT * 2.2e14, // 粗い推定値(核質量 ~2.2e14 kg 相当)
  radius: 5.5e3, // 粗い推定値(核長径の半分程度)
  // 出典: https://ssd-api.jpl.nasa.gov/sbdb.api?sstr=1P&full-prec=true (元期 JD2439875.5、
  // 1968年の近日点通過に近い元期)。非重力効果(彗星核からのガス噴出による軌道擾乱)は
  // 未収録なので、周期・形状は正確だが軌道上の位置は年代が離れるほど粗くなる。
  orbit: planetOrbit({
    a: 17.92863504856923 * AU,
    e: 0.9679359956953211,
    incDeg: 162.1905300439129,
    raanDeg: 59.09894720612437,
    lonPeriDeg: 171.3403787,
    l0Deg: 237.2306867,
    lRateDegPerCentury: 474.2130029037993,
    raanRateDegPerCentury: 0,
    incRateDegPerCentury: 0,
    lonPeriRateDegPerCentury: 0,
    eRatePerCentury: 0,
    aRatePerCenturyAu: 0,
  }),
};

const ENCKE: PlanetDef = {
  id: 'encke',
  mu: GRAVITATIONAL_CONSTANT * 6e13, // 粗い推定値(核質量 ~6e13 kg 相当)
  radius: 2.4e3,
  // 出典: https://ssd-api.jpl.nasa.gov/sbdb.api?sstr=2P&full-prec=true (元期 JD2459847.5)
  orbit: planetOrbit({
    a: 2.219688710074586 * AU,
    e: 0.8477496967533629,
    incDeg: 11.41227811179314,
    raanDeg: 334.1935846036774,
    lonPeriDeg: 161.327831,
    l0Deg: 90.0257458,
    lRateDegPerCentury: 10885.695675063265,
    raanRateDegPerCentury: 0,
    incRateDegPerCentury: 0,
    lonPeriRateDegPerCentury: 0,
    eRatePerCentury: 0,
    aRatePerCenturyAu: 0,
  }),
};

// 太陽を公転する小天体32個。軌道要素は JPL Small-Body Database(sbdb.api、full-prec=true、
// 元期 JD2461200.5)の黄道座標・J2000 の a/e/i/Ω(om)/ω(w)/M(ma) から、raanDeg=Ω・
// lonPeriDeg=Ω+ω・l0Deg=Ω+ω+M として求めた(360を超えて構わない)。tempel1(JD2457470.5)・
// wild2(JD2458808.5)・hartley2(JD2457152.5)・bennu(JD2455562.5)だけは別の元期を持つ。
// どの元期も simTime=0 に対応させるので、同一の実在時刻の空を再現しているわけではない。
// 直径は SBDB または各天体の観測文献、三軸半径 [m](a>=b>=c)は探査機・掩蔽・レーダー・適応光学
// など天体ごとの観測による。GM を SBDB が持つのはヒギエア・プシケ・エロス・リュウグウ・ベンヌ・
// ディディモスだけで、他は天体ごとに質量の出典か見積り方をその場に書く。セドナは直径が未測定
// なので、掩蔽・熱赤外観測から広く引用される推定値(半径 500 km)を使う。
const SEDNA: PlanetDef = {
  id: 'sedna',
  // 質量は未測定。同規模の太陽系外縁天体(オルクス 1,190・クワオアー 1,790・ゴンゴン 1,740 kg/m^3)
  // が示す密度の中央 1,500 kg/m^3 を、この表の半径に掛けて見積もった。
  mu: GRAVITATIONAL_CONSTANT * 7.85e20,
  radius: 500000.0,
  orbit: sbdbOrbit({ aAu: 543.7195289, e: 0.8598825, incDeg: 11.9252758, raanDeg: 144.5061663, lonPeriDeg: 455.6049389, l0Deg: 814.2006333 }),
};

export const QUAOAR: PlanetDef = {
  id: 'quaoar',
  // GM は未測定だが、系の GM はウェイウォットの軌道長半径と周期からケプラー第3法則で 8.109e10
  // と決まる。ここから衛星ぶんを引いた。含意する密度 1,790 kg/m^3 は外縁天体として妥当。
  mu: 8.093e10,
  radius: 545000.0,
  rings: QUAOAR_RINGS,
  orbit: sbdbOrbit({ aAu: 43.1561765, e: 0.0352002, incDeg: 7.9915758, raanDeg: 188.9191248, lonPeriDeg: 352.1281758, l0Deg: 644.9769333 }),
};

// クワオアーの衛星ウェイウォット。基準面は黄道面の二次引用の要素で、歳差周期は未公開(=0)。
const WEYWOT: SatelliteDef = {
  id: 'weywot',
  mu: GRAVITATIONAL_CONSTANT * 2.4e18,
  radius: 72e3,
  orbit: jplSatelliteOrbit({ a: 13329e3, e: 0.01111, incDeg: 13.62, periodDays: 12.42727, nodePeriodYears: 0, apsisPeriodYears: 0 }),
};

const CHARIKLO: PlanetDef = {
  id: 'chariklo',
  // 質量は環の力学から求めた 5.9〜6.9e18 kg(Morgado et al. 2021)の中央値。
  mu: GRAVITATIONAL_CONSTANT * 6.4e18,
  radius: 143800.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 143800.0, b: 135200.0, c: 99100.0 },
  rings: CHARIKLO_RINGS,
  orbit: sbdbOrbit({ aAu: 15.7343733, e: 0.1708196, incDeg: 23.4319043, raanDeg: 300.476891, lonPeriDeg: 541.6834978, l0Deg: 671.7725806 }),
};

const HYGIEA: PlanetDef = {
  id: 'hygiea',
  mu: 7000000000.0,
  radius: 217000.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 217000.0, b: 213000.0, c: 210000.0 },
  orbit: sbdbOrbit({ aAu: 3.150974, e: 0.1067093, incDeg: 3.8295299, raanDeg: 283.1198928, lonPeriDeg: 595.5441315, l0Deg: 847.5785557 }),
};

const EROS: PlanetDef = {
  id: 'eros',
  mu: 446300.0,
  radius: 17200.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 17200.0, b: 5600.0, c: 5600.0 },
  orbit: sbdbOrbit({ aAu: 1.4582437, e: 0.222878, incDeg: 10.8285441, raanDeg: 304.2679713, lonPeriDeg: 483.1861032, l0Deg: 545.6975582 }),
};

const RYUGU: PlanetDef = {
  id: 'ryugu',
  mu: 30.0,
  radius: 448.0,
  orbit: sbdbOrbit({ aAu: 1.1909189, e: 0.191073, incDeg: 5.8664425, raanDeg: 251.2897124, lonPeriDeg: 462.8987063, l0Deg: 525.2393806 }),
};

const BENNU: PlanetDef = {
  id: 'bennu',
  mu: 4.8904,
  radius: 252.35, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 252.35, b: 245.9, c: 228.35 },
  orbit: sbdbOrbit({ aAu: 1.126391, e: 0.2037451, incDeg: 6.0349438, raanDeg: 2.0608662, lonPeriDeg: 68.283927, l0Deg: 169.987879 }),
};

export const ORCUS: PlanetDef = {
  id: 'orcus',
  // GM は未測定だが、系の GM はヴァンスの軌道長半径と周期からケプラー第3法則で 4.237e10 と
  // 決まる。ここから衛星ぶんを引いた。含意する密度 1,190 kg/m^3 は外縁天体として妥当。
  mu: 3.656e10,
  radius: 479200.0,
  orbit: sbdbOrbit({ aAu: 39.377, e: 0.22052, incDeg: 20.5568, raanDeg: 268.4054, lonPeriDeg: 341.9739, l0Deg: 531.0712 }),
};

// オルクスの衛星ヴァンス。基準面は黄道面の二次引用の要素で、歳差周期は未公開(=0)。
const VANTH: SatelliteDef = {
  id: 'vanth',
  mu: GRAVITATIONAL_CONSTANT * 8.7e19,
  radius: 221.25e3,
  orbit: jplSatelliteOrbit({ a: 8999.8e3, e: 0.00091, incDeg: 90.54, periodDays: 9.539154, nodePeriodYears: 0, apsisPeriodYears: 0 }),
};

const GONGGONG: PlanetDef = {
  id: 'gonggong',
  // 質量は衛星シャンリュウの軌道から求めた実測値。衛星自体はこの表に登録していない。
  mu: GRAVITATIONAL_CONSTANT * 1.75e21,
  radius: 615000.0,
  orbit: sbdbOrbit({ aAu: 66.867, e: 0.50425, incDeg: 30.8991, raanDeg: 336.8383, lonPeriDeg: 543.4615, l0Deg: 655.1263 }),
};

const SALACIA: PlanetDef = {
  id: 'salacia',
  // 質量は衛星アクタイアの軌道から求めた**系の合計**。衛星をこの表に登録していないので、
  // 分割せずそのまま本体へ与えている(アクタイアは系の 4% 程度)。
  mu: GRAVITATIONAL_CONSTANT * 4.861e20,
  radius: 419000.0,
  orbit: sbdbOrbit({ aAu: 42.055, e: 0.1046, incDeg: 23.9272, raanDeg: 280.2543, lonPeriDeg: 589.2316, l0Deg: 723.9095 }),
};

const VARUNA: PlanetDef = {
  id: 'varuna',
  // 質量は自転による扁平から求めた密度 992 kg/m^3(Lacerda & Jewitt 2007)に、掩蔽で得た
  // 直径 654 km を掛けたもの。TODO: この半径は古い直径 900 km 由来で質量と整合しない —
  // 掩蔽由来の半径 327 km へ揃えると、表面重力も外形も現在の観測に合う。
  mu: GRAVITATIONAL_CONSTANT * 1.45e20,
  radius: 450000.0,
  orbit: sbdbOrbit({ aAu: 43.2, e: 0.051615, incDeg: 17.1405, raanDeg: 97.2158, lonPeriDeg: 370.5748, l0Deg: 486.2427 }),
};

const IXION: PlanetDef = {
  id: 'ixion',
  // 質量・密度とも未測定。同規模の外縁天体に共通する 1,000 kg/m^3 を、この表の半径に掛けた。
  mu: GRAVITATIONAL_CONSTANT * 1.77e20,
  radius: 348390.0,
  orbit: sbdbOrbit({ aAu: 39.346, e: 0.24356, incDeg: 19.6625, raanDeg: 71.0808, lonPeriDeg: 371.7031, l0Deg: 666.6707 }),
};

const ARROKOTH: PlanetDef = {
  id: 'arrokoth',
  // 質量はニューホライズンズの接近観測から求めた公称値(Keane et al. 2022)。
  mu: GRAVITATIONAL_CONSTANT * 7.485e14,
  radius: 17500.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 17500.0, b: 10000.0, c: 5000.0 },
  orbit: sbdbOrbit({ aAu: 44.053, e: 0.03556, incDeg: 2.4506, raanDeg: 159.0377, lonPeriDeg: 347.8884, l0Deg: 658.8723 }),
};

const CHIRON: PlanetDef = {
  id: 'chiron',
  // 質量は未測定。同じケンタウルス族で密度が実測されたカリクローの 800 kg/m^3 を、この表の
  // 半径に掛けて見積もった。
  mu: GRAVITATIONAL_CONSTANT * 8.38e17,
  radius: 63000.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 63000.0, b: 54500.0, c: 34000.0 },
  orbit: sbdbOrbit({ aAu: 13.68427, e: 0.379766, incDeg: 6.93057, raanDeg: 209.2961, lonPeriDeg: 548.5839, l0Deg: 765.3038 }),
};

const INTERAMNIA: PlanetDef = {
  id: 'interamnia',
  // 質量は VLT の掩蔽・測光解析による実測値。SBDB の GM(5 km^3/s^2、Landgraf 1992)は
  // この直径に対し密度 5 g/cm^3 を含意し、C型としてありえないため採らない。
  mu: GRAVITATIONAL_CONSTANT * 3.5e19,
  radius: 181000.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 181000.0, b: 174000.0, c: 155000.0 },
  orbit: sbdbOrbit({ aAu: 3.056812, e: 0.155059, incDeg: 17.3153, raanDeg: 280.1672, lonPeriDeg: 374.2289, l0Deg: 595.3737 }),
};

const EUROPA52: PlanetDef = {
  id: 'europa52',
  // 質量は VLT の掩蔽・測光解析による実測値。
  mu: GRAVITATIONAL_CONSTANT * 2.4e19,
  radius: 151959.0,
  orbit: sbdbOrbit({ aAu: 3.094136, e: 0.112483, incDeg: 7.4815, raanDeg: 128.5734, lonPeriDeg: 471.3774, l0Deg: 820.3002 }),
};

const DAVIDA: PlanetDef = {
  id: 'davida',
  // 質量は VLT の掩蔽・測光解析による実測値。
  mu: GRAVITATIONAL_CONSTANT * 2.66e19,
  radius: 178500.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 178500.0, b: 147000.0, c: 115500.0 },
  orbit: sbdbOrbit({ aAu: 3.161793, e: 0.189373, incDeg: 15.9498, raanDeg: 107.5541, lonPeriDeg: 444.084, l0Deg: 514.52 }),
};

const JUNO: PlanetDef = {
  id: 'juno',
  // 質量は VLT の掩蔽・測光解析による実測値。
  mu: GRAVITATIONAL_CONSTANT * 2.7e19,
  radius: 123298.0,
  orbit: sbdbOrbit({ aAu: 2.67099, e: 0.2557, incDeg: 12.9866, raanDeg: 169.8116, lonPeriDeg: 417.7067, l0Deg: 680.439 }),
};

const PSYCHE: PlanetDef = {
  id: 'psyche',
  // GM は SBDB(Farnocchia et al. 2024)。
  mu: 1.601e9,
  radius: 139000.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 139000.0, b: 119000.0, c: 85500.0 },
  orbit: sbdbOrbit({ aAu: 2.92572, e: 0.134932, incDeg: 3.0987, raanDeg: 149.9754, lonPeriDeg: 380.0081, l0Deg: 459.7775 }),
};

const EUNOMIA: PlanetDef = {
  id: 'eunomia',
  // 質量は他の小惑星への摂動から求めた実測値(Siltala & Granvik 2022)。
  mu: GRAVITATIONAL_CONSTANT * 3.026e19,
  radius: 170000.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 170000.0, b: 124000.0, c: 114500.0 },
  orbit: sbdbOrbit({ aAu: 2.641959, e: 0.187771, incDeg: 11.7614, raanDeg: 292.8808, lonPeriDeg: 391.3421, l0Deg: 551.0312 }),
};

const SYLVIA: PlanetDef = {
  id: 'sylvia',
  // 質量は2個の衛星の軌道から求めた実測値(Marchis et al. 2005)。衛星は登録していない。
  mu: GRAVITATIONAL_CONSTANT * 1.476e19,
  radius: 181500.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 181500.0, b: 124500.0, c: 95500.0 },
  orbit: sbdbOrbit({ aAu: 3.490931, e: 0.094242, incDeg: 10.8493, raanDeg: 72.946, lonPeriDeg: 340.0475, l0Deg: 463.9674 }),
};

const APOPHIS: PlanetDef = {
  id: 'apophis',
  // 質量は直径とS型の標準的な密度から置いた推定値(JPL の衝突リスク評価が使う値)。
  mu: GRAVITATIONAL_CONSTANT * 6.1e10,
  radius: 170.0,
  orbit: sbdbOrbit({ aAu: 0.922359, e: 0.191149, incDeg: 3.340997, raanDeg: 203.8937, lonPeriDeg: 330.5733, l0Deg: 505.9037 }),
};

const DIDYMOS: PlanetDef = {
  id: 'didymos',
  mu: GRAVITATIONAL_CONSTANT * 5.2e11,
  radius: 398.5, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 398.5, b: 391.5, c: 380.5 },
  orbit: sbdbOrbit({ aAu: 1.64271, e: 0.383123, incDeg: 3.413877, raanDeg: 72.9858, lonPeriDeg: 392.5665, l0Deg: 653.4278 }),
};

const TEMPEL1: PlanetDef = {
  id: 'tempel1',
  // 質量はディープインパクトが求めた密度 620 kg/m^3 と核の寸法から(A'Hearn et al. 2005)。
  mu: GRAVITATIONAL_CONSTANT * 7.5e13,
  radius: 3000.0,
  orbit: sbdbOrbit({ aAu: 3.146134, e: 0.5097, incDeg: 10.4734, raanDeg: 68.7536, lonPeriDeg: 247.9509, l0Deg: 584.5363 }),
};

const WILD2: PlanetDef = {
  id: 'wild2',
  // 質量は核の寸法と彗星核の標準的な密度 600 kg/m^3 から。
  mu: GRAVITATIONAL_CONSTANT * 2.3e13,
  radius: 2000.0,
  orbit: sbdbOrbit({ aAu: 3.449746, e: 0.5374, incDeg: 3.237, raanDeg: 136.1102, lonPeriDeg: 177.8354, l0Deg: 365.4321 }),
};

const HARTLEY2: PlanetDef = {
  id: 'hartley2',
  // 質量は EPOXI が求めた密度 200〜400 kg/m^3 と核の寸法から(Thomas et al. 2013)。
  mu: GRAVITATIONAL_CONSTANT * 3.0e11,
  radius: 800.0,
  orbit: sbdbOrbit({ aAu: 3.475652, e: 0.6936, incDeg: 13.5995, raanDeg: 219.7422, lonPeriDeg: 401.064, l0Deg: 652.8462 }),
};

const CRUITHNE: PlanetDef = {
  id: 'cruithne',
  // 質量は未測定。この表の半径(NEOWISE の直径 2.07 km)に、S型の標準的な密度 2,000 kg/m^3
  // を掛けた。
  mu: GRAVITATIONAL_CONSTANT * 9.3e12,
  radius: 1035.5,
  orbit: sbdbOrbit({ aAu: 0.997797, e: 0.5149, incDeg: 19.8024, raanDeg: 126.1887, lonPeriDeg: 170.0717, l0Deg: 352.2041 }),
};

const KAMOOALEWA: PlanetDef = {
  id: 'kamooalewa',
  // 質量は未測定。この表の半径に、S型の標準的な密度 2,000 kg/m^3 を掛けた。
  mu: GRAVITATIONAL_CONSTANT * 3.29e8,
  radius: 34.0, // 三軸の最長半軸(外接球)
  shape: { kind: 'triaxial', a: 34.0, b: 23.0, c: 19.5 },
  orbit: sbdbOrbit({ aAu: 1.00081, e: 0.10224, incDeg: 7.8026, raanDeg: 65.5932, lonPeriDeg: 369.9564, l0Deg: 613.3436 }),
};

const TK7: PlanetDef = {
  id: 'tk7',
  // 質量は未測定。この表の半径に、S型の標準的な密度 2,000 kg/m^3 を掛けた。
  mu: GRAVITATIONAL_CONSTANT * 5.7e10,
  radius: 189.5,
  orbit: sbdbOrbit({ aAu: 0.998508, e: 0.19027, incDeg: 20.9057, raanDeg: 96.4145, lonPeriDeg: 142.4843, l0Deg: 286.9046 }),
};

const EUREKA: PlanetDef = {
  id: 'eureka',
  // 質量は未測定。この表の半径に、S型の標準的な密度 2,000 kg/m^3 を掛けた。
  mu: GRAVITATIONAL_CONSTANT * 6.94e12,
  radius: 939.0,
  orbit: sbdbOrbit({ aAu: 1.523573, e: 0.06485, incDeg: 20.2811, raanDeg: 245.0121, lonPeriDeg: 340.4941, l0Deg: 677.4051 }),
};

// 彗星核・小惑星・太陽系外縁天体の表示名。
export const SMALL_BODY_NAMES: Record<SmallBodyId, string> = {
  halley: 'ハレー彗星',
  encke: 'エンケ彗星',
  sedna: 'セドナ',
  quaoar: 'クワオアー',
  weywot: 'ウェイウォット',
  chariklo: 'カリクロー',
  hygiea: 'ヒギエア',
  eros: 'エロス',
  ryugu: 'リュウグウ',
  bennu: 'ベンヌ',
  orcus: 'オルクス',
  vanth: 'ヴァンス',
  gonggong: 'ゴンゴン',
  salacia: 'サラキア',
  varuna: 'ヴァルナ',
  ixion: 'イクシオン',
  arrokoth: 'アロコス',
  chiron: 'キロン',
  interamnia: 'インテラムニア',
  europa52: 'エウロパ (52)',
  davida: 'ダビダ',
  juno: 'ジュノー',
  psyche: 'プシケ',
  eunomia: 'エウノミア',
  sylvia: 'シルビア',
  apophis: 'アポフィス',
  didymos: 'ディディモス',
  tempel1: 'テンペル第1彗星',
  wild2: 'ワイルド第2彗星',
  hartley2: 'ハートレー第2彗星',
  cruithne: 'クルースン',
  kamooalewa: 'カモオアレワ',
  tk7: '2010 TK7',
  eureka: 'エウレカ',
};

// 彗星核・小惑星・太陽系外縁天体を組む。宣言順がそのまま重力源配列・一覧の順序になる。
export function smallBodies(
  sun: StarMotion, simZeroEt: number,
): Record<SmallBodyId, CelestialEntity> {
  // 衛星を持つ天体の運動は、子の主天体として渡すため先に組む。
  const quaoar = planetSystem(planetDefForSimZero(QUAOAR, simZeroEt), sun);
  const orcus = planetSystem(planetDefForSimZero(ORCUS, simZeroEt), sun);
  return {
    // ハレー彗星 A_B=0.016(幾何 0.04 x q=0.393)
    halley: new CelestialEntity(
      planetSystem(planetDefForSimZero(HALLEY, simZeroEt), sun).body,
      SMALL_BODY_NAMES.halley, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0160, 0.0160, 0.0160])),
    ),
    // エンケ彗星 A_B=0.02(幾何 0.05 x q=0.393)
    encke: new CelestialEntity(
      planetSystem(planetDefForSimZero(ENCKE, simZeroEt), sun).body,
      SMALL_BODY_NAMES.encke, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0200, 0.0200, 0.0200])),
    ),
    // セドナ A_B=0.15(幾何 0.32 x q=0.461)
    sedna: new CelestialEntity(
      planetSystem(planetDefForSimZero(SEDNA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.sedna, 'dwarf', new SphereCelestialView(CelestialSurface.solid([0.1759, 0.1453, 0.1203])),
    ),
    // クワオアー A_B=0.05(幾何 0.109 x q=0.461)
    quaoar: new CelestialEntity(
      quaoar.body, SMALL_BODY_NAMES.quaoar, 'dwarf', new SphereCelestialView(CelestialSurface.solid([0.0616, 0.0479, 0.0363])),
    ),
    // ウェイウォット A_B=0.046(分類既定 幾何 0.10 x q=0.461)
    weywot: new CelestialEntity(
      new SatelliteMotion(satelliteDefForSimZero(WEYWOT, simZeroEt), quaoar),
      SMALL_BODY_NAMES.weywot, 'satellite', new SphereCelestialView(CelestialSurface.solid([0.0527, 0.0447, 0.0389])),
    ),
    // カリクロー A_B=0.014(幾何 0.035 x q=0.393)
    chariklo: new CelestialEntity(
      planetSystem(planetDefForSimZero(CHARIKLO, simZeroEt), sun).body,
      SMALL_BODY_NAMES.chariklo, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0161, 0.0137, 0.0108])),
    ),
    // ヒギエア A_B=0.028(幾何 0.072 x q=0.393)
    hygiea: new CelestialEntity(
      planetSystem(planetDefForSimZero(HYGIEA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.hygiea, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0301, 0.0278, 0.0239])),
    ),
    // エロス A_B=0.115(幾何 0.25 x q=0.461)
    eros: new CelestialEntity(
      planetSystem(planetDefForSimZero(EROS, simZeroEt), sun).body,
      SMALL_BODY_NAMES.eros, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.1367, 0.1129, 0.0718])),
    ),
    // リュウグウ A_B=0.018(幾何 0.045 x q=0.393)
    ryugu: new CelestialEntity(
      planetSystem(planetDefForSimZero(RYUGU, simZeroEt), sun).body,
      SMALL_BODY_NAMES.ryugu, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0212, 0.0174, 0.0141])),
    ),
    // ベンヌ A_B=0.017(幾何 0.044 x q=0.393)
    bennu: new CelestialEntity(
      planetSystem(planetDefForSimZero(BENNU, simZeroEt), sun).body,
      SMALL_BODY_NAMES.bennu, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0196, 0.0166, 0.0138])),
    ),
    // オルクス A_B=0.106(幾何 0.23 x q=0.461)
    orcus: new CelestialEntity(
      orcus.body, SMALL_BODY_NAMES.orcus, 'dwarf', new SphereCelestialView(CelestialSurface.solid([0.1053, 0.1053, 0.1146])),
    ),
    // ヴァンス A_B=0.031(幾何 0.08 x q=0.393)
    vanth: new CelestialEntity(
      new SatelliteMotion(satelliteDefForSimZero(VANTH, simZeroEt), orcus),
      SMALL_BODY_NAMES.vanth, 'satellite', new SphereCelestialView(CelestialSurface.solid([0.0355, 0.0301, 0.0262])),
    ),
    // ゴンゴン A_B=0.065(幾何 0.14 x q=0.461)
    gonggong: new CelestialEntity(
      planetSystem(planetDefForSimZero(GONGGONG, simZeroEt), sun).body,
      SMALL_BODY_NAMES.gonggong, 'dwarf', new SphereCelestialView(CelestialSurface.solid([0.1126, 0.0540, 0.0336])),
    ),
    // サラキア A_B=0.017(幾何 0.042 x q=0.393)
    salacia: new CelestialEntity(
      planetSystem(planetDefForSimZero(SALACIA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.salacia, 'dwarf', new SphereCelestialView(CelestialSurface.solid([0.0151, 0.0171, 0.0216])),
    ),
    // ヴァルナ A_B=0.059(幾何 0.127 x q=0.461)
    varuna: new CelestialEntity(
      planetSystem(planetDefForSimZero(VARUNA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.varuna, 'dwarf', new SphereCelestialView(CelestialSurface.solid([0.0645, 0.0585, 0.0476])),
    ),
    // イクシオン A_B=0.05(幾何 0.108 x q=0.461)
    ixion: new CelestialEntity(
      planetSystem(planetDefForSimZero(IXION, simZeroEt), sun).body,
      SMALL_BODY_NAMES.ixion, 'dwarf', new SphereCelestialView(CelestialSurface.solid([0.0589, 0.0485, 0.0392])),
    ),
    // アロコス A_B=0.065(幾何 0.165 x q=0.393)
    arrokoth: new CelestialEntity(
      planetSystem(planetDefForSimZero(ARROKOTH, simZeroEt), sun).body,
      SMALL_BODY_NAMES.arrokoth, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.1783, 0.0365, 0.0135])),
    ),
    // キロン A_B=0.063(幾何 0.16 x q=0.393)
    chiron: new CelestialEntity(
      planetSystem(planetDefForSimZero(CHIRON, simZeroEt), sun).body,
      SMALL_BODY_NAMES.chiron, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0695, 0.0623, 0.0512])),
    ),
    // インテラムニア A_B=0.029(幾何 0.074 x q=0.393)
    interamnia: new CelestialEntity(
      planetSystem(planetDefForSimZero(INTERAMNIA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.interamnia, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0308, 0.0288, 0.0251])),
    ),
    // エウロパ (52) A_B=0.023(幾何 0.058 x q=0.393)
    europa52: new CelestialEntity(
      planetSystem(planetDefForSimZero(EUROPA52, simZeroEt), sun).body,
      SMALL_BODY_NAMES.europa52, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0251, 0.0228, 0.0192])),
    ),
    // ダビダ A_B=0.021(幾何 0.054 x q=0.393)
    davida: new CelestialEntity(
      planetSystem(planetDefForSimZero(DAVIDA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.davida, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0230, 0.0208, 0.0173])),
    ),
    // ジュノー A_B=0.11(幾何 0.238 x q=0.461)
    juno: new CelestialEntity(
      planetSystem(planetDefForSimZero(JUNO, simZeroEt), sun).body,
      SMALL_BODY_NAMES.juno, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.1285, 0.1070, 0.0849])),
    ),
    // プシケ A_B=0.055(幾何 0.12 x q=0.461)
    psyche: new CelestialEntity(
      planetSystem(planetDefForSimZero(PSYCHE, simZeroEt), sun).body,
      SMALL_BODY_NAMES.psyche, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0606, 0.0540, 0.0479])),
    ),
    // エウノミア A_B=0.096(幾何 0.209 x q=0.461)
    eunomia: new CelestialEntity(
      planetSystem(planetDefForSimZero(EUNOMIA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.eunomia, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.1125, 0.0942, 0.0654])),
    ),
    // シルビア A_B=0.018(幾何 0.045 x q=0.393)
    sylvia: new CelestialEntity(
      planetSystem(planetDefForSimZero(SYLVIA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.sylvia, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0198, 0.0177, 0.0157])),
    ),
    // アポフィス A_B=0.161(幾何 0.35 x q=0.461)
    apophis: new CelestialEntity(
      planetSystem(planetDefForSimZero(APOPHIS, simZeroEt), sun).body,
      SMALL_BODY_NAMES.apophis, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.1698, 0.1595, 0.1496])),
    ),
    // ディディモス A_B=0.069(幾何 0.15 x q=0.461)
    didymos: new CelestialEntity(
      planetSystem(planetDefForSimZero(DIDYMOS, simZeroEt), sun).body,
      SMALL_BODY_NAMES.didymos, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0733, 0.0685, 0.0616])),
    ),
    // テンペル第1彗星 A_B=0.016(幾何 0.04 x q=0.393)
    tempel1: new CelestialEntity(
      planetSystem(planetDefForSimZero(TEMPEL1, simZeroEt), sun).body,
      SMALL_BODY_NAMES.tempel1, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0160, 0.0160, 0.0160])),
    ),
    // ワイルド第2彗星 A_B=0.012(幾何 0.03 x q=0.393)
    wild2: new CelestialEntity(
      planetSystem(planetDefForSimZero(WILD2, simZeroEt), sun).body,
      SMALL_BODY_NAMES.wild2, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0120, 0.0120, 0.0120])),
    ),
    // ハートレー第2彗星 A_B=0.011(幾何 0.028 x q=0.393)
    hartley2: new CelestialEntity(
      planetSystem(planetDefForSimZero(HARTLEY2, simZeroEt), sun).body,
      SMALL_BODY_NAMES.hartley2, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0110, 0.0110, 0.0110])),
    ),
    // クルースン A_B=0.069(分類既定 幾何 0.15 x q=0.461)
    cruithne: new CelestialEntity(
      planetSystem(planetDefForSimZero(CRUITHNE, simZeroEt), sun).body,
      SMALL_BODY_NAMES.cruithne, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0763, 0.0682, 0.0557])),
    ),
    // カモオアレワ A_B=0.111(幾何 0.24 x q=0.461)
    kamooalewa: new CelestialEntity(
      planetSystem(planetDefForSimZero(KAMOOALEWA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.kamooalewa, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.1394, 0.1073, 0.0643])),
    ),
    // 2010 TK7 A_B=0.039(分類既定 幾何 0.10 x q=0.393)
    tk7: new CelestialEntity(
      planetSystem(planetDefForSimZero(TK7, simZeroEt), sun).body,
      SMALL_BODY_NAMES.tk7, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.0432, 0.0383, 0.0337])),
    ),
    // エウレカ A_B=0.18(幾何 0.39 x q=0.461)
    eureka: new CelestialEntity(
      planetSystem(planetDefForSimZero(EUREKA, simZeroEt), sun).body,
      SMALL_BODY_NAMES.eureka, 'smallBody', new SphereCelestialView(CelestialSurface.solid([0.2377, 0.1718, 0.0912])),
    ),
  };
}
