...
export class Earth extends CelestialEntity {
  ...
  constructor(motion: PlanetMotion, name: string, atmosphereOptics: AtmosphereOptics | null) {
  ...
  sync(
    ...
    // 自転角の正本は physics の自転モデル。地球自転系の向きと地表テクスチャの向きを一致させる。
    const orientation = this.motion.orientationAt(displayTime);
    if (orientation !== null) this.earth.setRotation(orientation.spinAngle);
