// 使用するキー割り当ての唯一の定義。キーを読む側(Input.down / Input.takeKey)と、
// キーを説明する側(ヘルプ表・操作バー・タッチ操作パッド・マーカーのラベル)は、
// どちらもこの表だけを参照する。割り当てを2箇所に書けば必ず食い違うため、
// キーコードも表示名もここ以外には書かない。
//
// 操作の「意味」ごとに1エントリを持つ。同じキーに割り当てられた別々の操作
// (装填と再出撃の [R]、並進と Δv 編集の W/S/A/D/Q/E)は、キーが同じでも
// 別エントリにする — 消費するモジュールも説明文も別だから。
//
// 将来のキーコンフィグは、この表の差し替えとして実現する。
