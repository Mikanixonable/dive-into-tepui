...
import { fmtDateTime, fmtTime } from './hud/utils';
...
// simTime = 0 に対応する絶対時刻 [unix秒]。絶対日時表示にのみ使う。
const SIM_EPOCH_SEC = Date.parse(C.SIM_EPOCH_UTC) / 1000;

// 固定長プリセットの秒数。'orbit'/'manual' はこのテーブルに含めず durationSec 側で個別に扱う。
...
export class DisplayTimeManager {
  ...
  sync(simTime: number, referencePeriod: number): void {
    ...
    this.panel.setTicks(this.tickLabels(simTime));
  }
  ...
  private tickLabels(simTime: number): readonly string[] {
    const labels: string[] = [];
    for (let i = 0; i < TICK_COUNT; i++) labels.push(this.futureTimeLabel(i / (TICK_COUNT - 1), simTime));
    return labels;
  }
}
