...
export interface ObjectPickable {
  ...
  // 軌道物体一覧の行へ添える補助表示。
  listDetail(celestialSystem: CelestialBodies, viewer: Viewer | null, displayTime: number): string;
  // 軌道物体一覧の検索が照合する文字列。行に出さない情報を含めてよい。
  listSearchText(celestialSystem: CelestialBodies, viewer: Viewer | null, displayTime: number): string;
  ...
  menuItems(
    celestialSystem: CelestialBodies, viewer: Viewer | null, navTargetId: string | null,
  ...
  // 候補の位置を引き直す時刻。
  propertyRows(
    celestialSystem: CelestialBodies, viewer: Viewer | null, simTime: number, displayTime: number,
