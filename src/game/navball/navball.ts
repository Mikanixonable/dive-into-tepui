...
import {
  applyGridToggle, CelestialGridVisibility, DEFAULT_GRID_VISIBILITY, normalizeGridVisibility,
...
export class Navball {
  ...
  // 天球グリッドの表示設定が変わるたびに呼ばれる(同上)。
  onGridVisibilityChange: ((visibility: CelestialGridVisibility) => void) | null = null;
  ...
  constructor(viewOptionsPanel: ViewOptionsPanel) {
    viewOptionsPanel.onGridToggle = (key, on) => {
      ...
      this.onGridVisibilityChange?.(this.gridVisibility);
    };
    viewOptionsPanel.setGridVisibility(this.gridVisibility);
