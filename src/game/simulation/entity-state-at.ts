...
// t <= trajectory の現在時刻なら保持区間の内挿(at)、それより先なら center まわりの
// 二体ケプラー外挿(extrapolatedAt)で答える。両者とも先端以前は内挿に落ちる
...
export function trajectoryStateAt(
  trajectory: DynamicTrajectory, t: number, center: CelestialBody, ephemeris: Ephemeris,
): KinematicState | null {
...
export function entityStateAt(
  entity: GameEntity, t: number, center: CelestialBody, ephemeris: Ephemeris,
): KinematicState | null {
  if (t <= entity.state.t) return entity.actual.at(t);
  const predicted = entity.predicted;
  if (predicted === null) return null;
  return trajectoryStateAt(predicted, t, center, ephemeris);
}
