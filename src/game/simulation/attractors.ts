...
export type ClassifiedAttractors = {
  ...
  readonly grid: SpatialGrid<CelestialBody>;
};

// mu の重い順 GRAVITY_ALWAYS_COUNT 本目の値。同値の天体をまとめて always 側へ入れるため、
// 順位ではなく mu の値を返す。
function alwaysThresholdMu(attractors: readonly CelestialBody[]): number {
...
const griddedScratch: CelestialBody[] = [];

// グリッドへ載せた天体のうち最も重いものの引力が GRAVITY_NEGLIGIBLE_ACCEL まで落ちる距離。
// gridded が空のときのセル一辺は結果に影響しないので任意の正数でよい。
function gridCellSize(gridded: readonly CelestialBody[]): number {
  let heaviestMu = 0;
...
export function classifyAttractors(attractors: readonly CelestialBody[]): ClassifiedAttractors {
  ...
  const grid = new SpatialGrid<CelestialBody>(gridCellSize(gridded));
