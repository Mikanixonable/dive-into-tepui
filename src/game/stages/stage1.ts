...
export class Stage1 extends Stage {
  ...
  protected init(entities: EntityManager): void {
    ...
    // 各種軌道パターンの敵を配置する
    this.addEnemy(generatePhasedEnemy('HOSTILE-α', base, 1400, 2, 0xff4a3d, C.COLOR_ENEMY_ORBIT_LINE, worldSfx, fx, scene), entities);
    this.addEnemy(generateCoellipticEnemy('HOSTILE-β', base, -2800, 2500, 2, 0xff7a2d, C.COLOR_ENEMY_ORBIT_LINE, worldSfx, fx, scene), entities);
    this.addEnemy(generateCrossingEnemy('HOSTILE-γ', base, 2200, 2, 0xe0409f, C.COLOR_ENEMY_ORBIT_LINE, worldSfx, fx, scene), entities);
    this.addEnemy(generateEllipticEnemy('HOSTILE-δ', base, 5000, 3, 0xbf3dff, C.COLOR_ENEMY_ORBIT_LINE, worldSfx, fx, scene), entities);
