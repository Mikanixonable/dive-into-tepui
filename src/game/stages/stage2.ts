...
export class Stage2 extends Stage {
  ...
  init(player: Player, entities: EntityManager): number {
    ...
    // 通常軌道の敵
    this.addEnemy(generatePhasedEnemy('HOSTILE-α', base, 1800, 2, 0xff4a3d, C.COLORS.ENEMY_ORBIT_LINE, hud, sfx, fx, scene), entities);
    this.addEnemy(generateCoellipticEnemy('HOSTILE-β', base, -2600, 3000, 2, 0xff7a2d, C.COLORS.ENEMY_ORBIT_LINE, hud, sfx, fx, scene), entities);
    // モルニヤ級の高楕円軌道の敵
    this.addEnemy(generateMolniyaEnemy('MOLNIYA-γ', base.t, 0.4, 2.6, 3, 0xe0409f, C.COLORS.ENEMY_ORBIT_LINE, hud, sfx, fx, scene), entities);
    this.addEnemy(generateMolniyaEnemy('MOLNIYA-δ', base.t, 2.5, 0.9, 3, 0xbf3dff, C.COLORS.ENEMY_ORBIT_LINE, hud, sfx, fx, scene), entities);
    this.addEnemy(generateMolniyaEnemy('MOLNIYA-ε', base.t, 4.6, 3.8, 3, 0xff2d6b, C.COLORS.ENEMY_ORBIT_LINE, hud, sfx, fx, scene), entities);
    return 5;
  }
  // 敵の行動と補給品の湧きを進める。
