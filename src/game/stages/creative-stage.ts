  ...
  private syncBaseMarkers(project: ProjectFn, scale: ScaleFn, displayTime: number, overviewMode: boolean, visibility: MapVisibilityPolicy | null): void {
    ...
    for (const [i, base] of bases.entries()) {
      ...
      if (overviewMode) {
        ...
        this._markerManager.set(key, 'mk-base', display?.icon === false ? '' : '▲', p.x, p.y, p.front, display?.label === false ? '' : label, 1, undefined, rotationDeg);
        this._markerManager.hide(bearingKey);
      } else {
        this._markerManager.set(key, 'mk-poi', display?.icon === false ? '' : '●', p.x, p.y, p.front, display?.label === false ? '' : label);
        this._markerManager.setBearing(bearingKey, 'mk-poi', '△', p, display?.label === false ? '' : label, 0.9);
      }
    }
