...
import { Elements, apsisAltitudes, trueAnomalyAt } from '../../physics/elements';
...
function raanArgpFromBasis(el: Elements): { raanDeg: number; argpDeg: number } {
...
export function elementsFormFromState(state: OrbitState, bodies: readonly Attractor[]): ElementsForm | null {
  const strongest = strongestAttractor(state.r, bodies);
  const body = bodyDef(strongest.id).kind === 'star' ? bodies.find((b) => b.id === 'earth') ?? strongest : strongest;
  const el = elementsAround(state, body);
  ...
  const relative = toFrameState(frameOfAttractor(body), state);
