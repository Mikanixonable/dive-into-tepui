...
export type EllipticPlacementInput = {
  centerRadius: number; mu: number; sizeMode: 'apsides' | 'semiMajorEcc' | 'periodEcc';
...
export function validateEllipticPlacement(input: EllipticPlacementInput): string | null {
  ...
  return a > 0 && a * (1 - input.eccentricity) > input.centerRadius ? null : '近地点が天体表面より上の楕円軌道にしてください';
}
