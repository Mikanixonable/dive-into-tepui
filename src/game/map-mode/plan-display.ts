...
export class PlanDisplay {
  ...
  ghostLabel(plan: Plan, ctx: PlanCtx, sliderT: number): string {
    const duration = this.predictDurationSec(ctx);
    const t = this.displayTime(ctx.simTime, duration, sliderT);
  ...
  update(plan: Plan, ctx: PlanCtx, origin: Vec3, mapFrameRotating: boolean, sliderT: number, project: ProjectFn): void {
    this.line.setVisible(true);
    this.line.setOrigin(origin);
    plan.maybeRefresh(ctx, this.predictDurationSec(ctx));
    ...
    const p = project(sub(this.toDisplayFrame(sample.r, t, ctx, mapFrameRotating), origin));
    this.markers.set('ghost', 'mk-ghost', '⬡', p.x, p.y, p.front, this.ghostLabel(plan, ctx, sliderT));
  }

  private rebuildGeometry(plan: Plan, ctx: PlanCtx, mapFrameRotating: boolean): void {
    const nodeTimes = plan.nodes.map((n) => n.time);
    const segments: Vec3[][] = [];
    let segment: Vec3[] = [];
    let nodeIdx = 0;
    for (const sample of plan.trajSamples) {
      segment.push(this.toDisplayFrame(sample.r, sample.t, ctx, mapFrameRotating));
