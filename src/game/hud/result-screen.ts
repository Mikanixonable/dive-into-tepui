...
// 結果画面(#hud-result)の OverlayManager 登録先。閉じる手段は再出撃/タイトルへの
// 遷移(いずれもページ離脱かリロードで表現される)のみなので、ESC・外側クリックでは閉じない
// — 登録するのは入力ゲート(タッチパッドの解放・背景入力の遮断)のためだけ。
class ResultScreen implements OverlayHandle {
  ...
  render(hud: Hud, win: boolean, detailHtml: string, title?: string): void {
    ...
    hud.overlayManager.open('result', this, {
...
export function showResultScreen(hud: Hud, sfx: Sfx, win: boolean, detailHtml: string, title?: string): void {
  sfx.setThrust(false);
  sfx.stopBgm();
  resultScreen.render(hud, win, detailHtml, title);
}
