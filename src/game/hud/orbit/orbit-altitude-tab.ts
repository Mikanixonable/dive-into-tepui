...
export class AltitudeTab implements AnalysisTab {
  ...
  public draw(source: AnalysisChartSource, entity: DynamicEntity, reference: OrbitReference): void {
    ...
    if (series === null) {
      this.drawMessage('基準が重力中心ではないため高度を定義できません', source.palette);
      return;
    }
    ...
    this.drawOnAxes(source.palette, points, current ? [{ point: current, style: 'current' }] : []);
  }

  // 点列の代わりに案内文を出す。操作対象が無いときのフォールバック先としてウィンドウが使う。
  public drawMessage(message: string, palette: ThemePalette): void {
    this.drawOnAxes(palette, [], [], message);
  }

  // 横軸は経過時間の固定軸、縦軸は centerM を中心とした高度軸(下端は 0 でクリップ)。
  private drawOnAxes(
    palette: ThemePalette, points: readonly (ChartPoint | null)[], marks: readonly ChartMark[], emptyMessage?: string,
  ): void {
    this.chart.draw({
      points,
