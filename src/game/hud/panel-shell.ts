...
export function wirePanelCollapse(params: PanelCollapseWiring): () => void {
  const { toggleRoot, toggleId, toggleClassName, target, labels, storageId, defaultCollapsed = false } = params;
  const toggle = buildCollapseToggle(toggleRoot, toggleId, toggleClassName, target, labels);
