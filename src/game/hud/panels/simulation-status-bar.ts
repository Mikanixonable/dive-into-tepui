...
import { SIM_EPOCH_SEC, fmtDateTime, fmtElapsedDateTime, fmtTime } from '../utils';
...
export class SimulationStatusBar {
  ...
  sync(game: Game): void {
    this.setText('met', `${fmtDateTime(SIM_EPOCH_SEC + game.simulator.simTime)} / T+ ${fmtElapsedDateTime(game.simulator.simTime)}`);
