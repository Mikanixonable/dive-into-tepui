...
export class PhysicalObjectListOrder {
  ...
  public matches(item: MapPickable): boolean {
    ...
    const section = item.listSection;
    // 人工物は天体と敵を除いた区画のすべて。敵だけは専用の絞り込みを持つ。
    if (this.filter === 'artifact') {
      return section !== null && section !== 'body' && section !== 'enemy' && inFocusedSystem;
    }
    if (this.filter === 'enemy') return section === 'enemy' && inFocusedSystem;
    if (this.filter === 'lagrange') return item instanceof LagrangePointMarker;
  ...
  public refreshInputs(
    ...
    for (const item of items) {
      ...
      if (!changed && (!prev || prev.id !== item.id || prev.name !== item.name
        || prev.section !== item.listSection || prev.parent !== parent || prev.matched !== matched)) changed = true;
      if (prev) {
        prev.id = item.id; prev.name = item.name; prev.section = item.listSection;
        prev.parent = parent; prev.matched = matched;
      } else {
        this.prevInputs.push({ id: item.id, name: item.name, section: item.listSection, parent, matched });
      }
      i++;
    }
  ...
  public rebuildOrder(
    ...
    matched.length = 0;
    for (const item of items) if (item.listSection === section && this.matches(item)) matched.push(item);
    ...
    if (section === 'body' && this.filter === 'satellite') this.appendClusterParents(displayIds, parentOf, itemsById);
  ...
  private compare(a: MapPickable, b: MapPickable): number {
    ...
    if (a instanceof LagrangePointMarker && b instanceof LagrangePointMarker
