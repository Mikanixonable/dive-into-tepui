...
export class HudPanels {
  ...
  private setTarget(t: TargetData | null): void {
    ...
    body.innerHTML = `
      ...
      <div class="row"><span class="k">磁気装甲</span><span class="v">${Math.floor(t.hp)} / ${t.maxHp} <div style="display:inline-block; width:100px; height:8px; border:1px solid #aaa; background:#222; vertical-align:middle; margin-left:4px;"><div style="width:${Math.max(0, Math.min(100, (t.hp / t.maxHp) * 100))}%; height:100%; background:${t.hp <= t.maxHp * 0.3 ? '#ff4a3d' : '#ffc86e'}; transition:width 0.2s;"></div></div></span></div>
