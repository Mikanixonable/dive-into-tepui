...
export interface OrbitReference {
  ...
  readonly attractor: Attractor | null; // hasMass のときだけ非null。mu/radius を要る軌道要素解決に使う
}

// 常に strongestAttractor で基準を選ぶ(切替不可の場面向け)。プロパティウィンドウの
// 「軌道」欄など、常設パネルの基準選択とは独立に軌道要素を出す場所が使う。
export function autoOrbitReference(r: Vec3, attractors: readonly Attractor[]): OrbitReference {
  const center = strongestAttractor(r, attractors);
...
export class OrbitReferenceSelector {
  ...
  resolve(
    r: Vec3, attractors: readonly Attractor[], navTarget: NavTarget, entities: EntityManager,
    ephemeris: Ephemeris, t: number,
  ): OrbitReference {
    if (this.mode === 'earth' || this.mode === 'moon') {
      const found = attractors.find((a) => a.id === this.mode);
      if (found) return { id: found.id, state: found.state, hasMass: true, attractor: found };
    } else if (this.mode === 'target') {
      const resolved = navTarget.resolveState(entities, ephemeris, attractors, t);
      if (resolved) return resolved;
    }
    return autoOrbitReference(r, attractors);
  }
}
