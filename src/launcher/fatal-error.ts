...
import { FONT_FAMILY, FONT_M, FONT_XL, RADIUS_MICRO, RADIUS_CONTROL, Z_FATAL_ERROR } from '../theme';
...
export function showFatalError(title: string, message: string, error: unknown): void {
  ...
  overlay.style.cssText =
    ...
    `color:var(--text);background:var(--bg);font-family:${FONT_FAMILY};font-size:${FONT_XL};text-align:center;line-height:2;z-index:${Z_FATAL_ERROR}`;

  const panel = document.createElement('div');
  panel.style.cssText =
    `max-width:680px;background:var(--surface-opaque);border:0;border-radius:${RADIUS_CONTROL};padding:22px 32px`;
  ...
  detail.style.cssText = `color:var(--text-dim);font-size:${FONT_M};overflow-wrap:anywhere`;
  ...
  reload.style.cssText =
    `margin-top:14px;padding:8px 18px;color:var(--text);background:var(--bg);border:0;` +
