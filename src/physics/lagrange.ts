...
export function secondaryFrameOf(
  ...
  return {
    secondary, secondaryState: secondary.stateAt(pivot), primary, primaryState: primary.stateAt(pivot),
    rotation: motion.orbitFrameRotationAt(t), normal: motion.orbitNormalAt(t),
  };
}

// 副天体を基準にした5点の ECI 位置。
export function lagrangePointsOf(frame: SecondaryFrame): LagrangePoints {
  const { secondary, primary, secondaryState, primaryState } = frame;
  const R = len(sub(secondaryState.r, primaryState.r));
  const mu = secondary.def.mu / (primary.def.mu + secondary.def.mu);
  return lagrangePoints(mu, (x, y) => add(primaryState.r, qRotate(frame.rotation.q, v3(R * x, R * y, 0))));
}
...
export function lagrangeStateOf(point: keyof LagrangePoints, frame: SecondaryFrame): KinematicState {
  ...
  return kinematicState<'eci'>(
    frame.secondaryState.t, r,
    add(frame.primaryState.v, cross(rotation.omega, sub(r, frame.primaryState.r))),
  );
}
