...
// 第三体(潮汐)摂動。力の列挙を一箇所にまとめる唯一の定義箇所。THREE/DOM 非依存の純関数。
...
// 中心重力 + envAccel(環境加速度)+ 推力の RK4 1ステップ。sunPos/moonPos はこのステップぶん
// 呼び出し側が確定させた値(サンプリング方針 — ステップ先頭か中点か等 — は呼び出し側の裁量)。
// bcInv/thrust は種別ごとに異なるため引数で受け取り、モジュール内に既定値を持たない。
