...
// 要素の永年変化)では長半径の変化とケプラー方程式の e 依存ぶんが ν̇/ṙ から抜け落ちる。
...
// 積み上がり、その ulp が以降すべての評価の丸めを支配して、地球軌道上でメートル規模の誤差になる。
...
// 中心天体中心・ECI 軸での状態。軌道要素と平均黄経の変化率だけで決まる(lRate が平均運動そのもの)。
// 速度は動径変化 ṙ·r̂ と回転基準系の角速度による見かけの移動 omega×r の和で、後者が歳差ぶんを担う。
