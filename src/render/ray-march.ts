...
import { dot, exp, float, fract, max, screenCoordinate, vec2, vec3 } from 'three/tsl';
...
// 画素ごとに固定の 0..1 の疑似乱数。rayMarch の jitter へ渡す。Interleaved Gradient Noise —
// 画素座標だけから引くので**時間で動かない**(動くノイズは、静止した絵でちらつきとして目に付く)。
// 近傍の画素へ値を散らす性質があるので、白色雑音より粒が揃って見える。
export function screenJitter(): FloatNode {
  return fract(float(52.9829189).mul(fract(dot(screenCoordinate.xy, vec2(0.06711056, 0.00583715)))));
}
...
// medium は段の中点で評価される。
//
// jitter は段の境目を画素ごとにずらす 0..1 の数(screenJitter)。中点則のまま刻みの位相だけを
