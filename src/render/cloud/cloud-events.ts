// ARCHITECTURE R2 例外: この CPU 参照生成器はラボの全セルと任意時刻からイベント履歴を
// 再構成する。表示導出へ分けると、出生・液水から氷への配分・損失積分を二重実装するか、
// 装置へ内部の供給履歴を公開する必要があるため、評価用の純関数として render/cloud に限定する。
// 物理予報ではなく、質量保存と任意時刻再構成を検査する決定論的 surrogate model。
