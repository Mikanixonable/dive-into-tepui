...
// 重なりで抑える** — 半球に 4 つ、およそ 7100 km おきに並ぶ低気圧が、それぞれ別の閉じた流れを
// 保つ半径に留める。重なると暴風の道が 1 本の谷に溶け、折り目は傾きを失って東西に寝る。**数は
// 帯どうしが届かない疎さに取る** — 帯は低気圧 1 つにつき 1 本で、隣の帯とのあいだには帯の立たない
// 空が要る(`DEVELOP/SPEC/RENDERING.md`「隣り合う低気圧の帯は互いに届かず」)。比は前線の長さ
// — 気団の境目は谷を囲む閉じた流れの縁に沿って折れるので、谷を前線の向きへ伸ばした分だけ弧が
// 伸びる。1 まで戻さないのは、戻すと帯が向きを失って円盤の集まりに見えるため。
const LOW_COUNT = 8;
...
// 使う。**東進の速さは経度で測ると緯度ごとに違う**(1 つの寿命で 35° の低気圧は 57°、60° の
// 低気圧は 93° 進む)ので、位相の違う低気圧は寿命の中で詰まったり離れたりする。歩幅は、その
// 詰まりを通しても隣どうしの間隔が 37°(45°N で 2970 km)を下回らないものを取る。
const LOW_SLOT_STRIDE = 3;
// 生まれる経度が枡の幅のうち揺れてよい割合(枡の中心から ± 半分ずつ)。半球の低気圧は経度を等分した
// 枡に生まれるので、隣どうしは別々の閉じた流れを保つ。揺れは世代ごとに同じ並びになるのを防ぎ、
// 枡の並びが残す最小の間隔を食わない幅に取る(揺れを含めて残るのが上の 37° ≈ 2970 km)。
