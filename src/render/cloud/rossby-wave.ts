...
export class RossbyWave {
  ...
  public streamfunctionAt(direction: Vec3Node): FloatNode {
    return this.envelopeAt(latitudeOf(direction)).mul(sin(this.phaseAt(direction))).mul(STREAMFUNCTION_AMPLITUDE);
  }

  // 流線関数を球面上で微分した風摂動 [m/s]。WeatherModel はこの診断値を局所的な気圧風へ
  // 重ねるが、波のスカラー場と局所低気圧の圧力場は混ぜない。
  public perturbationAt(direction: Vec3Node): Vec3Node {
