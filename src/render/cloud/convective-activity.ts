...
// 気団のノイズの段の表と、その振れ幅。雲塊の配置(800 km)より粗い所から始めて、雲群(48 km)の
// 約 5 倍の 250 km まで届かせる — 活発度がセルごとではなくセルの群れごとに振れるので、セルは数百 km の
// 塊に群れ、塊のあいだは静かな隙間として晴れる(`DEVELOP/SPEC/RENDERING.md`「粒は数百キロの
// 塊に群れ、塊のあいだは晴れる」)。雲群の細部(48〜24 km)には届かせない — セルより細かい所で
// 活発度が振れると、セルが消え残るのではなく 1 つ 1 つが薄まる。振れ幅は、気団だけでは活発度が
