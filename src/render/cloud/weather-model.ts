...
import { abs, clamp, cos, dot, exp, float, max, min, normalize, sin, smoothstep, tanh, uniform, vec2, vec4 } from 'three/tsl';
...
// 上昇流の上限値 [m/s]。急な斜面と深い谷の芯では上昇流が並の何倍にもなり、線形のままだと湿度が
// 0/1 で切れて硬い縁の白い塊になる。
...
// 帯が飽和した領域の上昇流 [m/s]。深い谷の芯と同等の上昇流となるよう、上限値(LIFT_LIMIT)に揃える。
...
// 風が斜面を駆け上がる分の利得。等倍では偏西風や貿易風が山脈へ当たり続けるだけで上昇流が上限値に
// 達し、地形性の縞模様が定常的に固定化される — 慢性的な湿潤・乾燥は平年の雲量が保持する。
...
// 地表付近の沈降乾燥効果は上昇時より弱く設定する — 海洋境界層は沈降下でも層積雲を維持するため、同一利得では
// 亜熱帯高圧帯直下の海洋上が一様に完全快晴化してしまう。
...
// 上限値(LIFT_LIMIT)で飽和した時点で偏差は (1 + 利得) 倍 — 1 で 2 倍になる。
...
// 気圧の写しの 1 texel より粗い。全球正距円筒の 512 texel 高さでも 2/512 ≈ 3.9e-3 rad で、
// この刻みを越えない。
...
// 地理 prior は最終 coverage へ足さず、meanCloudinessを凝結parameterizationへ渡す。dynamic weatherと
// 平均雲量を同じ被覆率へ二重計上しないための境界である。
...
export class WeatherModel {
  ...
  public weatherAt(direction: Vec3Node): WeatherSample {
    ...
    // 日周期は low-frequency の humidity modulation にだけ使う。
    const diurnalPhase = equirectUvFromDirection(direction).x.mul(2 * Math.PI).add(this.dailyPhase);
    const diurnalHumidity = sin(diurnalPhase).mul(0.025);
    ...
    const backgroundComponents = eastNorthComponents(surfaceBackground, east, north);
    const windPerturbation = windComponents.sub(backgroundComponents);
    ...
    const frontProducer = frontForcing(
      temperatureFront, moistureFront, updraft, extratropical, warmth,
    );
    ...
    const dynamicDeviation = deviation;
    const dynamicSurfaceHumidity = float(SURFACE_HUMIDITY_BASE).add(dynamicDeviation);
    const dynamicUpperHumidity = advected.upperHumidity;
