...
// だけの滑らかな塊になり、6 km/texel まで寄れば 2 段とも乗る。上層の濃淡は数千 km の板(4200 km と
// 1600 km の段)がいちばん強く、そこから 640 km・270 km・100 km へ向かって痩せる。薄い雲は大半が
// 下地の透ける靄なので、いちばん細かい段が縁ではなく繊維の濃淡として直に見える。**数百 km 以下の
// 3 段は板の段の半分以下に取る** — 厚く取ると板が数百 km の斑に割れ、筋の向きも斑ごとに揃って空の
// 広い範囲で縞になる。薄く取った分だけ薄い雲はその尺度でなだらかになり、下の厚い雲が割れて見える。
// いちばん細かい段は地表付近の湿度と同じ周波数に留める — 湿度の写しの粗さは 2 つの表のいちばん
// 細かい段が決めるので、上層だけ細かくすると写しの texel 数が増える。
// **どちらの表も先頭に気団・気候の規模(3000〜4000 km)の段を置く** — 晴れと曇りの境がこの規模でも
// 移る。先頭の段の取り分は基準の段(地表付近 800 km、上層 1600 km)より小さく取る — 大きく取ると
// 惑星規模の濃淡が実写の数倍になり、空が数個の巨大な塊に割れる。
...
const UPPER_HUMIDITY_NOISE: readonly NoiseOctave[] = [
  ...
  { frequency: 4, amplitude: 0.7 }, // 1600 km
  { frequency: 10, amplitude: 0.28 }, // 640 km
  { frequency: 24, amplitude: 0.18 }, // 270 km
...
// 帯が飽和した所で地表付近の湿度へ足す底上げ。被覆率の伝達関数の幅(0.22)の 1.4 倍で、稜線(帯の
