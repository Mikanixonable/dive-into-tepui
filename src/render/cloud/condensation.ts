...
const DECK_HUMIDITY_ONSET = 0.40;
const DECK_SUBSIDENCE = 100;
const DECK_LIMIT = 0.34;
...
// weather から凝結する雲のグラフ。被覆率は「湿度(低周波)へ対流(高周波)を足した伝達関数」と
// 「沈降域の層積雲の板」の濃いほうから、雲頂高度は 層状の雲・塔・金床 の最も高いものから出る
// — 覆う広さは湿度が、層の高さは上昇流が、塔は対流の峰が、平らな天蓋は渦の芯が決める。
...
export function condense(weather: WeatherSample): CloudSample {
  ...
  const deck = smoothstep(DECK_HUMIDITY_ONSET, COVERAGE_ONSET, moistened)
    .mul(exp(subsidence.mul(-DECK_SUBSIDENCE)).oneMinus()).mul(DECK_LIMIT);
  return {
