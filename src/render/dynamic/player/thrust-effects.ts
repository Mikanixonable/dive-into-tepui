...
export class ThrustEffects {
  ...
  public sync(
    ...
    this.syncAt(
      position === null ? null : fo.RtoThreeV3(position), thrust, maxAccel, visible,
      cameraQuat, zoomActive, style, displayTime, plumeScale,
    );
  }

  // module asset の semantic thrust anchor をそのまま噴射口として使う。anchor は ship root の
  // world matrix 更新後に渡す。
  public syncFromAnchor(
    ...
    this.syncAt(
      anchor === null ? null : anchor.getWorldPosition(new THREE.Vector3()),
      thrust, maxAccel, visible, cameraQuat, zoomActive, style, displayTime, plumeScale,
    );
  }

  private syncAt(
    position: THREE.Vector3 | null, thrust: Vec3 | null, maxAccel: number,
    visible: boolean, cameraQuat: THREE.Quaternion, zoomActive: boolean,
    style: RenderStyle, displayTime: number, plumeScale: number,
  ): void {
    ...
    if (style === 'schematic') {
      ...
      this.schematicCone.sync(position, backward, ratio, plumeScale);
      return;
    }
    this.schematicCone.hide();
    ...
    this.core.sync(position.clone().addScaledVector(new THREE.Vector3(d.x, d.y, d.z), offsetCore),
      sc * THRUST_PLUME_CORE_SIZE_RATIO, THRUST_PLUME_CORE_BRIGHTNESS * flick, cameraQuat);
    this.outer.sync(position.clone().addScaledVector(new THREE.Vector3(d.x, d.y, d.z), offsetOuter),
