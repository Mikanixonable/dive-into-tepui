...
// CIE 1931 等色関数の多ローブ・ガウス近似(Wyman et al. 2013)。λ は nm。
...
function cieX(lambda: number): number {
  return 1.056 * gauss(lambda, 599.8, 37.9, 31.0)
...
function spectrumToLinearRgb(temperature: number): [number, number, number] {
  ...
  for (let lambda = 360; lambda <= 830; lambda++) {
    ...
    x += radiance * cieX(lambda);
    y += radiance * cieY(lambda);
    z += radiance * cieZ(lambda);
  }
  return [
