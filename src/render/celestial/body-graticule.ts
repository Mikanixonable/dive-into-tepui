...
// 本初子午線)で、半径 1 の球面の GRATICULE_RADIUS_RATIO 倍に置く。天体の位置・スケール・
// 自転姿勢を持つ group の子として置いて使う。
...
// 全天体で共有する経緯線のジオメトリとマテリアル。遅延生成し、以後ずっと保持する。
let sharedGeometry: THREE.BufferGeometry | null = null;
let sharedMaterial: THREE.LineBasicMaterial | null = null;

// 緯度・経度 [deg] から、半径 radiusRatio の球面上の点をモデル座標(経度 0 が +Z)で返す。
...
// GRATICULE_STEP_DEG 刻みの経線と緯線(両極を除く)を1つのバッファへ詰める。
...
// 天体1つぶんの経緯度グリッド線。ジオメトリ・マテリアルは全天体で共有する。
