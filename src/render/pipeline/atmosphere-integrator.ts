...
export class AtmosphereIntegrator {
  ...
  //
  // **ピークは 1 つだけ選び、変化の急峻な箇所を優先する。** 地表(または不透明面)での打ち切りと日没境界は
  // 被積分関数がそこで不連続に遮断されるのに対し、最接近点は滑らかな極大点にとどまる。急峻な側を外すと、
  // その急変区間全体が 1 ステップ内に包含されてバンディング（帯状ノイズ）が発生する。**最接近点しか存在しない視線では等間隔サンプリングを行う**
