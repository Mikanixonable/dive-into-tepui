...
interface BVHLeaf {
  readonly min: Vec3;
  readonly max: Vec3;
  // TriangleBVH.triangles 上の区間 [begin, end)。
...
type BVHNode = BVHLeaf | BVHBranch;

// 葉の三角形が連続区間に並ぶよう組み替えた三角形列と、その上の木。
...
// 軸ごとに並べた三角形の重心。添字は buildBVH が受け取った三角形の並び。
type AxisCentroids = Readonly<Record<'x' | 'y' | 'z', Float64Array>>;

/** 三角形群から BVH を組む。三角形が1枚も無ければ null。 */
export function buildBVH(triangles: readonly Triangle[]): TriangleBVH | null {
  if (triangles.length === 0) return null;
  const order = new Uint32Array(triangles.length);
  const centroids: AxisCentroids = {
    x: new Float64Array(triangles.length),
    y: new Float64Array(triangles.length),
    z: new Float64Array(triangles.length),
  };
  for (let i = 0; i < triangles.length; i++) {
    const { a, b, c } = triangles[i]!;
    order[i] = i;
    centroids.x[i] = (a.x + b.x + c.x) / 3;
    centroids.y[i] = (a.y + b.y + c.y) / 3;
    centroids.z[i] = (a.z + b.z + c.z) / 3;
  }
  const root = buildNode(triangles, order, centroids, 0, order.length);
  return { triangles: Array.from(order, (index) => triangles[index]!), root };
}

/** BVH へレイを飛ばし、maxDist 以内で最も手前の交差を返す。当たらなければ null。 */
export function raycastTriangles(
  origin: Vec3, dir: Vec3, maxDist: number, bvh: TriangleBVH | null,
): RayHit | null {
  return bvh === null ? null : raycastNode(bvh.triangles, origin, dir, maxDist, bvh.root);
}

/** BVH へ球を当て、最も深くめり込んだ接触を返す。触れていなければ null。 */
export function sphereCollideTriangles(
  center: Vec3, radius: number, bvh: TriangleBVH | null,
): SphereHit | null {
  return bvh === null ? null : sphereCollideNode(bvh.triangles, center, radius, bvh.root);
}
...
/** order の区間 [begin, end) を1ノードへ束ね、葉の枚数に収まるまで再帰的に分割する。 */
function buildNode(
  triangles: readonly Triangle[], order: Uint32Array, centroids: AxisCentroids,
  ...
  selectNth(order, begin, end, middle, centroids[axis]);
  return {
    min, max,
    left: buildNode(triangles, order, centroids, begin, middle),
    right: buildNode(triangles, order, centroids, middle, end),
  };
}

/**
 * order[begin, end) を並べ替え、key の昇順で nth 番目に来る要素を order[nth] へ置く。
 * その前後は key の大小で分かれるが、それぞれの中の並びは決まらない。
 */
function selectNth(
  order: Uint32Array, begin: number, end: number, nth: number, key: Float64Array,
): void {
  let low = begin;
  let high = end - 1;
  while (low < high) {
    // 区間の中央の値を軸に前後へ振り分け、nth を含む側だけを残して繰り返す。
    const pivot = key[order[low + ((high - low) >> 1)]!]!;
    let i = low;
    let j = high;
    while (i <= j) {
...
function raycastNode(
  triangles: readonly Triangle[], origin: Vec3, dir: Vec3, maxDist: number, node: BVHNode,
): RayHit | null {
  ...
  if ('begin' in node) {
    ...
    for (let i = node.begin; i < node.end; i++) {
      const hit = rayIntersectTriangle(origin, dir, triangles[i]!);
  ...
  const leftHit = raycastNode(triangles, origin, dir, maxDist, node.left);
  const rightHit = raycastNode(triangles, origin, dir, leftHit?.distance ?? maxDist, node.right);
...
function sphereCollideNode(
  triangles: readonly Triangle[], center: Vec3, radius: number, node: BVHNode,
): SphereHit | null {
  ...
  const leftHit = sphereCollideNode(triangles, center, radius, node.left);
  const rightHit = sphereCollideNode(triangles, center, radius, node.right);
