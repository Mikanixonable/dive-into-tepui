    ...
    if (step % pads.everySteps === 0) {
      ...
      for (const pitch of chord) {
        notes.push({
          ...
          offsetSec: 0,
          durationSec: track.stepDur * pads.lengthRatio,
          level: pads.level,
          wave: pads.wave,
    ...
    if (step % drone.everySteps === 0) {
      for (const voice of drone.voices) {
        notes.push({
          ...
          offsetSec: 0,
          durationSec: track.stepDur * drone.lengthRatio,
          level: voice.level,
          wave: drone.wave,
  ...
  private voiceNotes(voice: PulseVoice, step: number, transpose: number, octave: number): CompositorNote[] {
    ...
    if (harmonic !== null) {
      notes.push({
        ...
        offsetSec,
        durationSec: track.stepDur * harmonic.lengthRatio,
