...
自機右舷(+X)に垂れ下がる予備マガジンの連鎖の挙動を、実装から読み取れる事実として
...
(並進慣性・オイラー力・遠心力・コリオリ力)。`a_thrust` は `Player` から渡される
ワールド系推力加速度を `qRotate(qInvert(att.q), ...)` で機体系へ回したもの。
`α` は `att.w` の前フレーム差分(`prevBodyW`)から推定。
...
6. `resetIfFolded` — 非隣接リンク同士(|i-j| ≥ 2)の距離が `PITCH*0.5` 未満なら
   絡まったと見なし、根本から +X 方向の直線へ全リンクを整列し直す(速度・twist もリセット)。
   判定は O(n²) の総当たり。
...
| `MAG_BELT_PITCH` | `MAG_WIDTH + 0.18` | 距離拘束の目標長 |
...
- `belt-physics.ts:61,103,110,150,187` などに `(newLast)` のような**無意味な括弧**が
  残っている。かつて `clone()` を呼んでいた箇所の名残に見える(基底データは不変なので
  現在の代入で正しい)。
- `ships.ts:48-49` のコメントが「`MAG_BELT_PITCH` は game.ts が使う」と書いているが、
  実際の参照は `player/belt.ts` と `player/belt-physics.ts`。
- `resetIfFolded` の O(n²) は n = 18 なので毎フレーム 136 ペアと軽微。
