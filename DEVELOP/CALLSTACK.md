...
- advanceSimulation(dt)
  ...
  - predictor.update(simTime, player, displayWindowManager.current.duration, mode) // simulator.advance 内の substep cleanup 後に呼ぶ(死んだ個体を予測しない・積分後の実状態と突き合わせる)。horizon = displayWindowManager.current.duration(直前の displayWindowManager.resolve() が確定させた窓を読むだけ)。mode は 'map'(entities.all() 全対象・PREDICT_STEP_BUDGET)/'combat'(自機のみ・PREDICT_COMBAT_STEP_BUDGET)、cameraSystem.overviewMode で選ぶ
    ...
    - advanceBudget(player, ...) // 予算 PREDICT_STEP_BUDGET を操作対象の艦優先で消費。先端が simTime + horizon を覆っていたらループ冒頭で抜ける(重力源を1回も解決せず消費 0)。中心天体を選ぶ窓は最初の1歩だけ futureAttractors.at(tip.t).gravity(先端の時刻 tip.t で他の重力天体の displayState(tip.t, ephemeris) を引き直す — まだその時刻に達していない天体は先端からの二体ケプラー外挿で継ぎ、外挿もできない天体だけその回落ちる)を分類・絞り込みせずそのまま strongestAttractor(tip.r, ..., selfId)(自分自身の id を除いた線形走査)へ渡して解決し、2歩目以降は前の歩の中点で解決した窓(半歩ぶん古い)を持ち越して同じ strongestAttractor へ渡す — 半歩の古さは、そこから決まる dt(RK4 が受ける影響の小さい量)にも外挿の中心天体(元から1歩ぶん古い)にも同じ桁でしか効かない。その1回の結果(rawCenter)から dt(keplerPeriod / PREDICT_STEPS_PER_REV、horizon / PREDICT_MAX_STEPS で下限、horizon そのもので上限)と外挿用の中心天体(解析天体でなければ gravityAttractorsAt から先端時刻で選び直す)を両方求める。積分に渡す重力源はこれとは別に、ステップ中点時刻 tip.t + dt/2 の futureAttractors.at(...).classified を attractorsNearInto(自分自身の id を除く)で絞って決め(stepActual に対する substep と同じ分担)、この窓がそのまま次の歩の中心天体解決へ持ち越される。predictsFuture=false の個体は消費 0 で即 return
...
- environment.update(displayWindow.displayTime, cameraSystem.overviewMode) // 小惑星帯・トロヤ群点群の位置再評価。editor.update より前
- sections.enter(SECTION.plan)
- editor.update(displayWindow) // 計画折れ線の再積分とアプシスアイコン(赤道交点の更新/mapPickables.refresh より前)。内部で先に attractors.resolve(...) を呼び、FutureAttractors の保持を要否判定する — 今フレームの計画終端はこの呼び出し自身がこれから決めるので、revision の量子化は前フレームの終端(lastPlanEnd)を基準にする。provider.revision が前回と同じで起点・終端・基準天体も動いていない区間は再積分せず前回の積分結果を使う
...
- **積分器へ実際に渡す重力源配列は、`Simulator.substep` だけが独自に合流(`gravityBodiesAt` の
  解析天体 + 生存中の重力天体)→ `classifyAttractors` を行い、`Predictor.advanceBudget` のステップ
  中点評価と `PlanArc` の private `integrateTo` はどちらも `game/simulation/future-attractors.ts` の
  `FutureAttractors`(Game が生成し `Predictor`/`PlanEditor` が参照を共有)へ問い合わせて
  `.classified` を受け取る。3経路とも最後は `attractorsNearInto`(問い合わせ位置の27近傍グリッド)
  で絞る。**
  ...
  外挿もできない天体だけ落ちる)。同じ時刻への問い合わせだけを数スロット保持し(ある歩の終了時刻は
  次の歩の開始時刻と一致する)、`resolve` が畳んだ revision が動いたときにだけ捨てる。解析天体の
  窓は1回だけ引いて衝突体と重力源の両方をそこから組むので、同じ時刻の天体位置を二度計算することは
  ない。3経路とも `Ephemeris` の窓を直接書き換えず、常に新しい配列へ展開する。
