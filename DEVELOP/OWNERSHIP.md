...
└── Game
    ...
    │   │   │                              表示抑制(setSuppressed)は Game.sync が EntityTrajectoryLine(予測用インスタンス).supersedesAnalyticEllipse(この艦)から渡す(§付録参照)
...
| 自機 `OrbitLine` の表示抑制(`setSuppressed` の引数) | `EntityTrajectoryLine`(予測用インスタンス)の `supersedesAnalyticEllipse(player, simTime, horizon, overviewMode)` から導く真偽値。マップビューでは予測が表示範囲を覆いきるまで解析楕円を代替表示として残し、戦闘ビューでは予測線が描かれてさえいれば抑制する(視点近傍しか映さないため「覆いきったか」は意味を持たない)。`OrbitLine` 自身はこの理由を持たない | `Game.sync` が毎フレーム渡し直す |
...
| `EntityLineSet.lines`(`Map<GameEntity, TrajectoryLine>`。`EntityTrajectoryLine` が predicted/debug モードそれぞれ1つずつ private に持つ) | エンティティごとに `TrajectoryLine` を1本対応させるプールと、対象集合から外れた分の破棄だけを担う共通機構。線の見た目(色・不透明度・renderOrder)は各所有者がコンストラクタへ渡す `factory` が決めるので、このクラス自身は知らない(debug モードの分は `?debugLines=1` のときだけ実体化) | 対象集合(`sync` の引数)から外れたエンティティぶんを毎フレーム破棄 |
