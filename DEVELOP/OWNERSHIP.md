...
| `GameEntity.trajectoryLine` の点列(`syncTrajectoryLine` が渡す `predictedSamples()`) | 現在状態を先頭に据え、`predictedTrajectory` のうちそれより未来のサンプルだけを続けた配列。呼ぶたび新しい配列を作るので `TrajectoryLine.syncGeometry` の参照同一性による再bake抑制は表示中は常に効かない(先端を現在位置に一致させ続けるための意図的なコスト) | `syncTrajectoryLine` を呼ぶたび(=表示中は毎フレーム) |
