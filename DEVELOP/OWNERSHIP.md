...
- 全ての `GameEntity`(Player・Enemy[]・Bullet[]・DebrisPiece[]・AmmoPickup[]・BeltSection[] — 木の
  ...
  `PredictedArc.requiredEnd = simTime + horizon` / `retainFrom = simTime − ARC_RETAIN_MARGIN` へ書き込む
  可変フィールド、加えて `simulationMaxStep`(既定 `SUBSTEP_MAX_DT`、`Predictor` が
  `simulationMaxStep(simDt, SUBSTEP_MAX_DT, SUBSTEP_MAX_COUNT)` の値を毎回上書き — `Simulator.advance`
  自身のサブステップ幅上限と同じ関数)。`consumable`(`GameEntity` 側の弧は `consumesPrediction`
  の値、計画区間の own な弧は常に `false` — constructor 引数、以後不変)な弧はこの `simulationMaxStep`
  で刻み幅を決め、表示期間(`span = requiredEnd − retainFrom`)由来の項は使わない。
  ...
  **同じ `PredictedArc` が計画区間の積分弧も兼ねる**: `plan/plan-path.ts` の `PlanPath` がノードを持つ
  区間ごとに1本を own し(生成時の `keplerTail`/`consumable` 引数だけが違い、GameEntity 側は
  `keplerTail=true`(外挿の尾を持つ)/`consumable`=自身の `consumesPrediction`、計画区間は両方とも
  `false` — 外挿の尾を持たず、実シミュレーションから読み返されることもない)、伸ばすのは `Predictor` の
  予算パス(`PlanEditor.growableArcs()` 経由)。
...
| 状態 | 正本の所有者 | 備考 |
| --- | --- | --- |
| 自機の位置・速度・エポック (ECI) | `Player.state` | 書き換えるのは RK4 積分(`Simulator.substep` → `stepActual`)・予測列からの追従(`followPredicted` → `actual.follow`、下記)・反動(PlayerFire)・接触(ContactPhysics)。積分と追従は同じ substep でどちらか一方しか走らない(下記の行) |
| エンティティの過去 state 列(`StateQueue`) | `GameEntity.actual`(`DynamicTrajectory` が内部に持つ `StateQueue`。先端(`state`)も同じ列の最新要素) | 記録するのは `DynamicTrajectory.step`/`follow`(どちらも同じ private `advanceTip` を通るので保持方針は共通)。**ある時間帯の物体の状態を決める積分は常にちょうど1本**であることがこの列の正本性の前提 — `Simulator.substep` は `consumesPrediction`(=`mu===0`)な個体ごとに `followPredicted(t, ...)` を先に試し、その個体の `_predictedArc` が時刻 `t` を既に持っていればそれを `actual.follow(...)` で先端にして積分を省き、持っていなければ `stepActual(...)` が `actual.step(...)` を呼んで積分する — 同じ substep でこの2つが両方走ることはない。`mu !== 0` の個体(`Asteroid`)は常に `stepActual` のみ(重力源どうしの相互作用は同じ substep が組む共有窓の対称性に依存するため)。時間窓は `historyDuration` = `max(baseHistoryDuration, requestedHistoryDuration)`(前者は種別固定で既定 0、Ship/Asteroid のみ `SHIP_HISTORY_DURATION`。後者は過去表示の要求で、`Game.advanceSimulation` が毎フレーム `entities.requestHistoryDuration(displayWindow.pastDuration)` として全エンティティへ配る — 履歴を持たない種別は無視し、`HISTORY_DURATION_MAX` で頭打ち)、間引き間隔は `historySampleInterval()`(軌道周期ベース、`step`/`follow` どちらでも同じ)で `cleanup` に渡す |
| 自機の姿勢・角速度 | `Player.att` | 積分は Simulator.stepAttitudes に一元化 |
| 機体座標系トルク | `Player.torque` | 毎フレーム PlayerThrottle の戻り値で上書きされる |
| 推力加速度 | `GameEntity.thrust`(getter/setter、実体は private `_thrust`) | 自機は `PlayerThrottle.updateThrustState` の戻り値で毎フレーム上書き。無推力なら null。setter は非 null を書いた時点で `invalidatePrediction()` を呼ぶ(下記) — `PlanExecutor` が `'powered'` 自動操縦で直接書く場合も同じ経路を通る |
...
| 予測ラウンドロビンのカーソル | `Predictor.cursor` | その回の伸長対象集合(`EntityManager.all()` を `GameEntity.hasFutureReader(canDisplayFuture)` — 重力源・計画衝突体・表示時刻が先へ動けるときのゴースト・予測線を持つもの — で絞った部分集合)のインデックスとして毎フレーム進む。`tracked`/`finished` の2カウンタも同じインスタンスが持つが、`?perf=1` 表示専用の集計値で次フレームの挙動には影響しない |
...
| `GameEntity.predicted` | `game/simulation/predicted-arc.ts` の `PredictedArc`(`_predictedArc`)が持つ `DynamicTrajectory` への転送 getter。`Predictor` が毎フレーム `requiredEnd`/`retainFrom`/`simulationMaxStep` を書いてから予算ぶん `PredictedArc.step()` を呼び、漸進的に伸ばす未来軌道のキャッシュ(`predictsFuture = false` のクラスでは常に null)。伸ばす長さ(horizon)は `DisplayWindowManager.durationSec(referencePeriod)` の毎フレーム値で、`GameEntity`/`Predictor`/`PredictedArc` のいずれにも独立した状態としては残らない | 距離判定による乖離破棄は無い。`consumesPrediction`(=`mu===0`)な個体は `stepActual` が積分した直後(上記の行 — 積分した弧はもう現実を表さない)・`state` の setter(不連続な上書き)・`thrust` の setter(非 null を書いた瞬間)のいずれかで機械的に無効化される。`mu !== 0` な個体はこれらのどれにも当たらないので、`Predictor.update` が伸び切った(`!arc.needsGrowth`)弧に限り `simTime − arc.state0.t > ARC_REANCHOR_INTERVAL` で無効化し、実体との開きを戻す。無効化は破棄のみで即再構築はしない — 次フレーム以降の通常の予算配分で伸び直す |
...
| `DynamicTrajectory.sampleInterval` | 保持サンプル列の最も古い端での間引き間隔(`StateQueue.oldestGap`)。列がどれだけ粗いかという列自身の属性で、`game/trajectory-line.ts` の `TrajectoryLine` が保持列を超えて外挿する尾のサンプル間隔をここから導く(`extrapolationTargetInterval`) | 状態として持たず、読むたび列から導出する |
...
| `PredictedArc.state0`(readonly) / `.requiredEnd` / `.retainFrom` / `.simulationMaxStep` | 弧が答える範囲。`state0` は constructor で決まり以後動かない。`requiredEnd`/`retainFrom`/`simulationMaxStep`(既定 `SUBSTEP_MAX_DT`)は所有者(GameEntity 側は `Predictor`、計画区間側は `PlanPath.update` — ただし `simulationMaxStep` は `Predictor` のみが書き、計画区間の own な弧(常に非 consumable)には書かれない)が毎フレーム書く可変フィールドで、積分先端(`trajectory.state.t`)より手前にも先にもなりうる — 先端が `requiredEnd` を追い越して止まるのが通常の終端 | `state0` は constructor でのみ設定(readonly)。`requiredEnd`/`retainFrom`/`simulationMaxStep` は所有者の毎フレームの書き込み |
