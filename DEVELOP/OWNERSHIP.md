...
- 全ての `GameEntity`(Player・Enemy[]・Bullet[]・DebrisPiece[]・AmmoPickup[]・BeltSection[] — 木の
  ...
  `PredictedArc` 自身の所有。`GameEntity.predictedArc` は `_predictedArc` そのものへの素の getter(生成は
...
| `DynamicTrajectory.extrapolationCenter` | 直近の `step` に渡された、先端位置で最も強く引く解析天体。どれが解析天体かの判定は `step` の呼び出し側(現状 `game/simulation/predicted-arc.ts` の `PredictedArc.step` のみ — GameEntity の予測列・計画区間のどちらの弧でも同じ)が行い、`DynamicTrajectory` 自身は `Ephemeris` を持たないので判定しない。計画区間側の own な弧は `keplerTail=false` で構築されるため、この引数は常に `null` のまま渡る(外挿の尾を持たない)。`TrajectoryLine.syncGeometry` が先端より先を外挿する中心にそのまま使う(列を返す `kepler-extrapolation.ts` の `extrapolatedRelativeStates` へ直接渡す)。1点だけを答える `DynamicTrajectory.extrapolatedAt` は `GameEntity.displayState` の `ephemeris` 引数が経由し、`FutureAttractors.resolveAt` が未来時刻の重力源・衝突体を引くのに使う | `step`(引数省略時は `null` のまま)のたび、`reset` で破棄 |
...
| `FutureAttractors.revisionValue` / `.excluded` | 世代値と、衝突体から除く id 集合。`at(t)` は解決結果を保持せず毎回組む — 解析天体の窓(`Ephemeris.attractorsAt(t)`)を1回だけ引き、衝突体と重力源(`mu !== 0` のもの + 動的重力源)の両方をそこから組むので、同じ時刻の天体位置を二度計算しない。返した配列・Map は呼び出し側(弧の持ち越し窓)がフレームを跨いで保持してよいが、書き換えない契約 | `Game` が構築時に1つだけ作り、`PlanEditor`/`Predictor` が参照を共有する。`PlanEditor.update` の `resolve(除外id集合, plan.revision, lastPlanEnd)` が `futureSourceRevision` を畳み直し、**値が動いたときにだけ** `revisionValue`/`excluded` を差し替える(`Predictor` は `.at(t)` を読むだけで `resolve` は呼ばない) |
