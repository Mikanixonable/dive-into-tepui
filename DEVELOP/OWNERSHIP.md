...
| `PlanArc.periapsides` / `.apoapsides` | 同じ積分ループの中で、`apsisCenter`(呼び出し側が渡す基準天体。末尾区間以外は null で検出自体を省く)に対して `physics/trajectory-features.ts` の `apsisCrossing` をステップ対ごとに掛けた結果、見つかった近地点・遠地点を時刻昇順にすべて保持する配列(それぞれ独立)。折れ線サンプルではなく積分の生ステップ対から求めるので、衝突コースで動径速度が符号反転しない区間は近地点側が空のまま残る。`setRange` が区間の先頭(`state0.t`)より前の要素を落とすので、残る先頭要素が常に「いま答える範囲で最初の極値」になる。読み出し用の `periapsisPoint()`/`apoapsisPoint()` は先頭要素を返し、その時刻が `end` を超えていれば(区間が縮んでその先で見つかったことになれば)null を返す | 積分中に見つかるたび末尾へ追加。`setRange` が呼び出しのたび先頭側を刈る — arc を丸ごと差し替えることだけが全消去の契機 |
