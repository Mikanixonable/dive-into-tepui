...
ヘッドレス Chrome + CDP でステージ1を実機駆動して計測した結果。
ベルトの実装そのものの説明は [BELT.md](BELT.md)。

**§1(BeltPhysics の発散)は修正済み。§2・§3 は未修正。**
...
1. **根治**: `advanceOrientationConstraints` の書き戻し長を実測 `segLen` から
   `MAG_BELT_PITCH` に変更(`segLen` は方向の取り出しにのみ使う)。
   前点が既に補正済みであることによる誤差の掛け算増幅が止まる。
2. **保険**: `isFolded` に非有限判定を追加し、`update` の末尾でも `resetIfFolded` を呼ぶ。
   整列の起点も `beltPos[0]` からアンカーへ変更した(節点が非有限でも必ず有限の配置へ戻す)。

見送った案: 折れ角を機体+X軸からの累積でも制限する / 書き戻し時に速度を減衰させる。
1 だけで発散が止まり、2 が固着を防ぐため、挙動(たわみの自由度)を変える案は入れていない。

### 修正後の検証

同じ操作列(発砲なし、ロール→ピッチ→ヨー→推力→ロール+ピッチ→ヨー+推力):

| | 修正前 | 修正後 |
| --- | --- | --- |
| segLen | 4.18 → 1e38〜1e41 → NaN | **全リンク 4.18 で一定** |
| 表示マガジン | 2本(HUD は +8連) | **8本(HUD と一致)** |

さらに全軸回転+推力+射撃を同時に与える乱暴な操作を 725 フレーム流した結果:
非有限フレーム 0、最大リンク間隔 4.18(一度も超えない)、節点の最大絶対値 76.1 m で有界、
コンソールエラー 0。可視数と所持数の不一致は既知の1フレーム遅れ(消費フレーム)のみ 8 回。

固着からの復帰も確認: 節点3〜17に人為的に NaN を注入したところ **1 フレームで全節点が有限に復帰**した。
