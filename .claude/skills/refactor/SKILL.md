---
name: refactor
description: 作業範囲のコードが DEVELOP/CODING-RULE.md と DEVELOP/ARCHITECTURE.md から逸脱していないかを点検し、是正する手順。責務の移動・分割・統合、モジュールの新設・削除、重複実装の解消を伴う変更を終えた後や、ユーザーが整理を求めたときに通す。判断基準そのものは持たず、すべて CODING-RULE.md と ARCHITECTURE.md を参照する
---

# 規約からの逸脱を点検して直す

**判定基準は `DEVELOP/CODING-RULE.md` と `DEVELOP/ARCHITECTURE.md` が正本。** ここにあるのは
作業手順だけで、「何が違反か」は必ずそちらを読んで決める。この手順書に規則そのものを書き足さない — 規則が変わったなら
そちらを直す。
...
## 3. `CODING-RULE.md` と `ARCHITECTURE.md` の各節を当てる
...
- **`CODING-RULE.md`・`ARCHITECTURE.md` 自体を書き換えるのは、規則そのものが変わったときだけ。**
  現状を正当化するための例外節を足さない。違反を見つけたらコードを直す。
