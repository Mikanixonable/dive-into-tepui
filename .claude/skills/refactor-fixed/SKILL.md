---
name: refactor-fixed
description: このプロジェクトで確定済みの横断的な責務境界(Game にロジックを書かない、update と sync、physics/ の正確さと純度、独自 Vec3 と THREE.Vector3、ctx 引数の禁止、クロージャ注入の禁止、同時に切り替わるフラグの分離、GUI・マーカーの所有者、演出/SFXが読むのは入力元固有の状態でなく共有の物理フィールド、HUD レイアウトの置き場4種とパネル収納)。責務の置き場所を判断するときに必ず読む
---
...
- **実体の死は除去であって、亡骸を配列に残したまま別の真偽値で「無い」と言わない。** 自機も
  ...
  忘れると死んだ相手に対して処理が走る。**亡骸に用があるモジュール(追従カメラなど)が
...
- **`Game` はどのステージが動いているかを知らない。** `instanceof` も `mode === 'creative'` も
  ...
  - `Stage` の生成より前に要る能力 → ステージクラスの static 宣言を `stage-dictionary.ts` の
    `*For(stageId)` 関数で引く(`ephemerisConfig` / `initialPlayerCount` / `showsStatusInOverview`)。
...
`left`/`right`/`bottom` の直書きは④の画面固定バッジ(視点リセット・グローバルステータス・
マップ縮尺・トースト等、置き場を持たない単発の表示)にだけ許す。ある要素をどの種に属させるか
迷ったら、「動かせる/畳める/レイアウトに参加すべきか」で判断する — 参加すべきなら①か②、
一点固定でよいものだけが④。

- **レール幅・シェルフ高のような、画面クラスごとに値が変わる寸法は CSS カスタムプロパティで
...
- **`visualViewport` の購読は `hud/viewport.ts` の1箇所に集約する。** 画面回転・アドレスバー
  ...
  レンダラの解像度合わせ(`render/scene.ts`)は 3D 描画の関心事であってレイアウトではないので、
  この集約の対象外。
