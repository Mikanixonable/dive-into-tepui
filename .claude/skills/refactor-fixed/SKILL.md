...
メモ化などの措置が必要であるか、計算間引きなどの軽量化が必要であるかは、実測を行ってから判断してほしい。特に今回のように実装の構造が大幅に変わるリファクタリングにおいては、過去の構造のメモ化を維持することを絶対視してしまうと自然な実装に至れない。一度メモ化のことは忘れて物理的に正確で、責務分割が自然な実装をしてから、そのうえで明確なパフォーマンス上の問題が生じるか、それに対して1や2を犠牲にしてでも対処すべきか、どのようにパフォーマンスを向上するかを再度検討した方がよい。呼び出し回数は事前に見積れるかもしれないが、究極的には実測に依拠する。

4. 変更コスト
最も優先度が低い。現状のコードがそうなっているからというだけの理由ではその変更を行わない理由にならない。歴史的経緯を残すべきでないという方針にも一致する。ただし、一度の変更ですべてを書き換えるとバグが生じたときの原因の切り分けに苦労するため、今回のような大きな変更は小さな段階にわけ、各段階で問題が生じていないことの確認を取りながら進める。すべてのステップを踏むことで、歴史的経緯なく、当初の目的であった大きな変更が達成されていることを確認する。
