...
- `src/game/{input,camera,hud,audio,entities}.ts` — keyboard/mouse state + edge-trigger queue (left button = camera drag, with a short-drag click queue for map-mode node placement; right button = fire); ship-centered chase camera (up = radial), also driven by arrow keys via `C.CAM_KEY_YAW_RATE`/`CAM_KEY_PITCH_RATE`; DOM-overlay HUD — **light-gray neumorphic chrome with a single orange accent** for panels/buttons/end-screen/help (soft dual-shadow `box-shadow`, no borders), while in-world screen-projected markers keep their pre-existing functional color coding (green prograde, purple nodes, gold target-orbit, magenta planned-orbit, etc.) since a monochrome palette would be unreadable against both black space and bright Earth; WebAudio synth SFX + lookahead-scheduled synth battle BGM loop (no assets; starts on first user gesture, stops on win/lose); entity type defs.
