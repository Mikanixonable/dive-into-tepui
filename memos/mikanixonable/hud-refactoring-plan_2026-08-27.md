...
## 実施結果(2026-08-27 に全手順を実施して確定)

**手順 1〜26 はすべて実施した。** 作業は 16 本の git worktree へファイル単位で重複なく分け、
並列に進めたうえで 1 本へ統合した。33 コミット、104 ファイル変更、新規 29 ファイル。

### 実施の分け方(この計画の手順番号との対応)

計画はグループ A〜G を着手順に並べているが、**グループ単位では並列化できなかった。** 同じファイルが
複数のグループに登場するためで(`resource-transfer-dialog.ts` は A・B・D・E・G の 5 グループ、
`hud-root.ts` は C・D・E・G の 4 グループ)、`CLAUDE.md`「同じファイルを複数のエージェントが触ること
になる — 競合する」に反する。そこで**ファイル単位で重複なく分け、担当ファイルについて A〜G の該当
手順をすべて行う**形へ組み替えた。ファイルをまたぐ手順(手順 4〜10・18・20〜22)は分割せず 1 本で行った。

### 達成目標の充足

| 目標 | 結果 |
| --- | --- |
| 1. typecheck / build | ✅ 通る。**`test:physics` は本計画の着手前から赤**(下記) |
| 2. 重複の解消(grep 期待値) | ✅ `private setText` 0 / `nextSyncAt` 0 / `getElementById`(panels・orbit)0 / `コストなし` 集約先のみ / `Math.acos` 1 / `THEME_PRESETS.find` 0 / 折りたたみ配線 1 箇所 |
| 3. 500 行超・100 行超 | ⚠️ 100 行超の関数は 0 件。**500 行超が 1 件残った**(`orbit-guide-tab.ts` 622 行。下記) |
| 4. アクセス修飾子 | ✅ 0 件 |
| 5. 死んだコードの除去 | ✅ すべて 0 件 |
| 6. `forEach` の解消 | ✅ 0 件。ただし置換先は `for...of` 単独ではなく `Array.from(...)` 経由(下記) |
| 7. import 経路 | ⚠️ 各パネルの `hud-root` 経由 import は解消。**再 export ブロック自体は残した**(下記) |
| 8. コメント | ✅ 英語コメント実質 0 件(残る 6 件は `// "HH:MM:SS"` の書式例と区切りの `//`)。検討メモ参照 0 件。関数コメントの不足は、規約 3.2 が除外する「1 行の getter/setter」と「本文が単一のテンプレートリテラル」を除いて 0 件 |
| 9. 挙動と見た目が変わっていない | ✅ `npm run smoke:browser` が通る(製品ビルドで HUD が起動し、ページ/コンソールの致命エラーなし)。CSS 分割は下記の方法で等価性を確認した |

### 計画時の想定と違った点

**a. `NodeListOf` は型の上でイテラブルではない。**
`tsconfig.json` の `lib` は `["ES2022","DOM","WebWorker"]` で `DOM.Iterable` を含まないため、
`for (const x of nodeList)` は `TS2488` で通らない。手順 21 の「`for...of` へ置き換える」は
`Array.from(...)` を挟む形になる。既存の `context-menu.ts`・`object-picker.ts` も同じ形だった。
規約 1.11 の字面(「イテレーターが存在しない場合のみ素の `for` 文」)からは素の `for` も選べるが、
既存コードの多数派と可読性を採った。

**b. `hud-root.ts` の再 export ブロックは削除できなかった。**
手順 22 の注記どおり、除外対象の `physical-object-list-tree.ts` が
`COLLAPSE_COLLAPSED_GLYPH`/`COLLAPSE_EXPANDED_GLYPH` を `'../hud-root'` から取っている。
`predict-panel.ts`・`property-window.ts`・`view-options-panel.ts` は `'../widgets'` へ移したので、
**除外対象の対応が済み次第、ブロックごと削除できる**状態になっている。

**c. `resource-transfer-dialog.ts` が手順 17 の実施後にいったん 540 行へ増えた。**
区画分割で関数は 100 行未満に収まったが、コメントの補完で行数が増えた。RCS 燃料の補給・移送・
均等化の 4 メソッド(43 行)が **DOM に一切触れず**、外部から受けたタンク群だけを操作していた
(規約 1.4「クラス外から情報をもらいまくっていて、クラス内の情報に全然手を付けていない関数は、
置き場所が適切でない」)ため、`windows/rcs-fuel-transfer.ts` へ純関数として分離し 493 行にした。
**この置き場は暫定** — 中身は HUD ではなくゲームのロジックなので、`game/` 側へ移すのが筋の可能性がある。

**d. `orbit-guide-tab.ts` は 588 行から 622 行へ増えた。**
手順 10 の重複除去(`syncSharedKindFields` の新設)と手順 26 のコメント補完で増加した。
ユーザー判断項目 9 のとおり構造の切り直しには判断が要るため、**分割は行っていない**。
500 行未満にするには約 125 行を出す必要があり、候補は
`buildSharedKindFields`(61 行)+`buildKindRow`(22 行)+`buildCombinedKindRow`(53 行)+
`syncSharedKindFields`(19 行)の計約 155 行を「種類/小題の行を組み立てて共有設定を同期する」
1 モジュールへ括ること。ラベル表と群タブの永続化(約 40 行)を出すだけでは 582 行で届かない。

**e. `top-bar.ts` の二段初期化は残した。**
`sync()` の中で `dataset['speedOptions'] !== 'ready'` を見て `<option>` を初回だけ生成しており、
規約 1.9(`build` を `sync` でやっている)と 1.11(二段初期化)に触れる。`<option>` 生成自体は
`hud-root.ts` へ移せるが、`change` リスナーが `game` を必要とし、`game` は `TopBar` の生成後に
しか手に入らない。解消には `game.ts` 側の配線変更が要り、本計画の範囲を超える。

**f. `test:physics` は本計画の着手前から赤だった。**
`tests/physics/protein-combat-state.test.ts` と `protein-formation.test.ts` が
`ProteinCombatState` の private メンバーと削除済み export を参照して型エラーになる。着手時点の
`d5c4de35` で再現することを確認済みで、原因は先行コミット `674c7aad`(アクセス修飾子の明示)が
メンバーを private 化し `hasAttackSite` を削除した際にテストを追随させなかったこと。
**HUD の変更とは無関係。** `CLAUDE.md`「赤いまま main へ送らない」に該当するため、
**main へ送る前に解消が要る** — メンバーを公開へ戻すのかテストを書き直すのかは判断が要る。

### CSS 分割(手順 19)の等価性の確認方法

分割前の `PANEL_CONTENT_STYLE` と、分割後 14 本を集約役が結合した結果から、テンプレートリテラルの
本文を取り出し、`@media` のネストを条件付きで展開して `(メディア条件, セレクタ, 宣言)` の列へ
正規化して比較した。**362 ルールの多重集合が完全に一致**し、さらに**複数回定義されている
(メディア条件, セレクタ) 4 件について宣言の並びも完全に一致**した。カスケードの後勝ちは変わらない。
...
### 9. `orbit-guide-tab.ts` が 500 行基準を超えたまま残る(実施後の実測: 622 行)

手順 10 の重複除去と手順 26 のコメント補完を経て 588 → **622 行**になった。内部は単一責務
(1 つの軌道ガイドタブ)に閉じており、各 `build*`/`commit*`/`sync*` が UI 区画と 1:1 対応して
`this.current` と専用 `Map` へ強く結合している。全メソッドは 70 行未満に収まっている。

**選択肢**: (a) 達成目標 3 の例外として認める。(b) `buildSharedKindFields`(61 行)+
`buildKindRow`(22 行)+`buildCombinedKindRow`(53 行)+`syncSharedKindFields`(19 行)の
計約 155 行を「種類/小題の行を組み立てて共有設定を同期する」1 モジュールへ括る(→ 約 467 行)。
ラベル表と群タブの永続化(約 40 行)だけを出す案は 582 行で目標に届かない。

(b) は複雑な UI の構造変更なので、目視での挙動確認とセットで行う必要がある。

### 12. `windows/rcs-fuel-transfer.ts` の置き場
...
群ごとの `Map` から取り出す `.get(...)!` が 2 箇所ある。前提は
「`orbit-guide-settings.ts` の `GUIDE_GROUPS` が `GuideGroupId` を網羅する」かつ
「`orbit-guide-kind-ids.ts` が同じ集合の群 ID だけを返す」という**ファイルを跨いだ不変条件**で、
型では検査されていない。解消には両ファイルの変更が要る。

### 14. `top-bar.ts` の二段初期化(実施結果 e)

`sync()` 内で `<option>` を初回だけ生成しており、規約 1.9・1.11 に触れる。解消には
`game.ts` 側の配線変更が要る。
