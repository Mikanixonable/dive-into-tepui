...
作成日 2026-08-19。対象は第10版第3巻の艦体組立UI。**現状の「3D作業台」タブ(ドックビュー内)を廃止し、
Operations ビュー(戦闘ビュー、`ViewId: 'combat'`)上で完結する組立UIへ置き換える。**
状態: 未着手(計画のみ)。
...
**廃止・置き換えの対象**: `hud/base-view.ts` の `3D作業台` タブ全体(`buildWorkbenchTab` とその配下)、
および `docking.ts` 側でそのタブのDOMイベントに応じて動いていたグルーコード
(`pickWorkbenchObject` によるレイキャスト選択、`selectedMounts` を経由した暗黙のマウント選択、
HTML5 ネイティブ drag-and-drop によるインベントリ行 → ステージ矩形のやり取り)。

**維持する対象**: `格納艦艇` / `部品` / `生産` の3タブと、それらが担う一覧・修理・燃料補給・生産判定
(`producibility`)は今回のスコープ外。ドックビュー(`ViewId: 'dock'`)自体、および接岸(`Docking.enterDock`)の
前提条件も変更しない — 組立操作の入口だけが「ドックビューの中の1タブ」から「Operations ビュー上の
オーバーレイ」へ移る。
...
- `hud/base-view.ts`: `buildWorkbenchTab`/`buildWorkbenchSelection`/`buildWorkbenchEditControls`/
  `buildWorkbenchPartRow`/`buildTransferControl`/`showPartProperties`、`WorkbenchSelectionInfo` 型、
  `onWorkbench*` 系コールバック全て、`TAB_ITEMS` からの `'workbench'` エントリ、`DockTab` 型からの
  `'workbench'`。
...
| 大幅変更 | `src/game/hud/base-view.ts`(3D作業台タブの削除) |
| 大幅変更 | `src/game/docking.ts`(組立系メソッドの入れ替え、入口の追加) |
| 変更 | `src/game/vessel/dock-workbench.ts`/`dock-workbench-controller.ts`(ランタイムからの実接続) |
| 変更 | `src/game/map-context-actions.ts`(「組み立てる」項目の追加) |
| 変更 | `src/game/const.ts`(`COLOR_ASSEMBLY_GHOST_*` の追加) |
| 変更 | `src/game/camera/`(組立対象への一時的な注視、詳細は §9) |
| 文書 | `CLAUDE.md`(BaseView/Docking の説明更新。ついでに現存する記述の食い違い — `Docking.produceVessel` という既に存在しないメソッド名の記述、`BaseView` の説明に無い3D作業台タブの記載漏れ — も同じ変更セットで正す) |
...
5. **入口の到達経路**: マップビューのプロパティウィンドウ・Operationsビューの右クリックの両方に
   「組み立てる」項目を出す設計としたが、既存の「基地ビューを開く」項目(格納艦艇/部品/生産タブへの
   入口)とどう並べるか(同じメニュー内の別項目か、基地ビュー側にも「組立モードへ」ボタンを置くか)。

## 10. 完了条件

1. `hud/base-view.ts` に `workbench` という文字列が一切残っていない(全文検索で0件)。
...
5. Undo/Redo が実際に効く。
6. `npm run typecheck` が通る。HUD/DOM/CSSを触るため実装フェーズの着手前に `/ui-design` を通し、
   `DEVELOP/DESIGN-RULES.md` のウィジェット・置き場規約に従っていることを確認する。
7. `CLAUDE.md`/`DEVELOP/CALLSTACK.md`/`DEVELOP/OWNERSHIP.md`/`DEVELOP/SPEC.md` が同じ変更セットで
   更新されている。
8. 廃止対象の実装済み計画書2件(§8「文書整理」)が `old/` へ移されている。
