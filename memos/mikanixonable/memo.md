...
const RING_OPACITY = 0.3;      // annulus 表示時の不透明度
const LINE_OPACITY = 0.5;      // 細線(line)表示時の不透明度
const TORUS_OPACITY = 0.18;    // 厚みを持つ帯(扁平トーラス)の不透明度
土星の環だけはテクスチャ付き(band.texture あり)なので、createTexturedRing 経由でテクスチャのアルファチャンネル自体が濃淡(カッシーニ空隙など)を決めます — こちらは色・不透明度の定数を持ちません。

アーク状の帯(海王星のアダムス環など)の補正 — src/render/ring.ts:11 の ARC_BASE_OPACITY_RATIO = 0.35 が、アーク部分以外の全周ベースをアーク自体より暗くする比率です。

RING_COLOR は全惑星の非テクスチャ環で共有の単色(実際の環の色を再現しているわけではなく、灰青のダスト表現)。天王星・海王星・木星の環はすべてこの1色・1不透明度セットで描かれます。
