...
> **状態 — 後続計画へ置換 / 2026-09-23 再監査:** この計画が参照する `src/game/player/player.ts`、`dynamic-entity/base.ts` などは現行構造に存在せず、入力・表示 phase、entity lifecycle、船体・敵責務、保存境界の多くは後続リファクタリングで再編された。残件一覧も `done/remaining-tasks_2026-09-12.md` へ移っているため、このファイルを現行の実装計画としては使わない。アーキテクチャの現在の規則は `DEVELOP/ARCHITECTURE.md`、新しい課題は現行コードから再調査する。
...
優先順位を再整理した残タスクは[残タスク一覧](../done/remaining-tasks_2026-09-12.md)を参照する。
