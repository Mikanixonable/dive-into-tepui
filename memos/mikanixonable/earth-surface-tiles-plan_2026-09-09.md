...
透視投影では各点の深度、直交投影では投影スケールを使い、次の値を比較する。

```text
errorPx = max(projectedTileWidthPx) / 256
errorPx > 2px かつ z < 7 なら分割候補
errorPx < 1px なら統合候補
```

視錐台または楕円体の地平線の外側に完全にあるノードは除外する。地平線をまたぐノードは残す。
隣接ノードのz差は最大1段に保ち、分割した子がそろわない場合はその領域を親へ戻す。これをCPU側の
frontier（現在表示する葉の集合）としてページ表へ焼く。
...
粗いノードは、そのノードが覆うすべてのz=7セルへ同じ値を書き込む。シェーダはUVからz=7セルを
引き、`B`からタイル内UVを計算し、RとGを同じ層番号で色・法線配列から読み、Aで混ぜる。子が未取得なら
R=Gとして親だけを表示する。ページ表のセル更新は、色・法線・roughnessが同じ層へ公開された後に
まとめて行う。
...
14. **ページ表**: z=7のタイルセルと同じ256×128のRGBA8テクスチャを使い、最近傍サンプリングに固定する。
    R=現在の色・法線配列層、G=親層、B=現在のz、A=親子遷移率とし、255を全球ベースの予約値にする。
    CPU四分木のfrontierが変わったフレームだけ、色と法線が同じGPU層へ公開済みであることを確認して更新する。
15. **タイルファイル名とヘッダー**: 色は`earth/<datasetId>/{z}/{x}/{y}.jpg`、地形は同じキーの
    `{y}.normal-material.bin.gz`とする。地形本文の先頭32bytesはlittle-endian固定ヘッダー
...
17. **マニフェスト**: `earth-surface.json`にはschemaVersion、datasetId、半軸、z/x/y格子、タイル内側と
    ...
    Content-Length・SHA-256を必須項目として置く。クライアントはマニフェストを検証してから要求を発行する。
...
- **GPU**: 固定長128層の `DataArrayTexture` を色RGBA8/sRGBと法線・roughness RGBA16Fで用意する。
  色配列は`SRGBColorSpace`、法線配列は`NoColorSpace`、必須経路のmag/min filterは`LinearFilter`を明示する。
  手順1.5でmipmapを採用できる場合だけmin filterを`LinearMipmapLinearFilter`へ切り替える。ページ表から
  常駐層を引き、取得完了時は確保済みの層だけ更新する。シェーダグラフを作り直さない。
...
出ないことを画像で確認する。mipmapを使う場合は層更新後の生成時間、2texelガター、斜視のちらつきを
基底レベル経路と比較する。失敗した場合は手順2以降へ進めず、座標・法線・ページ表・標本化契約を修正する。
...
| 変更が必要な箇所 | すること |
| --- | --- |
| `tools/earth-surface/bake.py` | 全球の領域処理、余白、親段、極の近傍、測光、ETOPO/GSHHG由来の氷クラス、ERA5由来の12か月気候マップ、マニフェストを完成させる。生成順序と法線の有効近傍規則はデータ契約14～17へ従う |
| `tools/earth-surface/check.py` (新規) | 全領域の存在・入力ハッシュ・版・有限値・正規化・境界・水域・格子・32bytesヘッダー・本文長・ページ表の予約層を検査する |
...
| `src/render/earth-surface.ts` (新規) | `CelestialSurface`の共通契約へ地球の楕円体メッシュ、`earth-surface-tiles`、`EarthSurfaceMaterial`、`EarthSurfaceContext`を束ねる。tiles・material・Contextの所有者とdispose順をここへ固定する |
...
タイルヘッダー・マニフェストの検証、GEBCO由来の気候標高・海陸判定の残存である。
