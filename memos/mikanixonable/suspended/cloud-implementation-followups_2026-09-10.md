...
実施後スナップショット: `15d8ba14`

このメモは、CloudPresentation の疎結合化とは別に進めた雲描画フォローアップの実施記録と、次に扱う課題を保持する。仕様の正本ではなく、課題の優先順位と調査入口を保持するための開発メモである。

## 現行整理（2026-09-12）

実施済みの項目は引き続き有効だが、Blue Noiseの時間安定性、3D気象場、雲影・内部散乱、焼成更新コスト、
render-labの未登録ケースは未完了である。優先度を付け直した一覧は
[残タスク一覧](./remaining-tasks_2026-09-12.md)に集約した。
