...
> この文書は元計画の分類と判断を保持する履歴として `done/` に移動した。

## 統合時の更新

- 元計画の件数は `081743ce8` 時点の値であり、現行の抑制件数とは一致しない。2026-09-20時点の集計は統合計画に記録した。
- `no-non-null-assertion`、`no-explicit-any`、`no-console` を一律にコード違反として修正しない方針を維持する。
- lint設定の再基準化、抑制再集計、実違反の段階修正は統合計画の `L` として引き継いだ。
