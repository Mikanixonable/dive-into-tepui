...
## 4.5 確定した設計判断（2026-08-01）

質問への回答と、実装着手時に確定させた数値。**§5 の質問はこの節で上書きされている。**

- **要件追加 R8**: ラジエーターを展開すると自機の当たり範囲も広くなる。
  被弾判定半径 `PLAYER_RADIUS`(5 m) を、展開度に応じて
  `PLAYER_RADIUS_RADIATOR_FULL`(12 m) まで線形に広げる。
  剛体接触半径 `PLAYER_HULL_RADIUS`(2.6 m) は広げない（排莢直後の薬莢を弾いてしまうため）。
- **Q1 → (d)**: ステータスウィンドウのラジエーター行を右クリックしてメニューを出す。
  戦闘ビューの右クリック（射撃・ターゲットロック）と衝突しない。
- **Q2 → (b) の読み替え**: ガンは `FIRE_INTERVAL = 0.06` s（16.7 発/秒）で、
  1 マガジン 32 発は **1.9 秒**で撃ち尽くす。この発射速度では
  「ラジエーター全開なら撃ち続けられる」は成立しない（必要な放熱面積が
  900 m² 規模になる）。したがって **ラジエーターは連射の可否ではなく
  バースト間の冷却速度を変える**という形に読み替える:
  - 冷態から連射しっぱなしにすると **2 マガジン(64 発 / 約 3.8 秒)** で
    `MAX_HULL_TEMP` に到達して熱死する。1 マガジン撃ち切りは 787 K で安全。
  - 撃ったあとの冷却は、ラジエーター収納で ≈120 s、全開で ≈30 s。**約 4 倍差。**
  - → バースト規律とラジエーター運用が噛み合う。
- **熱応答の時定数の是正**: `HEAT_CAPACITY` を 1.5e6 → **3.4e5 J/K** に下げ、
  `HEAT_ABSORB_AREA` を 4 → **0.9 m²** に下げた。後者は前者と同じ比なので
  **空力加熱の温度上昇率は従来と同一**。ハルの放射冷却だけが 4.4 倍速くなる。
  再突入では空力加熱が MW 級で放射冷却を圧倒するため、再突入の生死判定への
  影響は小さい。この変更なしでは熱が数百秒スケールでしか動かず、
  射撃発熱もラジエーターも体感できない。
- **Q3 → 実面積で 2 枚 198 m²**（1 枚 `RADIATOR_PANEL_AREA = 99 m²` = 4.5 × 11.0 m の両面）。
  ハルの `RAD_AREA = 70 m²` と合わせて全開時 268 m² ≈ 3.8 倍。
  ISS のラジエーター(3.1 × 13.6 m)と同等の寸法で、モデルの見た目と一致する。
- **Q4**: 被弾 1 発で健全度 `RADIATOR_HIT_INTEGRITY_LOSS = 0.2` 減（5 発で全損）。
  **戦闘中に回復しない**（装甲と違い自然回復させない — 損傷が蓄積して撤退圧力になる）。
  **展開度 `RADIATOR_HITTABLE_DEPLOY = 0.15` 未満なら被弾対象にならない**
  → 「戦闘中は畳んで熱を溜め、間合いを取ったら開いて冷やす」駆け引きが成立する。
  R8 の当たり判定拡大もこれと同じ方向に効く。
- **Q5 → (b)**: `COLLISION_DAMAGE_MIN_SPEED = 50` m/s で無傷、
  `COLLISION_DAMAGE_FULL_SPEED = 500` m/s で装甲を最大値ぶん失う線形。
- **Q6**: デバッグステージはリリース版にも含める（タイトル隅の小さなボタン）。
  **`UnlockManager` のクリア記録には含めない**（他ステージの解放条件を汚さないため）。
- **Q7**: 「エンジン出力」は現状の `throttleIdx`（並進出力の弱/中/強）のこと。
  新しい消耗リソースは導入しない。なお実装確認の結果、**この行は既に実装済み**
  だったため、フェーズ 2 で追加したのは「温度」行のみ。

---

## 5. 確認したい質問（実装前に決めたいもの — §4.5 で回答済み）
