...
### R7. 敵の「集団」がマーカーの色で決まっている
- 症状: 同時発砲数の上限を数える母集団が「自分と同じ `accent`(画面マーカー色)の敵」。ステージ1/2 は全敵が別色=全員別集団なので上限が効かず、ステージ00 は乱数で HSL を揺らした色バケツが集団の単位になる。
- 場所: `src/game/dynamic/dynamic-entity/enemy.ts:113,374-375,382-389`、`src/game/stages/stage-utils/wave-attack.ts:243-263`
- 疑う理由: 同じ個体が `waveId` と `formationId` という本物の所属識別子を既に持っている。色の決め方を変えると難易度が変わる。
- 仕様: COMBAT.md「同じ集団に属する敵のうち、同時に発砲できる機数には上限があり」— 集団の定義は書かれていない
- 減るもの: 表示色と戦闘規則の結合が切れ、ステージ1/2 で上限が意図どおり効く。/ 確度: 高 / 確認: 自分で確認(`e.accent === this.accent` を現地確認)
...
### R9. 衝突マーカーのキー表が3本固定で、4区間目以降の衝突点が黙って出ない
- 症状: ノードを3つ以上置いた計画では、4番目以降の区間で見つかった地表到達点に ✕ が付かない。
- 場所: `src/game/plan/plan-display.ts:66,275-277`(`key === undefined` なら `return []`)、`src/game/plan/plan-editor.ts:36`(`MAX_PLAN_NODE_MARKERS = 12`)
- 疑う理由: 区間数は最大13になりうるのに、キーが `['planImpact0','planImpact1','planImpact2']` の固定3件。
- 仕様: MARKERS.md「衝突点 ✕」。PLAN.md は区間数や衝突点の本数制限を定めていない
- 減るもの: 固定文字列表と取りこぼし分岐が消え、衝突点が区間数に追随する。/ 確度: 高 / 確認: 自分で確認

### R10. 基地の画面外方位マーカーが `visible: false` 固定で、毎フレーム伏せて宣言されている
- 症状: 基地は画面外に出ても方位矢印が出ない。それでも字形・色・優先度の宣言は毎フレーム作られる。
- 場所: `src/game/dynamic/dynamic-entity/base.ts:214-217`(他は `enemy.ts:228`・`pickup.ts:116` が `true`、`player.ts:541` が距離条件)
- 疑う理由: `visible` に渡る値が常に `false` の一方向分岐。
- 仕様: MARKERS.md「戦闘ビューでは、画面外に出た対象を画面端の円周上の方位矢印 ↑ に置き換えて方位を示す」(対象に基地を含む)
- 減るもの: 基地専用の死んだ設定が消え、画面外の扱いが4種別で同じ規則になる。/ 確度: 高 / 確認: 自分で確認
...
### R13. パネル見出しをクリックして畳むと保存されない
- 症状: ▾ ボタンで畳めば記憶されるが、見出し文字をクリックして畳むと保存されず、ビュー切替や再読込で開き直る。
- 場所: `src/game/hud/panel-shell.ts:77,124`、`src/hud/widgets/collapse-toggle.ts:50`
- 疑う理由: 畳み状態の正本が DOM の `collapsed` クラスで、保存が「▾ の click でクラスを読み直す」片方の経路にしか配線されていない。
- 仕様: UI-DESIGN.md §5「隠れている間に折りたたみを変更しても、再表示時にその状態のまま出てくる」/ §7-1 は見た目から開閉を判定することを禁止
- 減るもの: 操作経路が1本になり、クラス読み出し3箇所が消える。/ 確度: 高 / 確認: 報告のみ

### R14. creative の手動スポーン連番がランを跨いで 1 に戻り、陣形 id が衝突する
- 症状: スナップショットから再開した直後に陣形を出すと、復元済みの `FORMATION-1` と同じ `formationId`・同じ個体 id が並ぶ。
- 場所: `src/game/creative/manual-spawn.ts:31-32,46,75`(同じファイル群の `object-placement.ts:83-87` は復元 id を予約している)
- 疑う理由: 採番の予約がここだけ抜けている。衝突すると右クリック・ターゲット・マーカーが別個体を指す。
- 仕様: COMBAT.md「同じ陣形に属する個体と各個体の役割は、セーブして再開した後も維持される」
- 減るもの: 陣形の取り違えと id 重複が消える(分岐は増えない)。/ 確度: 高 / 確認: 報告のみ / 計画: K2(creative の連番)、手順 6-5

### R15. ランの畳み口が BGM を宣言せず、捨てたランの曲が次のランへ続く
- 症状: `endRun()` は `bgm.syncRun(false)` を呼ばない。`switchSlot`(再開可能なステージがある場合)とスナップショット読込では、捨てたランの BGM がローディング中も鳴り続け、`syncRun(true)` が no-op なのでそのまま次のランへ連続する。
- 場所: `src/launcher/launcher.ts:134-139`(endRun)、`:122`(タイトル)、`:163`、`:170`、`:258`、`src/audio/bgm/bgm.ts:91-95`
- 疑う理由: `syncRun(false)` が「タイトルを出す」「結果を出す」「失敗した」の3箇所に分散し、本来の単一の畳み口だけが抜けている。**ユーザーの例1 の残り。** 段 1-6 の宣言化で大半は解消済み。
- 仕様: AUDIO.md「BGM が鳴るのはランの中(開始から勝敗が確定するまで)だけで、タイトル画面と結果画面では鳴らない」
- 減るもの: `endRun` 1箇所へ寄せれば呼び出しが4→2になる。/ 確度: 高 / 確認: 自分で確認
...
### R19. 自動保存の60秒タイマーがランを跨いで持ち越される
- 症状: 新しいランの最初の自動スナップショットが、前のランの経過しだいで開始直後〜60秒後のどこで撮られるか決まらない。
- 場所: `src/main.ts:245-248`、`src/launcher/save/autosave.ts:8,13-16`
- 疑う理由: `AutoSave` は `main()` で1個だけ作られ `lastCaptureReal` を構築時刻で初期化したまま、`startRun`/`endRun` のどこからもリセットされない。ランの計時がホストの寿命に載っている。
- 仕様: SAVE.md「実時間で60秒ごとに…自動的に記録する」— ランの境界での扱いは記述なし
- 減るもの: 「ラン開始から60秒ごと」という1本の規則になり、開始直後の無意味なスナップショットが消える。/ 確度: 中 / 確認: 報告のみ
...
### R21. 敵が自機の既定パーツ一式を継いでいて、宣言 HP が嘘になっている
- 症状: `ENEMY_MAX_HP = 6` と書いてあるのに実 maxHp が 11 になり、装甲の 20% 軽減が敵にも乗り、放熱板・太陽電池・RCS タンクというダミーのパーツに被弾が無作為に分配される。撃破所要弾数が 3〜14 発に散らばる。
- 場所: `src/game/dynamic/dynamic-entity/ship.ts:99-141,263-273`、`src/game/dynamic/dynamic-entity/enemy.ts:49,156`
- 疑う理由: `ProteinEnemy` は `initDefaultParts()` を空で上書きしていて(`protein-enemy.ts:139`)、パーツ式が敵に要らないことは既に自覚されている。`MetalEnemy` だけが継承のまま残っている。
- 仕様: COMBAT.md「弾が命中した場合のダメージは、撃った武装自体が持つダメージ量がそのまま適用される」(軽減は FLIGHT.md の自機側だけ)
- 減るもの: 敵から4分岐(パーツ再構築・装甲軽減・無作為分配・vital 即死)が消え、撃破所要弾数が宣言値どおりになる。/ 確度: 高 / 確認: `initDefaultParts` の空上書きと `ENEMY_MAX_HP` は自分で確認、maxHp=11 の算術は報告のみ

### R22. 太陽グレアの散布界補正が、仕様では未確定なのに実装され、地球専用の影モデルを持っている
- 症状: 自機・敵の射撃で散布角が照準と太陽の角度により 0.5〜2.0 倍される。影判定が「ECI 原点中心・`R_EARTH_EQ` の円柱」なので、他天体の影に入っていてもグレアが掛かる。
- 場所: `src/game/player/fire-control.ts:66-76,314`、`src/game/dynamic/dynamic-entity/enemy.ts:95-105,413`
- 疑う理由: `src/physics/shadow.ts` に本影・半影・金環を1式で出す厳密な日照率が既にあり、放熱板と発電はそれを使っている。同じ問いに戦闘だけが別の粗い近似を持つ。
- 仕様: COMBAT.md:177「未確定の案」に「太陽側を取ると相手の散布界がばらける」とあるのみ(=決定事項ではない)。確定仕様としての記述なし
- 減るもの: 関数2つと `R_EARTH_EQ` 依存が消え、命中率が視線と太陽の関係で暗黙に変わる要素が無くなる。残すなら `shadow.ts` の `sunlit` を引いて一般化。/ 確度: 高 / 確認: 仕様の位置は自分で確認、実装は報告のみ
...
### R25. 遠方消滅した補給の再投入が、通常投入の 1/20 の距離に湧く
- 症状: `respawnOnDespawn`(ステージ00・creative)では 50 km 先で消えた補給の数だけ自機から 12.5〜50 m に湧く。通常の自動投入は 312.5〜625 m。
- 場所: `src/game/stages/stage-utils/logistics.ts:21-22,124-132,227-230,248-251`
- 疑う理由: 距離定数が `STAGE00_` を名乗りながら creative とデバッグからも使われている。再投入という規則そのものが仕様に無い。
- 仕様: GAME.md §8「マガジン残数が減ると、自機の軌道上・進行方向 0.3〜0.6 km 前方に補給が自動投入される」— 再投入・近距離投入の記述なし
- 減るもの: `respawnOnDespawn` 引数・再投入ループ2つ・距離定数2つ。補給の湧く位置が1通りになる。/ 確度: 中 / 確認: 報告のみ
...
| R22 | 太陽側を取ると散布界がばらける | COMBAT.md「未確定の案」 | **仕様が未確定なのに実装が先行している。** 確定させるか実装を消すか |
