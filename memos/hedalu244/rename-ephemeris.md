...
`ephemeris` という語が層ごとに違うものを指していて、**同じ値が境界をまたぐたびに別名になる。**
`HelioEphemeris` は保持側で `precise`、渡す側で `pack` と呼ばれ、`.epk` の器も、それを読む口も、
どちらも `pack` と呼ばれている。加えて**位置・速度を答える口のほとんどが無標の `stateAt` /
`stateOf` で、原点(ECI か恒星中心か主天体相対か)が名前に出ない。**
...
構造の側の問題は `refactor_orbit_ephemeris.md`。**この計画は名前だけを直す。**

---

# 第1部 現況

以下はすべて **`f816c78c`** 時点のコードから読み取ったもの。**維持しない** — 手順を実施したら、
この部は役目を終える。

## 1.1 天体1体について、何が何を答えているか

天体1体の運動は `CelestialMotion`(`src/physics/celestial-motion.ts`)が答えるが、**恒星中心
まで**。ECI 化は `CelestialBodyWindows`(`src/physics/celestial-body-windows.ts`)だけが行う。
**供給源が2系統あるのは並進の位置・速度だけ。**

| 問い | 供給源 | 実体 |
| --- | --- | --- |
| 並進の位置・速度(表引き) | `.epk` の Chebyshev 位置係数 | `HelioEphemeris` |
| 並進の位置・速度(閉じた式) | ケプラー要素 + 摂動項 | `KeplerOrbit` / `SatelliteOrbit` |
...
**`CelestialBodyWindows.bodyAtIndex`(`celestial-body-windows.ts:102`)の1箇所だけ。** 引き算
そのものは `toEci`(`kinematic-state.ts`)が持ち、窓は「自分と原点天体を**同じ供給源で**引く」
判断だけをする。原点天体の恒星中心状態は `originAt(t)` が時刻ごとに1回へ畳む。

**軸の付け替え(ICRF → ゲーム ECI 軸)は別のファイル**(`icrfToGameEci`、
`absolute-ephemeris.ts:27`)で、`HelioEphemeris.stateOf` が重心差を取った直後に通す。
つまり「ECI **軸**にする」と「ECI **原点**にする」は別のファイルで起きていて、**`Eci` を名前に
持つのは軸のほう**(と `toEci`)。原点を移す側のファイル名・メソッド名には `eci` が無い。

加速度は経路が分かれない(`sub(motion.helioAccelAt(t), origin.accel)`)ので、**パックが引ける
期間でも `CelestialBody.accel` は解析値。**

## 1.3 位置・速度を答える口と、その原点

**原点は `KinematicState<F>` の `FrameTag` が型で持つ**(`kinematic-state.ts`)ので、取り違えは
型検査に止まる。**残っているのは名前が原点を言わないという読みやすさの問題だけ。**

| 口 | 原点 | 型に出る | 名前に出る |
| --- | --- | --- | --- |
| `CelestialBodyWindows.bodyAt` / `stateAt` / 配列3口 | ECI | `'eci'` | ✗ |
| `CelestialSystem.stateAt` / `celestialBodyAt` / 配列3口 | ECI | `'eci'` | ✗ |
| `CelestialEntity.stateAt` / `stateOf` / `bodyAt` | ECI | `'eci'` | ✗ |
| `FrameAnchorSource.stateOf`(実体 `FrameAnchors`) | ECI | `'eci'` | ✗ |
| `entityStateAt(entity, t, centerMotion)` | ECI | `'eci'` | ✗ |
| `ReferenceFrames.transformAt` | ECI | `'eci'` | ✗ |
| `lagrangeStateOf(point, frame)` | ECI | `'eci'` | ✗ |
| `CelestialMotion.helioStateAt` / `packedHelioStateAt` | 恒星 | `'helio'` | ✓ |
| `PlanetSystem.helioStateAt` | 恒星 | `'helio'` | ✓ |
| `HelioEphemeris.stateOf(id, simTime)` | 恒星 | `'helio'` | ✗(型名にはある) |
| `SatelliteMotion.relStateAt` | 惑星 | `'primaryRel'` | △(`rel` が何相対か出ない) |
| `keplerOrbitState(orbit, t)` / `stateOnOrbitAt(el, t)` | 主天体 / `el.center` | `'primaryRel'` | ✗ |
| `AbsoluteEphemeris.barycentricStateOf(id, jdTdb)` | 太陽系重心 | `BarycentricState` | ✓ |
| `ChebyshevEphemeris.stateOf` / `stateAtSeconds` | 太陽系重心・ICRF 軸 | `'barycentric'` | ✗ |
| `FrameKinematicState`(`frame.ts`) | 任意の座標系 | 別の branded type | ✓ |

**無標の `stateAt` / `stateOf` が、いちばん強い契約(ECI、`stateAt` はさらに pivot からの外挿)を
持っている。**
...
| 主天体相対 | 共通原点しか持たない | `packedPrimaryRelStateAt` が2回引いて差を取る(`celestial-motion.ts:321`) |
| 11 個に無い天体 | 収録外 | `SatelliteMotion.packedStateAt` が「収録済みの惑星 + 解析の惑星相対」で補う(`:444`) |

同期回転衛星(カッシーニ)の自転位相は `keplerOrbit` から出るため、**パックが引ける期間でも自転は
解析軌道を経由する。** `EphemerisProfile.orientationModelId` は自転モデルを名乗る宣言だが、`src/`
`tests/` `tools/` のどこからも読まれない(src 3 行) — **パックが自転を答えないという事実を、
この宣言が名前だけで否定している。**
...
| `CelestialMotion.precise`(src 20 行・tests 12 行)/ solar-system 9ファイルの引数 `pack`(117 箇所) | どちらも同じ1個の `HelioEphemeris` を指す | 4 |
...
`PlanetSystem.helioStateAt` は対を持たない(重心の軌道は常に解析)ので、**規範6 の有標化の対象外。**
...
| `windows.stateAt(id, pivot, t)` / `helioStateAt(t)` / `relStateAt(t)` | 原点が ECI / 恒星 / 惑星の3種。さらに `stateAt` **だけが外挿(近似)** で他は厳密 | **無標の `stateAt` が、いちばん強い契約を持っている。** 取り違えは `FrameTag` が止めるので、残るのは読みやすさ | 1 |
| `windows.stateAt(id, pivot, t)` の3引数 / 他の `*StateAt(t)` の1引数 | 第2引数が「厳密に引く時刻」、第3が「答える時刻」 | 同じ語族なのに引数の意味が違う | 1 |
...
| ファイル `planet-orbit.ts` | `PlanetOrbit` 型はもう無い。中身は単位変換 `planetOrbit()` と `planetAngles()` だけ | ファイル名が型を名乗っているが、その型は無い | 4 |
...
| `stateOf` | ①`HelioEphemeris.stateOf`(恒星中心・simTime)②`ChebyshevEphemeris.stateOf`(ICRF 重心・ET 秒)③`FrameAnchorSource.stateOf`(ECI・null 可)。**3つとも `(string, number)` で、戻り値のタグだけが違う** — 取り違えは型で止まるが、読んで区別がつかない | 3・5(③は触らない) |
...
| `analytic` | ①解析暦(`OriginState.analytic`)②閉じた式で書ける形(`GuideShape.kind === 'analytic'`) | **触らない**(1.6) |
| `orbit` | ①要素の束(`KeplerOrbit` / `SatelliteOrbit`)②状態から求めた瞬間の要素(`OrbitalElements`)③CR3BP の焼き込み点列(`orbit-catalog`)④ECI 曲線(`orbit-guide`)⑤描画物(`OrbitLine`)⑥**軌道ですらない直線**(`RelativeOrbitLine`) | 8 |
...
| `epoch` の3時刻軸(`epochJdTdb` / `epochOffsetSec` / `OrbitEpoch.t`) | 時刻軸の語彙は `physics/time/` の `JulianDate<S>` ごと考える話。`refactor_orbit_ephemeris.md` 3節の未決事項と地続きなので別計画 |
| `GuideShape.kind === 'analytic'` | `orbit-guide` の内部語彙。解析経路の `analytic` と衝突するが、両者が同じファイルに現れることはない |
...
## 2.1 `ephemeris` と `orbit` を分ける軸

**「表引きか閉じた式か」は軸ではない。** 両方向に反例がある — `AbsoluteEphemeris` は
`tests/physics/absolute-ephemeris.test.ts:10-21` で閉じた式として実装されており、逆に
`orbit-catalog.ts` の CR3BP 族は焼き込んだ点列である。表現の選択は族の境界ではない。

**「絶対時刻を引数に取るか」も軸ではない。** `OrbitalElements` は `epoch: OrbitEpoch | null` を
持ち、`stateOnOrbitAt(el, t)` が絶対時刻から状態を答える。`KeplerOrbit` も
`keplerOrbitAtEpoch` が simTime の原点を畳み込むので、`keplerOrbitState(orbit, t)` の t は
simTime そのもの。**いまや orbit も絶対時刻から状態を答える。**

**軸1 — 誰に対する位置か。** `orbit` は主天体相対で、主天体が変われば別の軌道になる。`ephemeris` は
系で共通の1原点(恒星 = 系の階層の根)に対する位置で、天体ごとに原点は変わらない。
`OrbitingMotion.packedPrimaryRelStateAt`(`celestial-motion.ts:321`)が主天体相対を得るために
**ephemeris を2回引いて差を取っている**のは、ephemeris が主天体相対を直接は答えられないため。
`PlanetSystem` の導入で「orbit は主天体相対」は構造としても裏が取れている — 系の重心の軌道と、
そこから引いた惑星本体・衛星が、木構造で一方向に並ぶ。

**軸2 — 天体を id として知っているか。** `ephemeris` は **天体 id で引く表**で
...
ので、`CelestialBodyWindows` が両方を持って切り替えられる。`orbitFrameRotationAt` /
...
5. **位置・速度を答える口は、原点を名前に出す。** `eci*` / `helio*` / `primaryRel*` /
   ...
   原点そのものは `KinematicState<FrameTag>` が型で守っているので、これは**安全のためではなく
   読みやすさのための規則。** 既存の無標 `stateAt` / `bodyAt`(`CelestialBodyWindows` →
   `CelestialSystem` → `CelestialEntity` の3層)は呼び出し箇所が広いので**改名せず、CODING-RULE
   と各クラスの冒頭コメントで無標が ECI を指すことを明記する**(この語だけを規約上の例外として
   固定する)。
6. **供給源が分岐する対は、両方を有標にする。** 片方だけ無標にしない —
   `analytic<原点>StateAt` / `ephemeris<原点>StateAt` の対で揃える。分岐しないもの
   (`PlanetSystem.helioStateAt` など)は対象外。
...
| solar-system 9ファイルの `pack` | `ephemeris` | 恒星中心・simTime の表 |
| `stage.ts` の `pack` | `barycentricEphemeris` | 上とは型が違う(重心・JD_TDB)ので語も分ける |
...
| ファイル `planet-orbit.ts` | `planet-orbit-elements.ts`(または `kepler-orbit.ts` へ吸収) | 型を名乗るファイル名から、中身(単位変換と平均角)へ |
| `OrbitLine` | `EllipseLine` | 閉じた楕円しか描けない |
| `RelativeOrbitLine` | `TargetRelativeLine` | 軌道ではなく2点を結ぶ直線 |

**据置** — `CelestialBodyWindows` / `CelestialSystem` / `CelestialEntity` の `stateAt` / `bodyAt`
(規範5 の例外)、`HelioEphemeris`(原点が名前に出ている)、`toEci` / `icrfToGameEci`
(原点/軸の変換そのもの)、`orbitFrameRotationAt` / `orbitNormalAt`(答えているのは orbit の量)、
`OrbitPickKind` の `'orbit-body'` / `'orbit-ship'` / `'orbit-guide'`(線の種類ではなく「何の軌道か」の
分類)、ワイヤ形式のキー `series` / `EPHEMERIS_PACK_VERSION` / `packId` / `packFormatVersion`(凍結)。
...
**代案D — `stateAt` も有標にする。** 規範5 の例外を作らずに済み、「無標 = ECI」という暗黙を
消せる。**欠点**: `stateAt` は windows / system / entity の3層に同名で並び、`src/` の広範囲から
呼ばれる。**改名の総量がこの計画の残り全部を超えるので却下**し、規約上の例外として明記する側を
採る。原点は `FrameTag` が守っているので、放置しても取り違えは起きない。

**代案E — `helioStateAt` を無標のまま残す**(`packedHelioStateAt` だけを `ephemerisHelioStateAt` へ)。
差分は小さいが、恒星中心の対だけが非対称に残る。**規範6 と衝突するので却下。**
...
語順として不自然で、`HelioEphemeris`(src 30 + tests 6)を含む既存の読みやすさを落とす。
...
2. `grep -rnE "\bpack\b" src/game/celestial/solar-system/` が 0 件(`.epk` の器を指す用例はこの層に
   無い)。`src/game/save/` の `packId` / `packFormatVersion` は据置。
...
5. `grep -rn "orientationModelId" src/ tests/ tools/` が 0 件(`tests/dist/` を除く)。
...
9. `node tools/ephemeris/cli.mjs verify src/assets/ephemeris/modern-2026-10y.epk` が
   ...
   を報告する(**`f816c78c` で実測済みの現状値**)。
10. 既存セーブが復元できる — `EPHEMERIS_PACK_VERSION` と `.epk` のワイヤ形式が変わっていない。
11. `tests/physics/celestial-eci-baseline.test.ts` の焼き込んだ期待値が1つも変わらない
    (`git diff` に数値の行が出ない)。
...
| 同(新設または「`celestialBody`」節へ追記)「原点を名前に出す」 | 規範5・6 を書く。**無標の `stateAt` / `bodyAt` が ECI を指すことを、唯一の例外として明記する**(原点そのものは `FrameTag` が守っていることも1文で) |
...
| `src/physics/ephemeris-pack/evaluator.ts:226-227` | 公開フィールド `manifest` / `pack` を private へ落とす(`.manifest` の他の当たりは `DecodedEphemerisPack.manifest` と `validateManifest` のローカル) |
...
`grep -rn "orientationModelId\|velocityOf" src/ tests/ tools/` が 0 件(`tests/dist/` を除く)。
...
| ファイル | 何をするか |
| --- | --- |
| `src/physics/ephemeris-pack/evaluator.ts:279-291` | `stateOf` を削除し `stateAtSeconds` へ一本化する。`positionOf` はテスト専用(`chebyshev-ephemeris.test.ts`)なので落とし、テスト側を `stateAtSeconds(...).r` へ書き換える |
| `tests/physics/chebyshev-ephemeris.test.ts` | `eph.stateOf` → `eph.stateAtSeconds`、`eph.positionOf` → `stateAtSeconds(...).r` |
...
| `src/physics/ephemeris-pack/format.ts` の冒頭コメント | ワイヤのキーが `series`、コード上の語が `segment` であることを1文で明記する |
...
| ファイル | 何をするか |
| --- | --- |
| `src/physics/celestial-motion.ts` の `precise` 全 20 行 | フィールド・引数 `precise` → `ephemeris` |
| 同 `:159` | `packedHelioStateAt` → `ephemerisHelioStateAt`(呼び出しは `celestial-body-windows.ts:111,128`) |
| 同 `:162,166,444,445` | `packedStateAt` → `ephemerisStateAt`(`SatelliteMotion` の override も) |
| 同 `:223,237,321` | `packedPrimaryRelStateAt` → `ephemerisPrimaryRelStateAt`。`orbitFrameRotationAt` / `orbitNormalAt` のローカル `packed` も揃える。**メソッド名は据置** |
| 同 `helioStateAt`(抽象 `:119` + `Star`/`Planet`/`Satellite` の実装3箇所 + 呼び出し `:359,426` + `celestial-body-windows.ts:115,129`) | `analyticHelioStateAt` へ。`PlanetSystem.helioStateAt`(`planet-system.ts:22`)は**据置**(対を持たない) |
| 同 冒頭コメント | 「暦パック」を供給源の意で使っている箇所は「天体暦」へ |
| `src/physics/celestial-body-windows.ts` の冒頭コメントと `OriginState` | `ephemeris` / `analytic` の対がメソッド名と揃うことを確かめる(フィールド名は既にこの対) |
| `src/physics/planet-orbit.ts` | ファイル名を中身へ合わせる(`PlanetOrbit` 型はもう無い)。行き先は `planet-orbit-elements.ts` か `kepler-orbit.ts` への吸収 — **どちらにするかは着手時に決める** |
...
| `tests/physics/celestial-motion.test.ts`(`precise` 12 行), `tests/physics/test-helpers.ts` | 引数名・メソッド名の追従 |
...
`grep -rnE "\bpack\b" src/game/celestial/solar-system/` が 0 件。達成目標11。
...
| `src/physics/ephemeris-catalog.ts` | `loadAbsoluteEphemeris` → `loadBarycentricEphemeris` |
| `src/game/celestial/solar-system/solar-system.ts:4,53` | 型・引数 `absoluteSource` → `barycentricSource` |
| `src/physics/{celestial-motion,planet-system,celestial-body-windows}.ts`, `src/game/stages/stage.ts`, solar-system 9ファイル | import の追従 |
| `src/physics/ephemeris-pack/format.ts` | コメント "absolute-ephemeris" の追従 |
| `tests/physics/{absolute-ephemeris,packed-absolute-ephemeris,celestial-motion,celestial-eci-baseline}.test.ts`, `tests/physics/test-helpers.ts` | import とテストファイル名の追従。**期待値の数値は1つも触らない** |

**達成条件と検証.** `npm run typecheck` / `npm run test`(全層)が通る。
`grep -rn "Absolute" src/physics/` が 0 件。達成目標11。
...
| ファイル | 何をするか |
| --- | --- |
| `src/physics/ephemeris-pack/format.ts:85-90,356,399` | `CanonicalEvaluatorEphemerisPack` を解消する。`ChebyshevManifest` の `coordinateFrame` / `timeScale` を必須にできるなら型ごと落とし、できなければ `toEvaluatorEphemerisPack` の戻り値型を `ChebyshevPack` のままにする |
...
| `src/physics/ephemeris-pack/types.ts` | `ChebyshevEphemerisPack` → `ChebyshevPack`(src 9 + tests 3) |
...
**アセットのディレクトリ `src/assets/ephemeris/` は据置**(`webpack.config.js` の asset ルールで
...
| 13 ファイル(`celestial-entity` / `geostationary-overlay` / `celestial-system` / `orbit-guide-lines` / `dynamic-entity` / `game` / `entity-line-manager` / `trajectory-line` / `map-context-actions` / `orbit-pickables` / `creative-stage` ほか) | import と識別子の追従(`OrbitLine` 51 行、`RelativeOrbitLine` 4 行) |
| `src/game/lines/trajectory-line.ts`, `src/game/lines/relative-orbit-line.ts` の冒頭コメント | 「OrbitLine(解析的な楕円)の兄弟」の追従 |

**`OrbitPickKind` の `'orbit-body'` / `'orbit-ship'` / `'orbit-guide'` は据置** — これは線の種類では
なく「何の軌道か」の分類なので、`orbit` の新しい定義に合っている。
**`tools/perf-probe.mjs` の `setOrbitLineFor` も据置** — HUD の行ラベルを操作する関数で、この型とは
無関係。
...
改名の総量から出す(いずれも `tests/dist/` を除いた実ファイルでの grep 行数、`f816c78c` 時点)。
...
| 4 | 約 170 | solar-system の `pack` 117 + `precise` src 20 / tests 12 + `packed*` 10 + `helioStateAt` の有標化 10 + `stage.ts` 2 + `planet-orbit.ts` の移動 |
| 5 | 約 60 | `AbsoluteEphemeris` src 18 + tests 12(内 `Packed…` src 7・`load…` src 6)+ ファイル名と import の追従 約 25 |
...
| `helioStateAt` の一括置換が `packedHelioStateAt` / `computeHelioStateAt` / `PlanetSystem.helioStateAt` まで巻き込む | 前2つは別の名前へ行くべきで、3つ目は据置 | 手順4。大文字 H を含む変種を先に処理し、`planet-system.ts` は除外すること |
| `OrbitLine` は `RelativeOrbitLine` の部分文字列 | 素朴な置換が `RelativeOrbitLine` を `RelativeEllipseLine` にしてしまう | 手順8。長い方から先に置換すること |
| solar-system 9ファイルの `pack` は `planetSystem(...)` / `new SatelliteMotion(...)` の**位置引数**として渡っている | 引数名の変更は無害だが、順序を触ると隣接引数と入れ替わっても型が通りうる | 手順4。引数の順序は触らないこと |
| コメント中の「暦パック」「精密暦」「解析暦」は識別子と語彙がズレたまま | 改名後もコメントだけ旧語彙で残ると、次に読む人が対応を取れない | 手順4・5・6。触ったファイルのコメントは同じ commit で揃える |
