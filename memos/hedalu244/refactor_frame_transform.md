...
どちらも機械的で、型検査が取りこぼしを拾う。**争点E の素の `Vec3`(位置を原点なしで受け渡して
いる箇所)はこれとは別に残る** — そちらのほうが実害は大きい。
...
| 案 | 得る | 失う・要確認 |
| --- | --- | --- |
| **1. 別モジュール2つを直列** | 上表の非対称(平行移動 / 剛体運動、供給源を揃える / 揃えない、時刻を持つ / 持たない)がそのまま型に出る。**混ぜてはいけない変換が型に現れない**ので、危険な組み合わせを書きようがない | モジュールが1つ増える |
| **2. 同一クラスの2インスタンスを直列** | 型と語彙が1つ。`FrameTransform` は既にあり、`frameOfCelestialBody`(`celestial-body.ts:145`)が「回転なしで天体を中心に置く」用途で使われている前例もある | stage 1 のインスタンスは **`q` = 恒等・`ω` = 0 という使わない欄**を常に持つ。恒等回転の `qRotate` が「サブステップごと × 天体数」の経路に入る(**未測定**。無駄であることは確か)。`FrameKinematicState` が時刻を持たないので、stage 1 の出力(ECI 値)がエポックを落とす |
| **3. 万能変換モジュール** | branded の利点が最大限に活き、モジュールが増えない | **経由地を間違えると桁落ちする** — 例: LEO の機体(地心 7e6 m)を ECI ↔ 月相対で往復させるのに重心を経由すると、1.5e11 m の加減算を挟んで有効桁を4〜5桁落とす。さらに**供給源の制約は「損をする」ではなく「禁止」**(解析の原点 × パックの天体は誤り)なので、許される対と許されない対を万能モジュールが持つことになる |

**1 を推す。** 2つの段は上表のとおり3点で非対称で、案2はその非対称を「使わない欄」と
「落ちるエポック」として引き受けることになる。案3は、**この設計がいま2つのハブ
(星系原点と ECI)で意図的に分けているものを1つに畳む**話で、桁落ちと禁止対の判定を
モジュールの内側へ持ち込む。

**判断を覆す材料:** 案2の恒等回転が実測で無視できる範囲で、かつ `FrameKinematicState` に
エポックを持たせる整理(あるいは stage 1 だけ別の出口を使う)が素直に書けるなら、
型が1つで済む案2のほうが読みやすい。

#### C-2. 後段への渡し方(C-1 を決めた後にも残る)

どの案でも **stage 2(`ReferenceFrames`)は ECI 値を入力に取る。** しかも
