...
### 手順 1. SPEC に雲の継ぎ目と細かさの連続を書く

**目的**: 仕様が先行する。極・日付変更線の継ぎ目が立たないことと、細かさが連続に変わることを、
見え方の要件として置く。`/modify-feature` から入る。

| ファイル | 何をするか |
| --- | --- |
| `DEVELOP/SPEC/RENDERING.md`(「雲の描画」節、「どの距離からでも破綻しない」の項) | 「SPEC に足す文面」のとおりに直し、1 項を足す |

**達成条件と検証**: `npm run typecheck`(SPEC だけだが常に走らせる)。
`git grep -n "経度 180° の線に沿った継ぎ目" -- DEVELOP/SPEC` が 1 件。
