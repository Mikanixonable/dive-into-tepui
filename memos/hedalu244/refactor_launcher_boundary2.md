...
**実施済み。** `720ff639` → `7c70522c` の 16 コミットで全手順を通した。
...
> この誤報を招いた側も直した(`e46a7e0a`)— `.claude/hooks/check-boundaries.mjs` は
> 生成・後始末・直列化を常に許可し、配線で占められたモジュールの表は書き換え前提の一覧に、
> 指摘は「違反」でなく「疑い」になった。`advanceSimulation` `handlePointerInput`
> `proteinMotionFrameSample` の3件は誤報ではないので出続ける。

**`/refactor` で見つけて直さなかったもの。** `game.ts` が 646 行(着手前 560 行)で 1.2 の
500 行基準を超える。1.2 の診断に従うと「同じ関心の実装が単に多い」— サブシステムの生成・配線と
毎フレームの呼び出し順で、分ける線が無い。`launcher/stage-select.ts` の 554 行も同様
(大半が CSS のテンプレート)で、どちらもこの整理の前から在る。
