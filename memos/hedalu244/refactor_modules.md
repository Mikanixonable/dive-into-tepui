...
「残す」で決着。** 残りは 5 / 6 / 9 / 10 / 14 / 21 / 22 / 23。
...
7. **論点23**(回転追従の散らばり)— 表示名の食い違いが利用者から見えるので、まず語彙を
   `SPEC/` で決める。実装の集約はその後。
8. **論点14** — 方針をユーザーへ問うてから。`SPEC/` の「未確定の案」に、エンティティ種別が
   今後増えるかの記述があるかを先に見る。
9. **論点21** — `/modify-feature` 案件。仕様は既に決まっているので、決めるのは警告文言だけ。
