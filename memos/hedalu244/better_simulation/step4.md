...
万有引力シミュレーター」は Step3(`step3.md`)でほぼ形になったが、現状調査(`survey.md`)と
ユーザーのフィードバックで、**モデルの表現力が固定値で狭められており、重力源の判定基準が二重で、
予測経路が物理的に不正確で、性能のためだけの絞り込みが重力経路に残っている**ことが分かった。
本書はそれらすべてと、Step3 が持ち越した性能面の判断・文書の同期を決着させる。
**本書の完了をもって万有引力シミュレーターが完成していることが目標となる。**
着手前に、必ず `goal.md` を参照し、目的を理解しながら行うこと。

**フェーズは作業範囲と変更リスクの小さい順に並べてある。** Phase 1 → 9 の順で進めること。
...
4. **`relevantAttractors` と `GRAVITY_NEGLIGIBLE_ACCEL` がコードから消えている。** 3つの重力積分
   経路(実積分・予測・軌道計画)は、`Ephemeris` の天体一覧(と小惑星の合流結果)をそのまま
   `stepDynamics` へ渡す。`grep -rn "relevantAttractors\|GRAVITY_NEGLIGIBLE_ACCEL" src tests` が0件(Phase 4)。
...
7. **27近傍の空間ハッシュ(`physics/spatial-grid.ts`)が存在し、万有引力の重力源列挙と衝突判定の
   ...
   実装前後の実測値が本書に記録されている。`step3.md` から「実測していないため要否が
   判断できていない」という記述が消えている(Phase 7)。
8. **`memos/mikanixonable/SOLAR_SYSTEM_PLAN2_2026-08-09.md` の前提が、Step3・Step4 の実装結果と
   一致している。** ユーザーの許可が得られなかった場合は、本書に「ユーザー未許可のため保留」と
...
#### 万有引力側 — 動的重力天体だけをグリッドに載せる

**分類はしきい値ではなく、配列の出自そのもので決める:**

- **解析天体(`Ephemeris` 側)は常に含める。** レジストリ天体は27体で固定であり、しかも一体ずつが
  重い(太陽・木星は星系全域に効く)。数を絞る理由も、絞ってよい根拠も無い。
- **動的重力天体(`entities.attractors()`)だけをグリッドに載せる。** ここだけが数千体まで
  増えうる側であり、しかも一体ずつは軽い。

これで **`sqrt(mu / しきい値)` のような分類用の定数を一切持たずに済む**(Step3 の旧設計は
`GRAVITY_NEGLIGIBLE_ACCEL` から分類を導いていたが、その定数は Phase 4 で消える)。

セルサイズ = 近傍とみなす半径 R は `game/const.ts` の定数1つ
(`DYNAMIC_GRAVITY_CELL_SIZE`)。**これは「R より遠い動的重力天体の引力は足さない」という近似で
あり、モジュール先頭コメントに限界として明記する。** Phase 4 が距離による絞り込みを退けたことと
矛盾しない — あちらは各エンティティが全天体の寄与を評価する O(N·M) の前段で、**加算のオーダーを
下げないまま実装だけを長大化させていた**。こちらは O(N·M) を O(M·k)(k = 近傍セル内の天体数)へ
落とす。オーダーを下げる対価があるかどうかが両者の分かれ目であり、§0 の優先順位を破っていない。

**グリッドは substep ごとに、合流済みの重力源配列から1回だけ組む。** 全エンティティが同じグリッドを
読むことが、重力天体どうしの相互作用が処理順に依存しないことを引き続き担保する(§2-4)。
27近傍は対称なので(A が B の27近傍にあるなら B も A の27近傍にある)、**セルサイズを両者で
同じにしている限り作用・反作用が食い違うことはない。**
...
### 2-8. `SOLAR_SYSTEM_PLAN2_2026-08-09.md` の前提が Step3・Step4 で変わる(Phase 8)

同文書の EP0(重力窓の位置依存化)は Step3 が肩代わり済みだが、実装の中身が同文書の設計
(`influenceRadius` という静的フィールド)とは異なる形(`relevantAttractors` — 実際の寄与
そのものによる判定)になり、**さらに本書 Phase 4 でその絞り込み自体を削除する**(§2-4)。
加えて **Phase 5 で `gravitySource` が消え、重力窓と全天体窓が1つになる**。
C-2(`AttractorId` は閉じた union)・
...
| `src/game/const.ts` | `GRAVITY_NEGLIGIBLE_ACCEL` を削除 |
...
| `src/game/const.ts`(Phase 7 ぶん) | `DYNAMIC_GRAVITY_CELL_SIZE` を追加。`MAX_ASTEROIDS`/`MAX_DEBRIS` を高負荷ステージの配置数まで引き上げる(§4 Phase 7) |
| `CLAUDE.md` / `DEVELOP/CALLSTACK.md` / `DEVELOP/OWNERSHIP.md` | 絞り込み・重力窓・予測の凍結に関する記述を実装結果へ同期 |
| `memos/hedalu244/better_simulation/step3.md` | 絞り込みを Step3 の到達点として挙げている記述を削除し、「空間インデックスは未実測・要否未判断」という記述を実装済みの事実へ書き換える |
| `memos/hedalu244/better_simulation/step4.md` | 実測値と高負荷の再現手順の記録、PLAN2 への提案の可否(許可されなければ保留)の記録 |
| `memos/mikanixonable/SOLAR_SYSTEM_PLAN2_2026-08-09.md` | **ユーザーの許可を得てから**、EP0/C-2/E-9/E-13 の前提を Step3・Step4 の実装結果に合わせて書き直す(§2-8、§4 Phase 8) |
...
- `game/simulation/attractors.ts`(Phase 5 で改名済み)で、**動的重力天体だけ**をセルサイズ
  `DYNAMIC_GRAVITY_CELL_SIZE`(`game/const.ts` に追加)のグリッドに載せ、解析天体は常に含める形に
  する。グリッドは substep ごとに1回だけ組み、全エンティティで使い回す(§2-7)。
  **「R より遠い動的重力天体は足さない」という近似の限界を、この関数のコメントに明記する。**
...
- **`step3.md`** — 絞り込みを Step3 の到達点として挙げている記述を削除し、「空間インデックスの
  要否が未判断」という記述を、Phase 7 で実装済みという事実へ書き換える。
- **`survey.md`** — §7 の残タスク表のうち、Phase 1〜7 で片付いた行を削除する。
- **本書(`step4.md`)** — §8 に実測値、Phase 8 の可否を記録する。
...
12. **重力側と衝突側でグリッドを共用しないこと。** セルサイズの意味が違う(片方は無視距離、
    ...
    そのインスタンスではない。
13. **空間ハッシュを、Phase 4・5 で削除した絞り込みの復活にしないこと。** グリッドが下げるのは
    オーダーであり、寄与の大きさによる採否判定を戻すのではない(§2-4・§2-7)。
14. **`SOLAR_SYSTEM_PLAN2_2026-08-09.md` をユーザーの許可なく書き換えないこと。** §2-8・Phase 8。
15. **`hitAttractor`/`hitCelestialBody` と、接触の判定式・撃力の解決に手を出さないこと。**
...
- **Step2 の残タスク**(分点歳差、月理論数値表の未検証項目)。`step2.md` に残したままにする。
