...
**限界温度は平衡ではなく遷移で較正する。** 再突入は速すぎて放射平衡に達しない — 降下する
物体が実際に到達する最高温度は、その高度の平衡温度よりはるかに低い。遠地点 413 km・近地点
30 km の楕円を降ろした実測(温度は各高度を最初に下回った時点):
...
**艦は 60 km 付近で頭打ちになり、そこから下は冷える** — 抗力で減速すると加熱が v³ で落ちる
ため。 = 1300 K に届かないので、LEO からの降下で艦を失わせるのは動圧
(35 kPa)であって温度ではない。これは変更前と同じ挙動で、 も明記している。

この表から較正した値と、その結果(×1 / 最高ワープ・低 fps):

| 種別 | 限界温度 | 失われ方 |
|---|---|---|
| 敵機 | 500 K | 温度、高度 78.5 km / 77.0 km |
| 破片・薬莢・弾薬 | 933 K(アルミ合金の融点) | 温度、高度 61.2 km / 51.1 km |
| 自機 | 1300 K | 温度では失われない(動圧か地表到達) |
