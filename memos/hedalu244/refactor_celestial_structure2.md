...
行番号付きの参照はすべて `9a7c45d4` 時点のコードから引いた着手点。食い違ったらコードを信じる。
...
- **星系が「定数表」で表されている。** `SOLAR_SYSTEM`(`physics/solar-system.ts:505-2023`)は
  ...
  解釈後の木)にある。
- **静的事実の正本が2箇所。** `game/celestial/celestial-appearance.ts:37,48,54,63,74,84,103` が
...
- **見た目の事実が5つの表に分散。** `CELESTIAL_APPEARANCES`(`celestial-appearance.ts:97-206`、
  名前・View)、`BODY_CLASSES`(`body-class.ts:13-111`)、`CELESTIAL_ALBEDO`
  (`render/celestial-albedo.ts:34-117`)、`CELESTIAL_TEXTURES`(`render/celestial-textures.ts:39-67`)、
  `ATMOSPHERE_OPTICS`(`render/atmosphere.ts:23-44`)。すべて id をキーにした表。
...
```
Launcher.startRun
├─ celestialSystem = await stageClass.createCelestialSystem(phaseOffsets, earthSpinPhase0, onProgress, startSimTime)
│     └─ 既定の Stage は太陽系パック solarSystem('earth', phaseOffsets, earthSpinPhase0, absoluteSource, …) を呼ぶ
...
太陽系パック(構築コード。系ごとに physics と game の2ファイルを参照で対にする)
   physics/solar-system/earth-system.ts   earthSystem(sun, phases, epoch, pack, origin) → { earth: PlanetMotion, moon: SatelliteMotion }
   game/celestial/solar-system/earth-system.ts   earthSystemEntities(m, earthSpinPhase0) → { earth: new Earth(m.earth, …), moon: new SphereEntity(m.moon, '月', …) }
   physics/solar-system/solar-system.ts   solarSystemMotions(originId, phases, epoch, pack, …) → { sun, earthSystem, marsSystem, …, all }(all は宣言順)
   game/celestial/solar-system/solar-system.ts   solarSystem(originId, phases, earthSpinPhase0, pack, …) → CelestialSystem
```
...
export type PhaseOffsets = Partial<Record<string, number>>;   // save/save-data.ts:336 と同じ形
...
7. **tests がコンパイルする game 側のファイル(`tsconfig.test.json` の include: `plan.ts` /
   `focus-target.ts` / `point-field.ts` / `placement-validation.ts` …)は、`CelestialSystem` ではなく
   物理の部品だけを受け取る。** `CelestialEntity` を要するテスト(`tests/game/body-visibility.test.ts` の
   `MapVisibilityPolicy` 部分・`tests/game/focus-target.test.ts`)は、物理部品で書けない部分を削る。
...
9. **`FutureCelestialBodyProvider`(`game/simulation/arc-bodies.ts:29-33`)の interface は残し、
   `FutureCelestialBodies` クラスだけを消す。** interface は `tests/game/predicted-arc.test.ts:25`
   が「地球1体だけ」のスタブを差す継ぎ目で、`CelestialSystem` が構造的に満たす。
10. **小惑星帯・トロヤ群の点群(`point-field.ts:66,138,215` が木星の軌道要素を直接読む)は太陽系
    パック固有のものとして残す。** 恒星が無い星系では組まない(`environment-scene.ts:191`)。
...
- 次が `src/` `tests/` から 0 件: `CELESTIAL_APPEARANCES`、`fallbackCelestialAppearance`、`BODY_CLASSES`、
  ...
  `celestial-appearance.ts`、`body-class.ts`、`future-celestial-bodies.ts`、`environment-scene.ts`、
...
| 6 | 1(motion のクラス) | 5(`ephemeris.ts`)、4(`fallbackCelestialAppearance` が `def.kind` を読むなら `motion.kind` へ) | 4 と並行できる。33 ファイルの import 変更を含む |
...
- **7b** 消費者を群ごとに移す(表の 9 群)。群ごとに typecheck が通るので、群ごとにコミットできる。
...
| `grep -rln "\bCelestialBodyId\b\|\bOrbitingId\b\|\bFrameAnchorId\b" src tests tools` の全件(33 + 12 + 6 ファイル、233 箇所) | `string` に置換し、import から外す。`Partial<Record<CelestialBodyId, number>>` は `Partial<Record<string, number>>` |

#### 達成条件と検証

- `npm run typecheck`、`npm run test`(`frame` は physics、`frame-anchors` / `focus-target` は game)。
...
| `CelestialMotion.at`(ECI 化と暦パック分岐。origin が自分自身なら厳密に 0) | `:363-377`、`:653-659` |
| `orientationAt` / `spinRotationAt` / `degree2At` / `atmosphereAt` | `:558-624` |
| `OrbitingMotion.orbitFrameRotationAt` / `orbitNormalAt`(暦パック分岐込み) | `:387-421` |
| `OrbitingMotion.lagrangeAt` / `lagrangeStateAt` | `:426-448` |
| `OrbitingMotion.hasUsableCollinearPoints` / `hasStableTriangularPoints` | `:451-478` |
...
(`:364-373, 389-403, 411-418`)は motion の中に入る。**パックは有効期間内(`precise.isValidAt(t)`)だけ
使い、期間外は解析へ落ちる** — この判定を motion へ移すときに落とさない。
...
| `game/simulation/arc-bodies.ts:114`(予測弧) | 弧の各歩の中点 | pivot と同じ | なし(歩ごとに厳密評価) |
| `game/nav-target.ts:173`、`game-entity.ts:533`、表示系の `positionOf(id, displayTime)` | t そのもの | — | なし(nav-target は精密暦の un-bake と揃えるため意図的に厳密) |
...
| `physics/celestial-motion.ts`(新規) | 上の階層。`ephemeris.ts:44-67, 229-478, 556-624, 653-659` の実装を移す |
...
- `npm run typecheck`。
- `npm run test` — `tests/physics/` の `ephemeris` / `celestial-body` / `body-orientation` /
  `equatorial-satellites` / `laplace-satellites` / `irregular-satellites` / `small-bodies` / `halo` /
  `frame` / `absolute-ephemeris` / `window-agreement` と `tests/game/plan.test` が数値を固定しているので、
  これが通ることが「挙動不変」の判定。
...
| ファイル | 何をするか |
| --- | --- |
| `game/simulation/arc-bodies.ts:29-33,91` | `candidates()` を `readonly defs: readonly Pick<CelestialBodyDef, 'id' \| 'mu' \| 'radius'>[]` に。`FutureBodyCandidate` の名前を消す |
| `physics/ephemeris.ts` | `readonly defs: readonly CelestialBodyDef[]`(宣言順)を公開 |
| `game/simulation/future-celestial-bodies.ts` | 削除 |
| `game/game.ts:21,106,150,201,222` | `futureCelestialBodies` を消し、`Predictor` / `PlanEditor` に `this.ephemeris` を渡す |
| `game/simulation/predictor.ts:20,42`、`game/plan/plan-editor.ts:32,102` | 型を `Ephemeris` に(plan-editor は `:100` の `ephemeris` を使う) |
| `tests/game/predicted-arc.test.ts:25-36`、`tests/physics/window-agreement.test.ts:18,141` | スタブの `candidates` → `defs`、`new FutureCelestialBodies(EPHEMERIS)` → `EPHEMERIS` |

#### 達成条件と検証

- `npm run typecheck`、`npm run test`(`predicted-arc` / `plan` は game 層、`window-agreement` は physics 層)。
...
| ファイル | 何をするか |
| --- | --- |
| `game/celestial/environment-scene.ts` → `celestial-system.ts` | `git mv` + クラス名。`private readonly ephemeris`(`:149`)を `readonly` に |
| `game/game.ts:35,66-67,95-96,145,176-184,311,343,551` | `_ephemeris` を消す。コンストラクタ内の `this.ephemeris`(`:150-171`)は引数を直接使う(構築順は変えない)。`get ephemeris()` は `this.celestialSystem.ephemeris` |
| `game/pickable/orbit-pickables.ts`(`EnvironmentScene` の import と `:29`)、`game/save/snapshot-service.ts:93`、`render/stars.ts:44`、`celestial-view.ts:27` | 型名・プロパティ名・コメント |
...
この手順の見た目の表は今の `celestial-appearance.ts` のまま id をキーにした `Record<SolarSystemId, …>` で、
**手順8 で系ごとの構築コード(名前付きフィールドの写像)へ置き換わる経過の形。**

```ts
// game/celestial/celestial-appearance.ts(手順8 で消える)
...
export function fallbackCelestialAppearance(motion: CelestialMotion): CelestialEntity;  // 未登録 id。恒星なら Sun、それ以外は単色の SphereEntity
```

`celestialBodyName(id)`(`hud/frame/frame-labels.ts:9`)と `bodyClassOf(registry, id)`
(`body-class.ts:114`)は、この手順では表を読む形に書き換えて残す(呼び出し元 44 箇所は手順7 で
まとめて直す)。
...
| `game/celestial/celestial-view.ts:26-34` | `sunIrradianceAt` は `star: CelestialMotion \| null` を `CelestialSystem` から受ける |
| `game/celestial/celestial-appearance.ts` | `:36-95` の各 entry を `motion` 受けに。`:97-206` に `bodyClass` を足す(`body-class.ts:13-111` の値を移す)。`:208-221` の fallback を `motion` 受けに |
| `game/celestial/body-class.ts` | `BODY_CLASSES` を消す。`bodyClassOf` は表を読む(手順7 で消える) |
| `game/celestial/celestial-system.ts:122,180-186,210-213,239-241` | `bodies: readonly CelestialEntity[]`。生成を `CELESTIAL_APPEARANCES[id]?.create(motion) ?? fallbackCelestialAppearance(motion)` に(`motion` は `ephemeris.motionOf(id)`)、`body.build(scene, sunOcclusion, sunLight)` へ。`instanceof EarthView` → `Earth` |
| `hud/frame/frame-labels.ts:5-10` | `CelestialView` の語を消す |
...
- `npm run typecheck`。
- `grep -rn "CelestialView\|BODY_CLASSES\|bodyDef(SOLAR_SYSTEM" src` が 0 件。
...
| `ReferenceFrames.frameOf` / `frameFor`(同じ対に同じ参照) | `:491-509` |
| `ReferenceFrames.transformAt`(登録天体の回転は motion へ委譲) | `:520-554` |
| `CelestialBodyWindows.celestialBodiesAt` / `gravityAttractorsAt` / `atmosphereCelestialBodiesAt` | `:629-650` |
...
`ALT_REGISTRY`(`stage-debug-alt-system.ts:33-58`)は `zephyrusSystemMotions()` という同じ形の構築
コードにする。
...
| `game/stages/stage.ts:103-114` | `createEphemeris` が `solarSystemMotions(...)` を組んで `new Ephemeris(motions, …)` |
...
| `new Ephemeris(SOLAR_SYSTEM, 'earth', EPOCH_T_OFFSET, phases)` を組むテスト(`tests/physics/` 18 ファイル) | `tests/physics/test-helpers.ts` の `solarSystemEphemeris(phases)`(内部で `solarSystemMotions('earth', …)`)に。`solar-system.test.ts` / `small-bodies.test.ts` 等の `bodyDef(SOLAR_SYSTEM, id)` は名前付き定数(`EARTH` 等)を直接読む |

#### 達成条件と検証

- `npm run typecheck`、`npm run test`(全層。`solar-system.test` / `small-bodies` / `equatorial-satellites` /
  `laplace-satellites` / `irregular-satellites` が個々の天体の値を固定している)。
...
- `solarSystemMotions(...).all.map((m) => m.id)` が `9a7c45d4` の `Object.keys(SOLAR_SYSTEM)` と同じ並び
  (テストとして1本残す)。
...
`ephemeris.*` の呼び出し(228 箇所)と `.registry`(68 箇所)、`Ephemeris` を受け取る 69 ファイルを、
次の対応で一斉に移す。`celestialBodyName` / `bodyClassOf`(44 箇所)も同時に消す。`CelestialSystem` は
...
| `physics/halo.ts:92-103,311,320`、`physics/orbit-guide.ts:70-74,134,202-208,246-272` の `ephemeris` 引数 | 副天体の `OrbitingMotion`(主天体は `motion.primary`)。`'earth' in registry` の判定は呼び出し側(`orbit-guide-lines.ts`)が `celestialSystem.has('earth')` で行う |
| `Stage.createEphemeris`(`stages/stage.ts:67-70,103-114`、`stage-debug-alt-system.ts:59-61`)、`launcher.ts:44-51,135-140`、`game.ts:130,145,176-178` | `Stage.createCelestialSystem(...)` が `solarSystem(...)`(game 側パック: 手順8 までは手順4 の表で個体を組む)を返す。Game は受け取って `build(scene, pipeline.*)`・dispose |
| `save/snapshot-service.ts:23,31,92`、`save/save-data.ts:336` | `celestialSystem.serialize()`(`phaseOffsets` は構築時に受け取った record をそのまま返す — 明示 0 のキーを落とさない) |
...
| 群 | ファイル |
| --- | --- |
| game 直下 | `game.ts`、`display-window-manager.ts`、`nav-target.ts`、`targeter.ts`、`orbit-reference.ts`、`frame-anchors.ts`(型のみ) |
| game/lines・pickable | `lines/entity-line-manager.ts`、`lines/trajectory-line.ts`、`pickable/orbit-pickables.ts`、`pickable/map-pickables.ts`、`pickable/map-context-actions.ts`、`pickable/map-pickable-menu.ts`、`pickable/map-property-rows.ts` |
| game/camera | `camera-system.ts`(`:163-169`)、`focus-markers.ts`、`focus-target.ts`(tests 対象: `ReferenceFrames` + `(id) => CelestialMotion \| null` を受ける)、`map-camera.ts` |
| game/celestial | `celestial-system.ts`(内部の `this.ephemeris.*` 約 25 箇所)、`body-visibility.ts`、`map-visibility.ts`、`planet-distance.ts`、`planet-light.ts`、`scale-grid-view.ts`、`orbit-guide-lines.ts`、`zero-velocity-lines.ts`、`point-field-view.ts`、`body-class.ts`(削除)、`celestial-appearance.ts`、`solar-system/solar-system.ts`(新規: game 側パックの入口) |
| game/hud | `frame/frame-labels.ts`、`frame/anchor-zone.ts`、`frame/frame-controls.ts`、`frame/rotation-zone.ts`、`frame/camera-frame-panel.ts`、`frame/trajectory-frame-panel.ts`、`orbit/orbit-analysis-data.ts`、`panels/physical-object-list-panel.ts`、`panels/physical-object-list-order.ts`、`panels/physical-object-list-tree.ts`、`object-groups.ts` |
| game/plan | `plan.ts`(tests 対象: `CelestialBodyWindows`)、`plan-display.ts`、`plan-editor.ts`、`plan-path.ts` |
| game/simulation | `simulator.ts`(`:120,176`)、`substep-bodies.ts`(`CelestialBodyWindows`)、`entity-state-at.ts`、`entity-manager.ts`、`predictor.ts` |
| game/game-entity・player・stages・creative・save | `game-entity.ts`、`enemy.ts`、`player.ts`、`player-fire.ts`、`stages/stage.ts`、`stage-debug-alt-system.ts`、`stage-utils/wave-attack.ts`、`creative/object-placer-panel.ts`、`creative/orbit-form-fields.ts`(`:46,59` の `SOLAR_SYSTEM` 直引きも `bodyOf(...).def` へ)、`save/snapshot-service.ts`、`save/legacy-save.ts`(コメント) |
| physics | `halo.ts`、`orbit-guide.ts`、`ephemeris.ts`(削除) |
| src 直下 | `launcher.ts`、`perf-meter.ts` |
| tests | `tests/physics/` の 18 ファイル(`solarSystemEphemeris(phases)` → `solarSystemMotions('earth', phases, …)` + `new CelestialBodyWindows(all)` / `new ReferenceFrames(all, origin)`)と、`Ephemeris` の型を受ける `tests/game/` のファイル。`tests/game/body-visibility.test` / `focus-target.test` の `CelestialEntity` を要する部分は削る。`tests/perf/` の壊れる実験は削除 |
...
手順4 の id キーの表(`CELESTIAL_APPEARANCES`)を、physics 側の系ファイルと対になる game 側の系ファイル
...
写像型が「physics 側に居る天体に見た目が無い」をコンパイルエラーにする。`fallbackCelestialAppearance` は
...
| ファイル | 何をするか |
| --- | --- |
| `game/celestial/celestial-appearance.ts` | `game/celestial/solar-system/{sun,earth-system,inner-planets,mars-system,jupiter-system,saturn-system,uranus-system,neptune-system,dwarf-planets,small-bodies}.ts` に分割して消す。手順7 で作った `solar-system/solar-system.ts` の入口が各系を集める。`fallbackCelestialAppearance` は架空星系のパックへ |
...
| `render/atmosphere.ts:23-47` | `ATMOSPHERE_OPTICS` / `atmosphereOpticsOf` を消す(地球は `earth-system.ts`、火星は `mars-system.ts` へ)。`AtmosphereOptics` 型・`atmosphereDraws`(`:163`)は残す |
...
| `game/celestial/celestial-system.ts:365-384` | `atmosphereOpticsOf(id)` → `entity.atmosphereOptics` |
| `game/hud/orbit/orbit-projection-tab.ts:16` | `textureOf(id)?.url` → `celestialSystem.bodyOf(id)` の surface から |
| `game/stages/stage-debug-alt-system.ts` | `zephyrusSystem()`(motions → fallback の個体 → `CelestialSystem`) |
...
- `npm run typecheck`。
- `grep -rn "CELESTIAL_ALBEDO\|CELESTIAL_TEXTURES\|ATMOSPHERE_OPTICS\|albedoOf(\|bondAlbedoOf(\|lightSourceAlbedoOf(\|textureOf(\|texturePhotometryOf(\|atmosphereOpticsOf(\|CELESTIAL_APPEARANCES\|celestial-appearance\|SolarSystemId" src` が 0 件。
...
| ファイル | 何をするか |
| --- | --- |
| `game/celestial/celestial-entity.ts` | `referenceLine: OrbitLine \| null`。`referenceElementsAt(t)`(`celestial-system.ts:548-560` の移動。中心は `motion.primary`)。`showReferenceLine(style)` / `hideReferenceLine()` / `syncReferenceLine(fo, camera, opacity)`。マップ専用の付随表示のための `syncMapOverlay(…)` フック(既定は何もしない) |
| `game/celestial/celestial-system.ts:45-53,71-74,128-135,162-166,203-204,269-278,399-439,510-518,527-546,561-565` | `referenceLines: Map` を消す。`sync` が個体ごとに「出すか(policy.body(id).orbit)・濃さ(`referenceLineOpacityAt`、フェード距離の定数 `:45-53`)」を決めて個体へ指示する。`referenceOrbitLines` getter は `bodies` から線を持つものを列挙 |
| `game/celestial/celestial-system.ts:55-69,130-131,419-424,440-508,209-213` | 静止軌道リング・GEO ラベル・`buildGeoElements`・`earthSpinPhase0` を `Earth` へ |
| `game/celestial/earth.ts` | 上を受け取る。`syncMapOverlay` で GEO を描く |
| `game/celestial/celestial-system.ts:324-353` | 環の影: `referenceIds` + `bodyDef` の走査を `bodies` の `def.rings` 走査に |
| `game/pickable/orbit-pickables.ts:41` | getter の戻り型に追随 |
...
いないこと。** 満たさないなら着手せず、残っている import(`CameraSystem` / `FloatingOrigin`(ともに
`game/camera/`)/ `OrbitLine`(`game/lines/`)/ `MarkerManager` / `MapVisibilityPolicy` / `game/const`)を
列挙して報告し、ユーザーの判断を待つ。**この時点で挙動は変えない。**
...
- `npm run typecheck`、`npm run test`、`npm run dev` で起動。

## 見積り

編集の見積り。検証(`typecheck` + `test` を手順あたり 2〜3 回 + 目視)は手順あたり 30 分で
別に足す(11 手順 × 30 分 ≈ 5.5 時間)。`test:physics` / `test:game` の層別・名前絞り込みで1回の
検証は短くなるが、全体を決めるのは編集量と委譲の並列度。

| 手順 | 導出 | 見積り |
| --- | --- | --- |
| 0 | 別名3つの置換 233 箇所(33 + 12 + 6 ファイル)× 0.5 分 + 定義の削除 | 2 時間 |
...
| 6 | 98 項を名前付き定数 + 構築関数へ 98 × 2 分 + 共通部の分割 500 行 × 0.2 分 + `kind`/`planet` 読みの置換 約 30 箇所 × 3 分 + import 33 ファイル × 3 分 + テスト 18 ファイル × 3 分 | 8 時間(系単位に委譲すれば実時間 4 時間) |
| 7 | 呼び出し 228 + 68 + 44 = 340 箇所 × 1.5 分(機械的な置換)+ 受け渡しの配線 69 ファイル × 5 分 + テスト 25 ファイル × 10 分 | 18 時間(9 群に委譲すれば実時間 5 時間) |
...
編集 約 52 時間(委譲込みで約 33 時間)+ 検証 約 5.5 時間。実測が得られたら置き換える。
...
| 手順1 で暦パック経路の衛星分岐(`ephemeris.ts:364-373`)か有効期間の判定(`isValidAt`)を落とす | パック期間で衛星が親からずれる / 期間外で例外 | 手順1: `absolute-ephemeris.test`。`npm run dev` で開始日時を高精度期間の内と外にして月の位置 |
| 手順1 で `origin` が自分自身のとき `at` が厳密 0 にならない書き方をする | 地球が原点から 1e-9 m 級でずれ、距離ゼロ判定が揺れる | 手順1: `celestial-body.test`(`:163` 地球が原点) |
...
| 手順4 で `Sun` の既定引数を消したあと、架空星系の恒星(fallback)が半径 0 になる | 恒星の見た目が消える | 手順4: `?stage=debug-alt-system` は恒星が無いので露見しない — 恒星付きの架空星系をテストで一度作る |
| 手順5 で `ReferenceFrames.frames` / `frameOf` が同じ対に同じ参照を返さなくなる | `lines/trajectory-line.ts` の `frame === lastFrame` が毎フレーム外れて焼き直し | 手順5: 座標系を切り替えて軌道線の描画が重くならない(負荷確認ウィンドウ) |
...
| 手順7 で `phaseOffsets` を motions から再構成し、明示 0 のキー(`{moon: 0}`)を落とす | セーブの内容が変わる(ロード結果は同じ) | 手順7: 構築時の record をそのまま返す。`tests/game/save-ephemeris-context.test` |
...
| 手順7 で tests がコンパイルする game ファイル(`tsconfig.test.json`)に `CelestialSystem` を渡す | `npm run test` が THREE / DOM の連鎖で落ちる | 手順7: `plan.ts` / `focus-target.ts` / `point-field.ts` / `placement-validation.ts` は物理部品だけを受ける |
| 手順7 で `tests/perf/` を放置する | `tests/perf` は npm script から呼ばれず、黙って腐る | 手順7: 壊れる実験ファイルは削除する(決めたこと 7) |
| 手順7 で `Game` のコンストラクタ内の `CelestialSystem.build(scene)` の位置を動かす | `scene.add` の順が変わり、renderOrder が同じ半透明物(環・大気・線)の重なりが変わる | 手順7: 今の `EnvironmentScene` 構築位置(`game.ts:176`)で `build` を呼ぶ |
| 手順8 で写像型を `Partial` にする、または fallback を太陽系パックでも使う | 見た目の書き忘れが黙って単色球になる | 手順8: 達成条件の「1体消して typecheck が落ちる」 |
...
| 手順9 で `Earth` の自転(`earth-view.ts:40` の `phase0 + 2πt/SIDEREAL_DAY`)を physics の `spinRotationAt('earth')`(`ephemeris.ts:581-584`、phase0 なし)へ揃えたくなる | 挙動が変わる(地球自転系の向きとテクスチャが一致するようになる)。是正としては正しいが本計画の外 | 手順9: 揃えない。別件として報告する |
