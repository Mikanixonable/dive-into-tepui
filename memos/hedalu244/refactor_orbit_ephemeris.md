...
> 全天体を配列へまとめる操作も — は軽微なはず。**

**この「軽微なはず」は未測定。** 持ち主は割り終えた(下表)ので、残る問いは
**それぞれのキャッシュが要るかどうか**だけ。

| # | 持ち主 | 形 | 見立て |
| --- | --- | --- | --- |
| 1 | `PlanetMotion.helioCache`(`celestial-motion.ts:407`)/ `SatelliteMotion.relCache`(`:472`) | `TimeRing` ×2種 | 1体ぶんの重い計算を、その1体が持っている。残す |
| 2 | `CelestialMotion.eciCache`(`:122`) | `TimeRing` ×N | 1体ぶんの ECI 瞬間値。ECI 化が「引き算1回」なら**要らない**(中の `helioStateAt` は #1 が畳む) |
| 3 | `CelestialMotion.eciOriginCache`(`:123`) | `TimeRing` ×N(埋まるのは原点の1本だけ) | 原点天体1体ぶんを「全天体が同じ t で引く」前提で畳む。**これは要る** — 落とすと原点の暦パック評価が天体数ぶん走る(実測: 1時刻あたりミス1回、ヒット 97 回) |
| 4 | `CelestialSystem.allCache` / `gravityCache` / `atmosphereCache`(`celestial-system.ts:84-86`) | `TimeRing<配列>` ×3 | 畳んでいるのは**配列の同一参照**だけ。まとめる操作が軽微なら要らない |
...
`game.ts:521-522` は `simTime` と `displayTime` の2本を引き、`substep-celestial-bodies.ts:21`
...
(`celestial-system.ts:228`)は**合算しか出さない**。内訳を一時的に出し、
**ヒットしていない・ヒットが1回しかないキャッシュは消す。**
...
1. **論点2 を測る。** 持ち主は割り終えたので、あとは内訳を出して要らないキャッシュを消すだけ。
