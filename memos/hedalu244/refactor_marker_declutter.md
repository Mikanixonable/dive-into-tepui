...
| モジュール | 前 | 後 |
| --- | --- | --- |
| `marker-manager.ts` | 670 | **382** |
| `label-declutter.ts` | — | **103**(新規) |
| `label-layout.ts` | — | **246**(新規) |
| `crowding.ts` | 122 | **139**(`MARKER_PRIORITY` を受け入れた) |
| **合計** | **792 / 2 モジュール** | **870 / 4 モジュール**(+10%) |

最長のメソッドは `label-layout.pushApartFromNeighbors` の 61 行
(前は `relaxLabelRects` の 134 行)。

**性能**

同じ処理を同じ順序で呼ぶので変わらない。ひとつだけ減る方向へ動いた:
`NEVER_HIDE_ICON_CLASSES`(9件)と `COMBAT_MARKER_CLASSES`(7件)の `String.includes` 走査が、
衝突ペアごとからレコード生成時1回へ移った。衝突ペア数を P とすると 16P 回/frame の部分文字列探索が
0 になるが、P はせいぜい 100 台なので**誤差の範囲**。これは目標ではなかった。
