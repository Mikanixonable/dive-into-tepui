...
| 6.2 | `'primaryRel'` が中心天体を持たない | 実行時に中心が決まる経路(`el.center`)があるので**型では塞げない**。手順5 で数えたところ `addPrimaryRelative` の呼び出しは **5箇所**(しきい値の3を超える)ので専用関数へは割らず、`kinematic-state.ts` のタグ説明に「中心が誰かは型に現れない」と明記するに留めた。別作業 |
