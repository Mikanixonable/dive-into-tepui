...
9. 通常ステージと高負荷デバッグステージ(タイトルで `L`)の両方で、負荷確認ウィンドウの
   `gravitySources` が **65 で一定**(天体暦の重力天体数)になる。
...
天体暦のレジストリは全101天体、うち `mu ≠ 0` が 65 天体(実測。残りは表示専用)。

- **通常ステージ**: 変化なし。`attractorsAt` は現在も 65 体を返す。
- **高負荷デバッグステージ**: `attractorsAt` が 65 + 300 = 365 体 → 65 体(実測)。
  `classifyAttractors` は `mu` の全ソートに支配されるので、比較回数は
  365 × log₂365 ≈ 365 × 8.5 ≈ 3,100 → 65 × 6 = 390 で **約 1/8**。
