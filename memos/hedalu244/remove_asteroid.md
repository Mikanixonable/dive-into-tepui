...
`DEVELOP/SPEC/ORBIT.md` と `DEVELOP/SPEC/COMBAT.md` から、重力を及ぼす小惑星に関する記述を
削除し、廃止後の意図へ書き換える。経緯は残さない。

**「多体重力」節**の書き換えで、次の3点が本文に立っていること。

- 重力源として合算するのは天体暦上の天体だけである。
- 艦・敵機・弾丸・薬莢・破片・補給物資は、いずれも重力を及ぼさない。物体どうしが影響を
  及ぼし合う経路は剛体接触だけであり、それは連続的な軌道の変更ではない。
- 質量が比較しうる複数天体の真の多体問題は、この仕様では扱わない。

**「未来予測」節**では、次の3つの項目を**削除する。置き換えの記述を足さない。**

- 「予測を消費する経路に乗るのは、自身は引力を及ぼさない物体に限る」という限定と、その理由。
- 消費されない弧を一定のシミュレーション時間ごとに実位置から張り直す規則。
- 「小惑星の外挿位置を重力源・衝突相手として用いる」の項。

**ここに「何が消費されるか」を言い直した文を書き足さない。** 予測を辿るか自分で積分するかを
決めるのは実シミュレーション側の責務であって、予測の仕様ではない。**予測の仕様が負うのは
「実シミュレーションへ引き継がれうる精度で積むこと」だけで、それは同節の「消費される予測の
刻み幅と保持サンプル間隔は、実シミュレーションのサブステップと同じ規則で決まる」が既に述べている。**
限定を消せば、直前にある「ある時間帯の状態を決める積分は、常にちょうど1本だけ存在する」が
例外なく成り立つ形で残る。

「艦・小惑星などの未来の軌道(予測軌道)は」の列挙からも小惑星を落とす。

**「消費される予測」という限定語は残す。** 限定が外れるのではなく、指す対象が「引力を及ぼさない
物体の予測」から「物体の予測すべて」へ広がるだけで、**対比の相手は軌道計画の区間の弧のまま。**
書き換え後に読み返し、その対比が読み取れるか確かめる。

**「天体表面への到達判定」**の「重力を及ぼす小惑星は対象に含まれない」、**「剛体接触」**の
「重力を及ぼすかどうかは剛体接触に影響しない(小惑星も対等に反発する)」、
**「数値的な限界」**の「多数の小天体が密集する場」の例も削除する。

**完了条件**: `grep -rn "小惑星" DEVELOP/SPEC/` の一致が、天体暦上の準惑星・大型小惑星
(ケレス・ベスタ等)を指すものだけになる。
