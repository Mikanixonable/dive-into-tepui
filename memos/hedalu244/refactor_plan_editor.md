...
### 手順1. `SPEC/PLAN.md` へ、噴射後の軌道の表示と大気圏警告を書く

**目的.** 軌道計画パネルが選択中ノードについて何を示すかは、いま `SPEC/` のどこにも書かれて
いない。手順8 で判定の根拠を変えるので、先に「どう振舞うべきか」を確定させる。
**この手順ではコードを変えない。**
...
| ファイル | 何をするか |
| --- | --- |
| `DEVELOP/SPEC/PLAN.md`「4. Δv の編集」末尾 | 下の2項を足す |

```markdown
- ノードを選択しているあいだ、そのノードを実行した後に乗る軌道の近点・遠点の高度、軌道傾斜角、
  周期を軌道計画パネルに示す。
- **中心天体が大気を持ち、その軌道の近点が大気の抵抗を無視できないところまで下がっているときは、
  近点の行に `⚠ 近地点が大気圏内` を添える**(近点の呼び名は中心天体に従う)。大気を持たない
  天体では出ない。これは助言であって、大気へ入る計画を禁じるものではない — 計画軌道が
  打ち切られるのは天体の地表へ到達したときだけである(ORBIT.md「未来予測(予測軌道)」)。
```

**達成条件と検証**

- `grep -n "大気圏内" DEVELOP/SPEC/PLAN.md` が 1件。
- 足した文に識別子・定数値・アルゴリズムが入っていない(`SPEC/README.md`「何を書くか」)。
- `git diff --stat src/` が空(コードは1行も変えていない)。
