...
コードの手順はすべて実施済みで、`npm run typecheck` と `npm run test:game` は通っている。残るのは
`npm run dev` でマップモードを開いて目で見る検証だけ。**手順8 の項は他と混ぜずに単独で見る** —
ここだけ挙動が変わるので、混ぜるとどちらの変更で見え方が変わったのか分からなくなる。

- (手順3)カメラをズームイン/アウトして、矢印ハンドルの間隔と 3D 矢印の見かけサイズが変わる。
- (手順4)矢印ハンドルを 60px 未満ドラッグするとその矢印が少し伸び、60px を超えて続けると
  さらに伸びて上限で止まり、離すと戻る。ラッチさせたまま別の場所をクリックして選択を外すと、
  Δv の加算が止まる。
- (手順5)ノードを選ぶと6方向のハンドルがノードの周りに並ぶ。
- (手順6)矢印ハンドルで加えた向き・符号・量どおりに TRAJECTORY パネルの PRO/NRM/RAD と `m/s`
  が動く。月の影響圏の内側と外側の両方で見る。
- (手順7)ノードを2つ以上置き、選択を切り替えるたびに `▸` が移り、選択を外すとパネルが消える。
- (手順8)地球周回でレトログレードに Δv を増やして近地点を下げると、`近地点 Pe` が 120km を
  切るあたりで `⚠ 近地点が大気圏内` が出る。傾斜角 0° 付近と 90° 付近の両方で見る — 極寄りに
  近点がある軌道では、表示値が約 99km まで下がらないと出ないのが正しい(表示の近点高度は赤道
  半径基準、大気の高度は基準楕円体基準)。月周回では近月点をどれだけ下げても出ない。

## 実測

| 量 | 見積り | 実測 |
| --- | --- | --- |
| `plan-editor.ts` | 603行 | 599行 |
| `syncGizmo` | 44行 | 39行 |
| `syncPanel` | 35行 | 19行 |

しきい値の数値裏取り(地球の層表と `physics/atmosphere` の実関数): 赤道上の近点は表示高度 120km で
警告が出て 122km で消える。極上の近点は表示高度 100km でまだ出ない(基準楕円体高度 121.4km)。
絶対 ECI を渡した場合は密度 0 になる(落とし穴の表のとおり)。
