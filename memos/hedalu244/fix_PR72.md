...
- PR-B(小さな修正と計測): 手順 4〜7(実施済み)
- PR-C(雲場の作り直し): 手順 8(実施済み。fix_cloud_projection.md も完了)
...
| | 手順 7 の時点 | LOD 撤去後 | cap へ移したあと |
| --- | --- | --- | --- |
| 雲の生成 | 12.00 ms | 9.64 ms | 5.57 ms |
| 表面雲 | 5.31 ms | 4.14 ms | 3.91 ms |
| 大気(雲あり) | 9.60 ms | 7.32 ms | 7.28 ms |
| 雲影 | 1.46 ms | 1.07 ms | 0.96 ms |

**手順 7 の列だけ計測の間の周波数のドリフトで 2 割ほど高い**(同じ巡で「Gバッファ」も動いた)。
段どうしを厳密に比べるなら測り直すこと。
