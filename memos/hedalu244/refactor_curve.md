...
### 決定1b. 頂点数の上限は `Curve` が既定で決める。外から渡すのは収束しない曲線だけ

`maxVertices` は本来「適応分割の打ち切り」という補助的な安全弁だったが、6箇所すべてが外から
値を渡す形になり、品質のチューニングノブとして扱われるようになっていた。「なぜか角ばる」に
対してどれを弄ればよいか分からず、アドホックな調整が設計思想から外れる原因になる。

**1周ぶんの閉曲線は 360 頂点ほどで自ら収束する**ので、上限は本来効かない。既定値 512 を
`Curve` が持ち、外から渡すのは**適応分割が収束しない曲線**(1本に何周ぶんも入るもの)だけに
限る。

調整が要るときに触る先は、負荷側の頂点数ではなく**品質側のサジッタ目標**
(`CurveOptions.maxSagittaPx`、既定 0.5px)にする。既定で足りているので**現時点では誰も
書き込まない** — 入口だけ用意し、必要になった線が現れたらそこで渡す。

→ 覆した場合: 呼び出し側ごとに頂点数を宣言する形へ戻る。
