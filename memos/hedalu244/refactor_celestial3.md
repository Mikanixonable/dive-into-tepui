...
| 3 | 回転基準系が2枝で食い違う | **解消済み**(`ef074bd1`)。3節へ | — |
...
## 3. 回転基準系 — 実状態へ統一済み(`ef074bd1`)

`orbitFrameRotationAt` / `orbitNormalAt` は**常に実状態から組む。** SPEC/CELESTIAL.md 8節が
規定する「x̂ = 主天体→対象の実位置、ẑ = その瞬間の公転面法線」に一致し、供給源(暦パック /
解析暦)で定義が変わらない。

### 実態調査で分かったこと

Creative の既定元期は `20115-05-14T06:00:00 TDB` = JD 9068045.75 で、**far-future パックの
`validStartJdTdb` そのもの。** パックは読み込まれ、`earth` `moon` `mercury`〜`pluto` の
**10体がパック枝**、残る **87体が解析枝**で動いていた。

**統一で値が動いたのは月だけ。** 他の 86 体は周期項を持たないので実状態＝平均要素(実測
0.0000 deg)、惑星も軌道が乗る点が系の重心でその解析運動は純二体。

「混ぜると角速度が滑らかでなくなる」という記録(`satellite-orbit.ts`)の実量は、月で
**max|dω/dt| が 1.39 倍、1時間刻みの2階差分が 1.54 倍。** 跳ねる高周波成分は入らない
(月の周期項は最短でも半月周期)。
...
| | 前 | 後 |
| --- | --- | --- |
| 月が回転系 x̂ から離れる角 | 最大 1.94°(コメントの見積り 2.5°) | **0**(厳密) |
| L3 の反月方向からのずれ | 最大 1.4° | 1e-10 未満 |
| L4/L5 と月の距離の正三角形からのずれ | 3% | 1e-12 未満 |
| パック有効期間の端での定義の変化 | あり | **無し** |

端に残る 153° の飛びは、**月の ECI 位置そのものの飛び(762,886 km)を忠実になぞったもの** —
far-future パックの月と解析暦の月が 10 年で二体距離の2倍ずれる、というモデル差(下の別件)。

### 残った別件

- **ω が軌道面自身の歳差を含まない。** `(r×v)/|r|²` は x̂ の回転を厳密に表すが、ẑ が動く
  ぶんを落とす — 月で |ω| の 1.1%(2.7e-8 rad/s)、LEO の機体位置で 0.014〜0.27 m/s。
  **これは元からパック枝にあった性質**で、統一によって全天体へ広がった。状態だけからは
  robust に取り出せない — ĥ の有限差分はパック由来と解析由来で 100% 以上食い違い、外惑星では
  丸めに埋もれる(同じ地球で 1.9e-14 と 8.5e-11)。直すなら軌道要素の変化率を運ぶ形が要る。
  `frame.test.ts` のしきい値はこの実測から引いてある(0.5 m/s。ω をまるごと落とすと 18〜22 m/s)。
- **far-future パックの月と解析暦の月が、有効期間の終端で 762,886 km 離れる。** 端をまたぐ
  瞬間に月が飛ぶ。パックの外へ出たとき何を見せるべきかは仕様側の問い。
