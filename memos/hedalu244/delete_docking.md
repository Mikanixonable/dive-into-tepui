...
### 手順 1. SPEC からドッキングと基地パネルの仕様を落とす
...
| `DEVELOP/SPEC/SAVE.md` | 51行「基地の状態(所持金・保管庫・ドックしている艦)」→「基地の状態(所持金)」。53行「(戦闘ビュー/マップビュー/ドック)」→「(戦闘ビュー/マップビュー)」 |
| `DEVELOP/SPEC/UI-DESIGN.md` | 376-378行「物資・電力の融通」の項を落とす |
| `DEVELOP/SPEC/README.md` | 21行の GAME.md の説明から「基地とドック」→「基地」 |

**達成条件と検証.**
`git grep -n 'ドッキング\|基地パネル\|格納艦\|新造\|物資・電力の融通' -- DEVELOP/SPEC` が 0 件。
`git grep -n 'ドック' -- DEVELOP/SPEC` の残りが MAP.md:156 と RENDERING.md:123,127 の
「左ドック」3件だけ。コードを触っていないので `npm run typecheck` は自明に通る(念のため走らせる)。
