...
9. **§13 `/refactor-fixed` への追記** 今後の再発防止。

各セットで守ること(`/refactor` の改名規則):

- 簡単な作業はSonnetサブエージェントに行わせる。
- TypeScript内のリネームは参照検索等の適切なツールを使って効率的に行う。
- 文書内も含め、旧名が 0 件になるまで消す。`grep -rn "<旧名>" src tests DEVELOP CLAUDE.md .claude memos`(`tests/dist` はビルド出力なので除外)
