...
### 手順1. CODING-RULE へ規範を書く

**記録するのは規範1・2・5・6・7 と、規範4 のうち「無標 = ECI」の1項。** 規範3 と規範4 の
語順2項はこの計画で適用するが記録しない(是正後に再発の余地が無い / 条件として強すぎる)。
規範8 は保留。**コードは変えない。**

#### 1-a. 2.1 一般規則 — 「悪い命名」へ1項、規則を1段落 + 例外を1段落(規範5・規範4 の一部)

「悪い命名:」のリストへ次を足す:

```
- 片方だけ無標の対。並列なものを名前で並べるとき、片方に語を付けて片方に付けない命名。
```

同リストの直後へ:

```
**並列なものは両方を有標にする。** 同じ契約に対する2つ以上の実装・経路・分類を名前で並べる
とき、片方を無標のまま残さない — 無標の側は「これが既定」「これが本来のもの」という順位を
暗に主張してしまい、3つめが増えた時点で壊れる。並列でないもの(対を持たない単独の口)は
対象外。

**例外は1つだけで、明記して固定する。** 位置・速度を答える無標の `stateAt` / `stateOf` /
`bodyAt` は ECI(原点に置いた天体を中心とする座標)を指す。**新しい無標の口を増やさない** —
ECI 以外の原点を答えるものは、必ず原点を名前に出す(`baryStateAt` / `icrfStateAt` /
`primaryRel*`)。原点の取り違えそのものは `KinematicState` の `FrameTag` が型で止めるので、
これは安全のためではなく読みやすさのための規則。
```

#### 1-b. 2.2 へ新設「`ephemeris`」(規範1・7)

`### celestial / dynamic` 節の直後へ置く(その節が既に `ephemeris` を「この軸に載せない」と
書いており、その続きになる):

```
### `ephemeris`

**`ephemeris`(暦)は、天体 id と時刻から並進の位置・速度を答える供給源の総称。**
閉じた式か数値積分の表かは問わない。自転・重力場・大気・**加速度**は含まない — 暦が答えるのは
位置と速度だけ。日本語の「暦」「天体暦」と1対1で対応させる。

**`celestial` の内側の総称であって、`dynamic` を覆うほど上位ではない。** 積分の履歴に依存する
軌跡は天体 id で引ける表ではないので、暦ではない。前節のとおり、この語を
`celestial` / `dynamic` の対そのものに混ぜてもいけない。

**例外**: `ephemerisSeconds` / `EphemerisScale`(`physics/time/`)は天文の定訳
"ephemeris time" であって供給源とは無関係。`OrbitalElements` と同じく定訳を優先する。
```

#### 1-c. 2.2 へ新設「`analytic` / `numeric`」(規範2)
...
同じ量に2つ以上の求め方があるとき、**求め方は表現で切る。**

- **`analytic`** — 閉じた式で解いたもの。
- **`numeric`** — 数値積分したもの、およびその結果を区間多項式などの表へ焼いて引くもの。

**精度・優劣の語で切らない。** `precise` / `accurate` / `high*` は「どれだけ合っているか」と
いう別の問いに属していて、実装が入れ替われば嘘になる — 数値解が解析解より必ず正確とは限らない。
名前が答えるのは「どう求めたか」であって「どれだけ合っているか」ではない。
```

#### 1-d. 2.2「`orbit` / `path` / `trajectory`」の `orbit` の定義を差し替え(規範6)

現行の1行(「解析的に解ける軌道。時刻から位置が閉じた式で決まるもの。」)を次で置き換える。
**`path` / `trajectory` の行は触らない**(規範8 が保留のため):

```
- **`orbit`** — **基準となる重力源に対して定まる1本の経路。** 判定基準は基準の有無だけ。
  - **基準は1天体とは限らない。** ケプラー軌道の中心天体1体でも、CR3BP の2天体系と
    そのラグランジュ点(ハロー・リヤプノフ・DRO・リサジュー)でもよい。
  - **どう求めたかは問わない。** 閉じた式で解いたものも、数値積分して焼き込んだ点列も軌道。
  - **閉じることを条件にしない。** 永年変化率を持つ軌道は歳差と公転周期が無理数比になれば
    厳密に非周期だし、双曲線軌道はそもそも繰り返さない。どちらも軌道である。
  - **基準は値と一緒に運ばれていなければならない。** その場で選び直す手続き
    (`strongestAttractor` など)は基準の代わりにならない — 選んだ結果を値へ載せて初めて
    軌道になる。基準を持たない点列は軌道ではない。
```

末尾の1文にも1句足して、`trajectory` との境界を新しい基準で言い直す:
...
**検証.** `grep -n "解析的に解ける軌道" DEVELOP/CODING-RULE.md` が 0 件。
`grep -cn "^### .ephemeris.$" DEVELOP/CODING-RULE.md` と `grep -cn "^### .analytic. / .numeric.$" DEVELOP/CODING-RULE.md` がそれぞれ 1 を返す。
`grep -n "並列なものは両方を有標" DEVELOP/CODING-RULE.md` が1件。
`grep -n "無標の .stateAt" DEVELOP/CODING-RULE.md` が1件。
**`pack` の規則が増えていないこと**(規範3 は記録しない)。

### 手順2. `.epk` 読み口の器の語を1層へ寄せる

`ephemeris-pack/` の中だけ。**置換範囲を `src/physics/ephemeris-pack/` と `tools/ephemeris/` に
限る**(`series` は HUD のチャートでも使う)。

2.3「器」の表の8件を実施。`EphemerisSeries` → `PackSegment` は **TS 型だけ**で、
`validateManifest` が読むワイヤキー `series` には触らない。`format.ts` の冒頭コメントへ
「ワイヤのキーは `series`、コード上の語は `segment`」を1文足す。

**検証.** `npm run typecheck` / `npm run test:physics`。
`grep -rn "EphemerisSeries\|EphemerisPackFormatError\|DecodedEphemerisPack" src/ tests/ tools/` が 0 件。
`grep -n "EPHEMERIS_PACK_VERSION = " src/physics/ephemeris-pack/format.ts` が `1`。
達成目標6 のコマンド。
