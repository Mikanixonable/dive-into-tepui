...
| 4 | R4 の全文、R11 の1・2。R1・R5・R10 の「ラン跨ぎ」を「セーブを跨いで共通」へ言い直す | — | (R11 の3は段 6 で足す) | `game/viewer/` への片方向、`settings/` と `game/viewer/` の相互 import、セーブの型から表示の導出への import |
...
2. **「mutable ≈ セーブされる領域」は、R11「モデル層 = セーブ対象」として規則にできる。** mutable な値は、操作途中の状態を除いて、ほとんどすべてが何らかの形で残るべきである。残し方は2つあり、取り違えない(ユーザー判断 2026-09-17)。
   ...
   - **視点なのにセーブされていない値を載せる**(手順 4-4)。軌道要素の基準と、予測パネルの状態(計画軌道の座標系、未来側・過去側の表示期間と任意期間、スクラブ位置、目盛りの表示と相対表記、通過時刻の併記、カメラの基準に追随)。
   - **進行なのにセーブされていない値を載せる**(手順 4-4)。ワープ段と `autoWarpUntil`、基地の計画・計画実行モード・`fineAttitude`、creative の手動 spawn の距離と表示設定、デバッグステージの状態、採番器の次の番号。
...
| ワープ段・`autoWarpUntil` | 進行(手順 4-4 でセーブへ載せる) | 積分の刻み(`simulator.ts:80-91`)、AI の射撃可否(`enemy-fire-controller.ts:62`)、接触(`engagement-zone.ts:51`)、補給(`logistics.ts:124-134`)が読む |
...
| 予測パネルの状態: 計画軌道の座標系(中心と回転)、未来側・過去側の表示期間と任意期間、スクラブ位置(`sliderT`)、目盛りの表示と相対表記、通過時刻の併記、`followCamera`(カメラの基準に追随) | 視点(手順 4-4 でセーブへ載せる) | 座標系は天体・機体・役割を指し、表示期間とスクラブ位置はそのゲームの時間を指すので、§6 によりセーブに残る(時間の側は手順 4-0 で SPEC に足す)。残りの3つは SPEC が決めないので、同じパネルの状態と一緒に置く(K1-2)。いまは `display-window-manager.ts:90-98` が持つが、目盛りの表示だけは正本を持たず DOM の `hidden` を直に書いている(`predict-panel.ts:368-373`)。どれもセーブされず、ページの読み込み直しで既定へ戻る。表示期間は予測の horizon と履歴の保持長を決めるが、進行へは需要として届く(`display-window-manager.ts:63-71`)。`followCamera` の正本は `711c6611` で HUD の public フィールドからここへ移った |
| 軌道要素の基準モード | 視点(手順 4-4 でセーブへ載せる) | 地球・月・ターゲットを指す選択。表示だけが読む(`orbit-reference.ts:54`)。いまはセーブされない |
| 軌道ガイドの選択 | 視点 | 系と参照軌道が天体を指すので、`962db128` でブラウザからセーブへ移った。いまは `Game` が持ち(`game.ts:165,247,290-293,715`)、型と読み直しは `game/celestial/orbit-guide/orbit-guide-settings.ts` にある。**セーブの形式の一部なので、手順 6-3 で表示の導出へ出さない** |
...
| creative の spawn 距離・表示設定・連番 | 進行(手順 4-4 でセーブへ載せる) | spawn の規則が読む。距離と表示設定は保存されない。連番は構築時に復元済みの敵から数え直す(`66896940`、`manual-spawn.ts:35-45`)ので衝突はしないが、破棄済みの番号が再び出うるので、保存前と同じ番号にはならない |
| 採番器の次の番号(`EntityIdAllocators`) | 進行(手順 4-4 でセーブへ載せる) | 復元した id の最大値の次から数え直す(`entity-id.ts:21-25`)。破棄済みの実体(弾など)があると、保存前より小さい番号へ戻る |
| 基地の計画・計画実行モード・`fineAttitude`、デバッグステージの `enemyFireEnabled`・`waveCount` | 進行(手順 4-4 でセーブへ載せる) | `base.ts:204-219`(`BaseSaveData` に計画が無い)、`stage-debug.ts:23-24`。SAVE.md は基地について所持金しか挙げず、ワープ段とデバッグステージには触れない(手順 4-0 で「進行はすべて保存する」と書く) |
...
新設するのは `src/presentation/`・`src/celestial/`・`src/game/viewer/`・`src/marker/` の4つ。**`src/marker/` は段 1 で切り出して実在する。** 残る3つは名前だけの変更なら手順 4-2〜4-4・5-3・7-3・7-4 のパスが変わるだけ。
...
3. **保存形式の互換が保たれている。** 既存の項目は変えず、足すのは省略可能な項目だけ(SAVE.md「形式の版」により版は据え置く)。段 1 に着手する前に書き出したセーブが読め、同じ位置・同じ操作対象・同じカメラで再開する。
...
| 4 | R4(全文)・R11 の1・2 | 進行のパスから `game/viewer/` への import が 0 件。`settings/` と `game/viewer/` の間の import が双方向とも 0 件。手順 4-4 で洗い出したモデル層の値が、キャッシュを除いてすべてセーブに載っている。§6 が設定と言う値を `game/` が持っていない |
...
**この段で書く規則**: R4 の全文、R11 の1・2。R1・R5・R10 の設定の文面を、セーブを跨いで共通かで言い直す。その前に、何がセーブに残るかの SPEC を直す(K3-5)。
...
既存のセーブの項目は変えない。視点のうちセーブに無いものを足すのは手順 4-4。
...
- **的面の通過の記録は進行の処理**(`game/dynamic/target-board-passes.ts`)で、どの対象について記録するかを需要(ターゲットの id)として受ける。Game は命令の適用の直後に視点から id を読んで渡す。残った `targeter.ts` は表示の導出なので、手順 5-2 のファイル単位の例外に載せる。
...
- **ロード直後にマップのカメラの pan が 0 に戻り、機体・役割への追従が慣性系へ落ちることがある不具合**(`game.ts:402` の `frameControls.setFocus` の副作用)は、§6 と「同じカメラで再開」に反するので、**保存どおり戻すように直す**(ロード時にカメラへ注視の命令を積まない。予測パネルの「カメラの基準に追随」だけは合わせる — 手順 4-4 で座標系を保存したら、保存値があるときは合わせない)。
- **実体ごとの表示設定。** 軌道線のトグルは視点が実体の **id** を鍵に持つ(実施時に同一性から変えた。復元時、アセット待ちで顔ぶれにまだいない個体の分も戻すため。航法ターゲットも id で持っている)。メニューのトグル操作は実体の `runMenu` へ届けず、表示の導出が視点の命令にする。**タンパク質の表示は、いまの UI では全個体と生成時の選択が常に同じ値に揃うので、視点の1つの値にまとめる**(実体の `displaySettings` と `ManualSpawn.display` を消す)。保存形式の各個体の `display` は、セーブを組むときに合成する。読み込みでは保存された最初のタンパク質敵の `display` から戻す(4-4 で専用の省略可能項目を足す)。
...
2. **振り分けて直す。**
   ...
   | 表の行 | すること |
   | --- | --- |
   | 視点か進行で、セーブに無い | セーブへ載せる。いま分かっているのは K2 で「手順 4-4 でセーブへ載せる」とした行 |
...
3. **往復の検査を足す。** 視点の保存形の組み立てと分解の往復で値が変わらないことを、テストで固定する(SAVE.md「保存される内容」と §6 が期待値の正本)。進行の往復は `Game` を組まないと確かめられないので、`npm run dev` で見る。
...
- `npm run check:boundaries` で、セーブの型から表示の導出への import が 0 件(手順 4-2)。
- `npm run typecheck`、`npm run test:game`(新しい往復の検査)。
- `npm run dev` で次を見る。
...
**手順と達成条件**: 「段を main へ送る(共通)」のとおり。PR 本文には次を書く。

- 手順 4-0 の SPEC の変更(§6 に時間を足したこと、SAVE.md で進行を例外なく保存すると書いたこと)。
- R4 の全文と R11 の1・2。言い直した R1・R5・R10 の文面。
- 手順 4-4 の洗い出しの表。セーブへ足した値と、キャッシュとして載せない値。
- 挙動の変化: 手順 4-4 で足した値が、読み込みで元に戻るようになったこと。既存のセーブの項目は変えていない。
...
| ファイル | 何をするか |
| --- | --- |
| `tools/check-boundaries.mjs` | 対応表を割る。`game/{hud,marker,view,pickable,map,lines,input}` = 表示の導出(`input` は段 3 の入力の解釈で、`input/input` を読むので、モデル層に数えると違反になる)、`game/viewer/` = 視点、`game/celestial/` = 未分類(暫定)、`game/game.ts`・`game/game-host.ts` = 組み立て(暫定)、その他の `game/**` = モデル層(進行)。モデル層(進行と視点)の出ていく import と `from 'three'`、進行から視点への import を判定する。`render/` で定義された語彙を `import type` で引く辺だけは暫定で通し、値の import は違反にする |
...
| 同 — 外すもの | 段 3 で入れたパス指定の禁止パターン3本(「表示の選択が進行へ漏れる禁止」「モデル層が生の入力を読む禁止」「モデル層が出来事の装置を持つ禁止」。どれも本体のコメントに「段 5 で外す」とある)と、手順 4-2 のパス指定のうち、進行から `game/viewer/` への禁止、`game/viewer/` から `settings/` への禁止、`game/save/` から表示の導出への禁止を外す(`settings/` から `game/viewer/` への禁止は段 7 まで残す)。**同じ違反を層の規則と禁止パターンの両方で数えない** — 二重に数えると許可リストが空にならない |
...
| ファイル | 何をするか |
| --- | --- |
| `tools/check-boundaries.mjs` | 対応表を R2 の最終の表と同じにする。`hud/` を装置として判定し、`settings/` の import 規則(定義層・時刻層だけを import する値の定義モジュールに限る)を足す。暫定で通していた辺(`game.ts`・`game-host.ts` の組み立て扱い、`render/` の語彙の型 import)を落とし、**手順 5-2 のファイル単位の例外の一覧を消す**(フォルダが層に揃うので要らなくなる)。手順 4-2 の `settings/` から `game/viewer/` への禁止は、`settings/` の import 規則が覆うので外す |
...
| | 7-7 main へ送る | 120 |
| | **段 7 計** | **1,953** |
| | **残りの合計** | **6,628 分 ≒ 110 時間** |

- 残りの commit は 19 本、PR は 4 本。**段の中で並行してよい組は、段 4 以降には無い**(どの手順も同じファイル群を重ねて触る)。
...
| 保存形式を組み立ての変更で変えてしまう | 既存のセーブが読めない、または戻る位置がずれる | 手順 4-3、4-4、6-4、7-6(段 1 に着手する前に書き出したセーブで確かめる。足すのは省略可能な項目だけ) |
...
| 規則の上ではモデル層なのにセーブされない値が残る(ワープ段・基地の計画・creative の手動 spawn の設定・デバッグステージ・採番器) | 読み込みで元に戻らず、「セーブ = モデル層」が崩れて見える | 手順 4-4 で洗い出し、キャッシュ以外はすべて載せる(R11) |
| 作り直せない値を「キャッシュ」として載せずに済ませる | 読み込み後に番号や状態が保存前と食い違う | 手順 4-4(作り直すと近似を超えて違う値になるものはキャッシュではない) |
| 洗い出しが所有者の直下で止まり、深い所の値を取りこぼす | 載っていない値が残ったまま、R11 を満たしたことになる | 手順 4-4(往復の検査で視点を固定し、進行は段 5 で表示担当を外すときに実体のフィールドを見直す) |
