...
| c | 予測パネルの状態(目盛りの表示を含む) | |
| d | ビュー | |
| e | カメラ | |
| f | 実体ごとの表示設定(軌道線のトグル、タンパク質の表示) | |
| g | `lastFocusId` の除去と、視点の書き込みの呼び元の洗い直し(`/inv-callstack`) | |
...
- **実体ごとの表示設定。** 軌道線のトグルは視点が実体の同一性を鍵に持つ(`WeakSet` など)。メニューのトグル操作は実体の `runMenu` へ届けず、表示の導出が視点の命令にする。**タンパク質の表示は、いまの UI では全個体と生成時の選択が常に同じ値に揃うので、視点の1つの値にまとめる**(実体の `displaySettings` と `ManualSpawn.display` を消す)。保存形式の各個体の `display` は、セーブを組むときに合成する。読み込みでは保存された最初のタンパク質敵の `display` から戻す(4-4 で専用の省略可能項目を足す)。
