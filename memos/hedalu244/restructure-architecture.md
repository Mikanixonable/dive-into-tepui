...
**段 1・段 2 は main へ入っている(PR #80・#81)。段 3 は PR #84 で送った。段 4 はコードが 4-3 まで済(このブランチ)で、残りは規則を段の範囲へ戻す 4-4 と、送る 4-5 だけ。その後が段 5〜8。**
...
調査時点: 当初の洗い出しは `workspace4` @ `3ee0aeb3`。**文書中の `path:line` は、`restructure-architecture` @ `57fba6fa`(段 3 の終わり)で引き直してある。ただし K8 と手順 5-1〜5-6 の `path:line` は `65146de8`(4-3 の終わり)で引いた。**
...
`rg -c "暫定 — 段" DEVELOP/CODING-RULE.md` が段ごとに変わり(段 1 で 3、段 2 で 4、段 3 で 7、段 4 で 6、段 5 で 4、段 6 で 6、段 7 で 4)、段 8 で 0 になる。**いまは 5**(4-1 で2つ外した)。段 4 は手順 4-4 で1つ増える — `game.ts` を段 5 まで割らないための暫定で、5-1 で位相の印と一緒に外れる。
...
- `serialize(): SerializedT` は引数を受けない。逆は `static deserialize(serialized: SerializedT, …): T` で、`serialized` の後ろに受けてよいのは、直列化に含まれない協力者(R13)と復元時刻だけ。
...
| 4 | R4 の全文、R11 の1。R1・R5・R10 の「ラン跨ぎ」を「セーブを跨いで共通」へ言い直す(以上 4-1 で済)。**4-1 で先に書いた R11 の2は、4-4 で落として段 5 へ回す**(K8-7) | — | **`game.ts` は段 5 まで分割しない**(4-4。段 5 で外す) | `game/viewer/` への片方向、`settings/` と `game/viewer/` の相互 import、セーブの型から表示の導出への import(以上 4-2 で済。落とす判定は無い) |
...
**段の切り方は「規則が完結する単位」で決めた。** R7 は装置3つ(`render/`・`marker/`・`audio/`)が揃って宣言的になるまで書けないので、段 1 はその3つを含む。R6 は表示担当が族ごとに揃うまで書けないので、dynamic の族(段 6)と天体の族(段 7)に分かれる。R12・R13 は段 4 に入れた — 手順 5-5 で足す直列化のコードを、新しい形で一度だけ書くためである。同じ理由で、`game.ts` の分割(5-3)も段 4 で済ませる(K8-5)。**手順を段の間で動かすなら、その手順が満たす規則も一緒に動かす。**
...
7. **繋ぎ方の作業は、段 4 に混ぜず独立した段 5 にする**(ユーザー判断 2026-09-18)。段 4 は R4(モデル層を進行と視点に分ける)と R11(セーブの規格化)の2つを載せていたが、R11 の側が上の 1〜6 で7手順に膨らんだ。**4-3 までで R4 は満たしているので、ここで段を切って main へ送る。**
   ...
   - **`game.ts` は 1040 行のまま main へ入る。** 計画の外で割られないよう、4-4 で次の3つを置く: 1.10 の位相の印の行き先を段 5 のランの組み立てへ直す、1.2 に「段 5 で3つの根へ分けるまで分割しない」暫定を置く、`game.ts` の冒頭に同じことを1行書く。段 4 の PR 本文にも書く。
...
| 4 | R4(全文)・R11 の1 | 進行のパスから `game/viewer/` への import が 0 件。`settings/` と `game/viewer/` の間の import が双方向とも 0 件。§6 が設定と言う値を `game/` が持っていない。`rg -n "例外なくすべてセーブ" DEVELOP/CODING-RULE.md` が 0 件(規則は段 5 で書き直す)。`game.ts` を段 5 まで分割しないことが、1.10 の位相の印・1.2 の暫定・`game.ts` の冒頭・PR 本文の4か所に書いてある |
...
**段 3 の終わりに残っていた暫定**(`rg -c "暫定 — 段" DEVELOP/CODING-RULE.md` が 7。段 4 の2つは 4-1 で外したので、いまは 5):
...
**この段で書く規則**: R4 の全文、R11 の1。R1・R5・R10 の設定の文面を、セーブを跨いで共通かで言い直す。その前に、何がセーブに残るかの SPEC を直す(K3-5)。

**R11 の2(モデル層の値は例外なくすべてセーブする)は、この段では書かない。** 4-1 で書いてしまったので、手順 4-4 で落として段 5 へ回す。載せる作業(旧 4-4、いまの 5-5)が段 5 へ移ったためで、規則だけが先に main へ入ると「書かれている規則はコードが満たしている」が崩れる(K8-7)。

**段の後、他の作業へ効くこと**: 遊ぶ人の選択のうち、セーブごとに持つもの(視点)は `game/viewer/` に置き、セーブを跨いで共通のもの(設定)は `settings/` に置く。どちらに置くかは `INVARIANTS.md` §6 が決め、「見え方を決める値か」では決めない。§6 が決めない値は、1つの所有者の状態を両方へ割らない側に置く。進行のコードから `game/viewer/` を import しない。視点への書き込みも、進行への書き込みと同じく命令の列を通す(設定への書き込みは、これまでどおり callback で `settings/` へ返す)。セーブはモデル層の直列化で、表示の導出と設定を含めない。**`src/game/game.ts` は段 5 で3つの根に分けるので、計画の外で割らない。**

**手順 4-0(SPEC、`659ed354`)と 4-1(規則、`d3f84718`)は済。** `rg -c "暫定 — 段" DEVELOP/CODING-RULE.md` は 5。R11 の2には、R1 が保存しないと決めた乱数の系列と、需要が残させる軌跡の履歴(`652621bf`、5-5 のユーザー判断)も「セーブしないもの」として書き足した — これも 4-4 で一緒に落とし、段 5 で書き直す。

**手順 4-2(検査、`681bf1d5`)は済。** 判定はコードが満たしているので、落とすものは無い(`npm run check:boundaries` が 18 判定・空の許可リストで 0)。計画の指定から次を変えた。

- **進行のパスから `game/save/` と `targeter.ts` を外した。** `game/save/` はモデル層(進行と視点)全体の保存形式なので、視点の型を引いてよい(R11)。`targeter.ts` は、進行の処理である的面の通過の記録を 4-3 で進行のモジュールへ出すので、残りは表示の導出になる(段 6 のファイル単位の例外へ載せる)。
- **進行のフォルダにある表示の導出を、名指しで外す一覧 `MISPLACED_PRESENTATION_FILES` を段 4 で作った。** 手順 6-2 の「ファイル単位の例外」はこの一覧を引き継ぐ。いま載っているのは `plan/{plan-editor,node-gizmo,plan-panel,plan-axis-drag,plan-display}.ts`、`creative/{object-placer-panel,slider-field,stage-controls-panel}.ts`、`stages/stage-utils/status-panel.ts`。
- **視点から出ていく辺(`hud/`・`marker/`・`three`・`render/` の値)は、段 4 では判定しない。** 生の入力と出来事の装置の禁止パターンだけを視点へ当てた。残りは段 6 のモデル層の規則が覆う。それまでは 4-3 の設計(`game/viewer/` は定義層・時刻層・進行・保存形式の型・自分自身だけを引く。`render/` の語彙の型だけは `import type` で許す)をレビューで守る。
- 検査の実装中に見つかった、段 6 で扱う辺: `creative/manual-spawn.ts`(進行)が `creative/stage-controls-panel.ts`(表示の導出)から `STAGE_CONTROL_ENEMY_SHAPES` を引いている。`plan/plan-path.ts` は計画の区間の予測弧と、描画の宣言(`three` の型)が同居している。

#### 手順 4-3. 視点を `game/viewer/` に集める

**目的**: R4 の視点を1か所に集め、進行から切り離す。対象はカメラ・ビュー・予測パネルの状態(計画軌道の座標系・表示期間・スクラブ位置ほか。K2)・軌道要素の基準・航法ターゲット・軌道ガイド・実体ごとの表示設定。カメラは「視点の状態」と「入力を視点の命令へ変える操作係(表示の導出)」に分ける。**集めた視点への書き込みは、段 3 の命令の列へ通す**(段 3 で後回しにした分。手順 4-1 で暫定を外すので、この手順の終わりに満たしていないと規則に反する)。

`settings/` へ移すものは無い。いま `game/` が持つ遊ぶ人の選択は、§6(手順 4-0 の後)と K7 に照らすと、どれも視点か操作途中の状態である。軌道分析ウィンドウの開閉は操作途中の状態として R5-4 のまま置く(K7-1)。

既存のセーブの項目は変えない。視点のうちセーブに無いものを足すのは手順 5-5。

**サブ手順と進み具合**(同じファイル群を重ねて触るので直列。1つずつ commit する)

| | 対象 | 状態 |
| --- | --- | --- |
| a | 視点の根、航法ターゲット、軌道要素の基準、操作対象の選択から視点への書き込みの除去、的面の通過の記録 | 済(`cdcd0a13`) |
| b | 軌道ガイドの選択 | 済(`5224a683`) |
| c | 予測パネルの状態(目盛りの表示を含む) | 済(`d08118b5`) |
| d | ビュー | 済(`0dc4e04a`) |
| e | カメラ | 済(`f6f9cdef`) |
| f | 実体ごとの表示設定(軌道線のトグル、タンパク質の表示) | 済(`bd7b544c`) |
| g | `lastFocusId` の除去と、視点の書き込みの呼び元の洗い直し(`/inv-callstack`) | 済(`9204a172`)。`src/`・`tests/` を走査し、視点の所有者の書き込みを命令の口の実装・視点の内部・`game.ts`・テストの外から呼ぶ箇所は 0 件。所有者のクラス型を受ける `game/viewer/` 外のモジュールは `game.ts` だけ |

**4-3 のコードは済。** `npm run typecheck`・`npm run check:boundaries`・`test:game`・`test:render` は通っている。**達成条件のうち `npm run dev` での操作の確認と、段 1 の前に書き出したセーブの読み込みは未実施。**

**4-3 で決めた設計**(2026-09-17。覆すなら該当するサブ手順をやり直す)
...
| `src/game/display-window-manager.ts:90-98,112,157-206` | 予測パネルの状態(`:90-98` の各フィールド)を視点へ移す。座標系を選び直す規則(`followCameraFocus`・`dropStaleRotatingFrame`、`:194-206`)も視点に置く。`PredictPanel` の所有(`:120-153`、`:256`)と、状態から表示窓を組む `resolve` は表示の導出に残す |
| `src/game/hud/panels/predict-panel.ts:368-373` | 「目盛りを表示」は正本を持たず DOM の `hidden` を直に書いている。視点の値にして、パネルは sync でそれを当てる |
| `src/game/game.ts:165,247,290-293,715`、`src/game/celestial/orbit-guide/orbit-guide-settings.ts` | 軌道ガイドの選択(`962db128` でセーブへ移った)を視点へ移し、書き込みを命令の列へ通す。型・既定値・丸め・セーブからの読み直しは視点の語彙として `game/viewer/` へ移す。群の語彙(`GUIDE_GROUPS`・`GuideGroupId`)は群タブの設定(`hud/hud-selection.ts`)とも共有するので、import を持たないモジュールに分ける |
...
| `src/game/hud/hud.ts:62,96-106` | 軌道分析ウィンドウの開閉は視点へ移さない。何も指さず、ほかにも影響しない操作途中の状態なので、R5-4 のまま置く(K2・K7-1) |
...
| `src/game/save/save-data.ts:245-293`、`src/game/game.ts:235-248` | 既存の項目は変えない。組み立てと分解を所有者ごとに分け、視点の分は `game/viewer/` の根が1つにまとめて返す |
| 視点の所有者と、それを DOM のハンドラ・入力の解釈から直に呼んでいる箇所 | **段 3 で列へ通さなかった書き込みを、命令の口にして `CommandQueue` へ積む。** 口は段 3 と同じく所有者の隣に置く(`*-commands.ts`)。段 3 の時点では、視点の所有者が5つのモジュールに散らばり、注視の差し替えだけでも呼び元が4モジュールあった(`508c70d7`)。集めた後の所有者について、`/inv-callstack` で呼び元を洗い直す。メニューの実行(`object-windows.ts` の `runAct`)が即時に済ませている `focus`(注視)と `target`(航法ターゲット)も、ここで列へ通す。予測パネルの操作(`display-window-manager.ts:128-153` のハンドラ)も同じ |

**達成条件と検証**

- 進行のパスから `game/viewer/` への import と、`settings/` と `game/viewer/` の間の import が、`npm run check:boundaries` で 0 件になる。
- 視点の所有者の命令を、命令の口の実装の外で DOM のハンドラから直接呼ぶ箇所が 0 件になる(`/inv-callstack` で確かめる)。
- DOM のハンドラから来る視点の操作(メニュー・一覧・パネルのボタン)は、次の進行の位相で反映されるので最大1フレーム遅れる。入力の解釈の位相で積む操作(キー・ホイール・ドラッグ)は同じフレームで適用されるので遅れない。`npm run dev` で、注視の差し替え・ビュー切替・航法ターゲット・軌道ガイド・表示期間とスライダーの操作に破綻が無いことを見る。スライダーのドラッグが1フレーム遅れて見えないこと。
- `rg -n "trajectoryLineVisible|displaySettings" src/game/dynamic` が 0 件。`Game` が軌道ガイドの選択をフィールドに持たない。
- `npm run typecheck`、`npm run test:game`(`camera-orientation`・`focus-target`・`frame-anchors`・`map-visibility`・`orbit-guide-settings`)。
- 段 1 に着手する前に書き出したセーブファイルを読み込み、次が保存どおりに戻ることを `npm run dev` で見る。
  - 操作対象・航法ターゲット・カメラの視点・ビュー
  - 敵の軌道線の表示
  - タンパク質の表示

#### 手順 4-4. 規則を段 4 の範囲へ戻し、`game.ts` を段 5 まで守る

**目的**: 段 4 をここで閉じられるようにする。4-1 で先に書いた R11 の2はコードが満たしていないので落とし、段の番号が1つずつ繰り下がったことを暫定の印へ反映する。あわせて、**段 5 で3つの根に分ける `game.ts` を、それまで計画の外で割られないようにする。** コードの挙動は変えない。

**変更が必要な箇所**

| ファイル | 何をするか |
| --- | --- |
| `DEVELOP/CODING-RULE.md` の R11 | 2つ目の項(「モデル層の値は、例外なくすべてセーブする」と、セーブしない3つの例外)を落とす。残すのは1つ目(スナップショットはモデル層の直列化で、表示の導出と設定を含めない)だけ。落とした文面は手順 5-1 で書き直す |
| 同 — 暫定の段番号 | 段が繰り下がったので、`(暫定 — 段 5 で…)` を `段 6` へ(`game/` の中を割らない・モデル層の禁止はパスで当てる の2か所)、`(暫定 — 段 7 で…)` を `段 8` へ(`hud/` の扱い・表示の導出の置き場 の2か所)直す。残る1つ(位相の順序)は次の行 |
| 同 1.10 の位相の印 | 「位相の順序を組み立てるのは `src/game/game.ts` である。(暫定 — 段 7 で `main.ts` へ移す)」を、**「(暫定 — 段 5 でランの組み立て `src/run/` へ移す)」** に直す(K8-5 で行き先が変わった) |
| 同 1.2 — 置く暫定 | **`src/game/game.ts` は、段 5 でモデル層の根・表示の導出の根・ランの組み立ての3つへ分ける。それまで 500 行を超えたままにし、計画の外で分割しない。**(暫定 — 段 5 で外す)。1.2「長いモジュールは、まず原因を診断する」の隣に置く |
| `src/game/game.ts` の冒頭 | 同じことをコメントで1行書く(分割の予定と、外で割らないこと) |
| `tools/check-boundaries.mjs` | コメントの段番号を繰り下げる(`段 5` → `段 6` が 6 か所、`段 7` → `段 8` が 2 か所。`段 2`・`段 3` は済んだ段なのでそのまま) |

**達成条件と検証**

- `rg -n "例外なくすべてセーブ" DEVELOP/CODING-RULE.md` が 0 件。R11 の見出しと1つ目の項は残っている。
- `rg -c "暫定 — 段" DEVELOP/CODING-RULE.md` が 6(段 5 が 2 — 位相の順序と `game.ts` の分割、段 6 が 2、段 8 が 2)。
- `rg -n "段 5" DEVELOP/CODING-RULE.md src/game/game.ts` が、位相の印・500 行の例外・`game.ts` のコメントを指している。
- `npm run check:boundaries` が空の許可リストで 0。`npm run typecheck`。

#### 手順 4-5. 段 4 を main へ送る

**手順と達成条件**: 「段を main へ送る(共通)」のとおり。4-3 で積み残した確認(`npm run dev` での操作と、段 1 に着手する前のセーブの読み込み)は、ここで必ず通す。PR 本文には次を書く。

- 手順 4-0 の SPEC の変更(§6 に時間を足したこと、SAVE.md で進行を例外なく保存すると書いたこと)。**SPEC はコードより先行してよいので、この文面は残る**(R11 の2を規則から落としたことと矛盾しない)。
- R4 の全文と R11 の1。言い直した R1・R5・R10 の文面。
- 視点を `game/viewer/` へ集めたこと、視点への書き込みが命令の列を通るようになったこと。
- **次の段(段 5)の予告**: `game.ts` を3つの根に分けること、直列化を `serialize`/`deserialize` へ揃えること、セーブの版を上げて形式を作り直すこと。**`game.ts` を計画の外で割らないでほしいこと。**
- SPEC に書かず、いまの挙動のまま残したもの(K7-1 の操作途中の状態、K7-3 の画角と投影の置き場)。
...
作業量は次の式で出す。**段 1〜3 は実施済みなので、残り(段 4〜8)だけを載せる。** 段 3 から回した項目の分は、受ける手順の行に足して括弧で示した。
...
| | ~~4-2 検査~~ 済 | 0 |
| | ~~4-3 視点を集める~~ 済 | 0 |
| | 4-4 規則を段 4 の範囲へ戻す(R11 の2を落とす 15、暫定の段番号 5 か所と検査のコメント 7 か所 20、`game.ts` を守る暫定とコメント 15、検証 20) | 70 |
| | 4-5 main へ送る(4-3 で積み残した `npm run dev` の確認とセーブの読み込みを含む) | 150 |
| | **段 4 計(残り)** | **220** |
...
| | 8-7 main へ送る | 120 |
| | **段 8 計** | **1,883** |
| | **残りの合計** | **9,023 分 ≒ 150 時間** |

- 残りの commit は約 31 本(段 4 が 2 本、段 5 が 14 本 — 5-1 が2本、5-4 が群ごとに7本)、PR は 5 本。**段の中で並行してよい組は、段 4 以降には無い**(どの手順も同じファイル群を重ねて触る。5-4 の群も、実体の基底クラスと `Game` の復元経路を重ねて触るので直列にする)。
- **段 4 はここで閉じる。** 残り 220 分で main へ送れる(K8-7)。繋ぎ方の約 3,300 分は段 5 が持つ。
