...
1. **鏡面の近似の限界を実測する**(§1-4)。鏡面照度バッファは材質の F0 を持てないので
   ...
   **Phase 4 の手順 0 で 2 経路を並べたところ、差の正体は F0 近似ではなくマテリアルパスの
   BRDF 落ち(1/π と (1−金属度))だった。** それを直したあと `leo` の 2 経路は画素値で
   一致している(外殻 123.1 / 123.1)ので、**F0=1 近似の誤差は少なくともこの構図では
   検出限界以下。** 金属度の高い面を大きく写す構図で測り直すのが残り。
