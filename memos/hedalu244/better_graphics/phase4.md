...
**実施結果**: リムの符号は**旧の BackSide シェルに合わせた**(奥側の交点で、カメラを向く側の
法線で測る)。外向き法線で測ると昼側の縁が光るが、旧の絵とは別物になる — 旧は逆光の縁光り。
もやは `a = haze·sunFactor`・内部散乱に `sunFactor` を掛ける形で、旧が全体へ掛けていた
昼夜係数を引き継いだ。`a = haze` だけにすると内部散乱が低太陽高度で 2.4 倍明るくなる。
`earth` ケースの分割前後の平均差は 4.4/255 で、残りは標準 BRDF が足す誘電体の鏡面反射。
...
**実施結果**: `AMBIENT_INTENSITY` は据え置き(0.79)で夜側が一致した — 統合前の
`NIGHT_AMBIENT`(0.04)と `AMBIENT_INTENSITY / SUN_INTENSITY × 環境光の色` が元から同じ値
 だったため。天体の直書き色へ掛ける換算は **`1 / SUN_INTENSITY` ではなく `π / SUN_INTENSITY`**
(Lambert の 1/π がマテリアルパスへ入ったため)。実測は月距離の球の昼側 154→159、夜側 36→35
(sRGB 8bit)。昼側がわずかに暖色へ寄るのは、恒星の色が掛かるようになったため。
...
- `[F3]` で遮蔽・大気パスの GPU 時間を読み、見積り(遮蔽 0.3〜0.8ms / 大気 0.2〜0.5ms)と
  比べて外れていたら書き戻す。
...
| **`camera.position === (0,0,0)` の前提が将来崩れる** | **解消済み。** 圧縮とともに `view-compression.ts` が消え、画面空間パス(遮蔽・大気)も view→描画座標の行列からカメラ位置を引く形にしてある。星殻・天球グリッドだけは原点直書きで、これは「カメラ追従の殻」という仕様そのもの | — |
...
| **受け手が乗っている天体自身を遮蔽器に数える** | **実施の結果、無害だった。** 真球では「中心が太陽と逆側」の判定と N·L = 0 が厳密に同じ面で切り替わるので、遮蔽が 1 を下回るのは既に N·L = 0 の領域だけ。扁平な天体でだけ、法線が動径からずれるぶんの僅かなずれが残る | 手順 8。**除外は要らない** |
...
| **大気を先に合成することで軌道線との重なりが変わる** | **変わった。** 大気パスはマテリアルパスと world パスの間にあるので、**軌道線・軌跡線・ビルボードは必ず大気の手前**になる | 実機で地球の縁を横切る軌道線を見る |
