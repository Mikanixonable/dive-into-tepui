...
| | 手順 | 独立に commit できるか |
|---|---|---|
| **前** | 0。Phase 3.5 の残件 — ライトプリパスとフォワードの明るさの食い違いを詰め、原因がライトプリパス側なら直す | 単独。**4-A の前に片付ける** |
...
| **後** | 12。手順 0 が (b) だったときだけ — 色の入力値で元の絵へ寄せる | 単独。**4-B の後** |
...
**深度分解能は導出と一致した。** 非反転 24bit・near=2。前面の球が赤、後面が青。

| ケース | 距離 z | 導出 ε_min | 実測 |
|---|---|---|---|
| `depth-1e4` | 1e4 | 3.0e-4 | ε=1e-3 は前面が保たれる(輪郭付近だけ僅かに斑)。1e-4 と 1e-5 は全面が斑。1e-6 は後面が優勢。**1e-7 は斑にならず全面が後面** |
...
後面(青)が全面を取った状態(深度が等しいと `LessEqual` で後から描いた方が勝つ)。
**達成目標 4-A-5 の「澄む」は前者のことで、色で読み分ける。**

**2 経路の差**: 球・線・背景は完全に一致する(`leo-diff.png` のその領域が真っ黒)。
**艦だけが全面で飽和し、ライトプリパス側が明るい。** → **手順 0。**

### この環境での検証の代替策

**この開発環境のヘッドレス Chrome は 1〜2fps しか出ない**(実測: 起動・HUD・コンソールとも
正常なまま、30 秒間の rAF が 40 回)。
...
### 手順 0.【Phase 3.5 の残件】ライトプリパスとフォワードの明るさの食い違いを詰める

**目的**: `leo` の `diff` で**艦の全面が飽和している**(増幅 8 倍で 255 = 生の差が 32/255 以上)。
**ライトプリパス側が明るい。どちらが物理的に正しいのかを決める。**
Phase 4 は明るさの統合(決めたこと (3)、達成目標 4-B-17)を含むので、基準がぶれたまま先へ進めない。

**目的は 2 経路の完全一致ではない。求めるのは物理的な正規化である。**
**絵を一致させるためにパイプラインの PBR 性を歪めるのは、このフェーズの目的に反する。**

**予想が外れている点から始める**: 1/π 正規化を落としていれば**暗くなる**方向のはずで、
ライトプリパスが**明るい**のは逆を向いている。**ここを説明できるまで直さない。**

| 疑い | 確かめ方 |
|---|---|
| 拡散項の 1/π 正規化 | 照度バッファへ畳むときと `NodeMaterial` の既定とで、どちらに 1/π が入っているか |
| 環境光の二重計上 | マテリアルパスが `SunLight` の環境光を掛けたうえで、シーンの `AmbientLight` も効いていないか |
| 光の単位 | `DirectionalLight.intensity` と `SunLight` の `intensity` が同じ量を指しているか(three の物理単位と legacy lights の扱い) |

**分岐**

- **(a) 原因がライトプリパス側にある** → **この手順で直す。** 直した結果として
  `leo-diff.png` の艦は輪郭とハイライトだけになる。
- **(b) ライトプリパス側が物理的に正しい**(フォワード側が three の都合で暗い) →
  **実装は直さない。** 色の辻褄合わせは**手順 12** へ送る。

**達成条件と検証**

- **(a)/(b) のどちらなのかを判定し、根拠(式)をこの文書へ書いてある。**
- (a) なら `.render-lab/shots/leo-diff.png` の艦が輪郭とハイライトだけになる。
- (b) なら手順 12 の入力(何がどれだけ変わるのか)が書けている。
...
### 手順 12.【手順 0 が (b) だったときだけ】色の入力値で元の絵へ寄せる

**目的**: 正規化を正したことで変わった明るさ・色を、**アルベド・モデルのマテリアル・光源の値**の
側で元の見え方へ寄せる。**動かしてよいのは入力の値だけ** — パイプラインを歪めて絵を合わせるのは、
手順 0 で禁じたことそのものである。

**比較の取り方**(この器の制約から来る手順): render-lab で前後を並べても、**値を動かした時点で
「変更前」の絵も一緒に変わるので比較にならない。** 値を触る前に、**元のバージョンで撮った PNG を
一時ファイルへ取っておき、それと差分を取る。**

```
git stash                                  # あるいは元の commit を checkout
npm run render-lab:shot
cp -r .render-lab/shots "<一時ディレクトリ>/before"   # リポジトリへは入れない
git stash pop
# 値を調整 → npm run render-lab:shot → before と見比べる
```

**基準は「差が 0」ではない。元の絵の印象へ戻ったかで見る。**
**正規化を戻す方向の調整はしない。**
...
| ファイル | 何を |
|---|---|
| `src/render/ships.ts` | モデルのマテリアル(色・金属度・粗さ)。**焼き込み元はここ** — 直したら `npm run export-assets` で `src/assets/models/*.json` を焼き直す(識別子だけの差分は commit しない) |
| `src/game/const.ts` | 光源の値(`SUN_INTENSITY` / `AMBIENT_INTENSITY` / `COLOR_SUN`) |
| 天体の直書き色 | 決めたこと (3) の `1 / SUN_INTENSITY` を掛ける操作と同じ場所 |

**`src/render/pipeline/` は触らない。**

**達成条件と検証**

- `git diff --stat src/render/pipeline/` が 0 件。
- 一時ディレクトリの before と撮り直した after を並べ、**艦・天体の昼側と夜側が元の印象へ戻っている。**
- `npm run render-lab:shot`
- `npm run typecheck`
...
**手順 0 と 12 はこの見積りの外。** 0 は原因究明が本体で、直すとしても数行。12 は値の調整だけで行数は動かない。
...
| リスク | 影響 | 露見する場所 |
|---|---|---|
| **絵を合わせるためにパイプラインを歪める** | 2 経路の完全一致は目的ではない。歪めたぶんは Phase 5(アルベドの物理量化)以降でそのまま負債になる | 手順 0。**直す前に「どちらが物理的に正しいか」を式で書けているか** |
| **調整の比較対象を撮り忘れる** | 手順 12 で値を動かすと render-lab の「変更前」も一緒に変わる。**元のバージョンの PNG が無いと、戻ったかどうかを判定する術が無い** | 手順 12。値を触る前に before を一時ディレクトリへ撮ってあるか |
