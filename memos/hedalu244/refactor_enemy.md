...
**目的.** 手順 3〜4 でコメントが大量に移動する。移動先で意味が通らなくなったものと、分割によって
不要になったもの(委譲を説明していたコメント)を落とし、非自明な理由が消えていないかを見る。
...
| ファイル | 何をするか |
| --- | --- |
| `src/game/dynamic/dynamic-entity/enemy.ts` | `EnemyClass` の各メンバーに、なぜ静的側で宣言するのかを書く。コンストラクタ本文(39行)へ文脈コメントを入れる |
| `src/game/dynamic/dynamic-entity/metal-enemy.ts` | コンストラクタの呼出規約を書く。慣性の非対称(ジャニベコフ効果)の説明を残す |
| `src/game/dynamic/dynamic-entity/protein-enemy.ts` | コンストラクタの呼出規約を書く。`initDefaultParts` の無効化と HP の正本が `combat` である理由は**残す**(非自明) |
