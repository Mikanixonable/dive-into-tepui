...
### 手順 1. CODING-RULE にモジュール粒度の診断と、多態の復元の形を書く

**目的.** 今回直す 2 つの逸脱が、規約から読み取れない状態になっている。1.2 は
「1箇所からしか参照されていない小さな関数を…別モジュールへ置くこと」を既に禁じているが、
**長いモジュールに出会ったとき何を確かめるべきか**が書かれていない。「長いから切り出す」も
「長くても切り出すな」も、多態にするかどうかも、原因の診断なしには決まらない。
多態にしたものをどう保存・復元するかも、どこにも書かれていない。コードを触る前に規約を先に直す。

**変更が必要な箇所.**

| ファイル | 何をするか |
| --- | --- |
| `DEVELOP/CODING-RULE.md` 1.2「モジュール化」(26〜47 行) | 悪いモジュール化の箇条書きへ「短すぎるモジュールが多数あること」を追加し、その下に小節「長いモジュールは、まず原因を診断する」を新設する(下記の文面)。**多態にすべきかどうかの判断はこの小節に置く** |
| `DEVELOP/CODING-RULE.md` 1.6「データ構造」の末尾(213 行、1.7 の見出しの手前) | 小節「多態を保存し、復元する」を追加する(下記の文面) |

**節番号は動かさない。** 新しい `##` 見出しを作らないので、1.7 以降の繰り下げと相互参照の
追随は発生しない。

1.2 へ足す小節の文面(要旨。清書は実施時):

> ### 長いモジュールは、まず原因を診断する
>
> **モジュールが長いこと自体は違反ではなく、責務が複数あることの徴候である。** 直す前に、
> 何が同居しているのかを診断する。**診断結果によって、正しい直し方が変わる。**
>
> - **独立した意味を持つ手続きが直に書かれている** → その手続きをモジュールとして切り出す。
>   切り出した先が**それ自身の名前で呼べる責務**を持つなら、これが正解である。
> - **1つの `kind` やフラグで、多くのメソッドの大部分が切り替わっている** → 多態で分ける。
>   判断はフィールドの有無ではなく、**その値を固定したときに通らなくなる行がどれだけあるか**で
>   下す。ある値のとき読み飛ばす行が各メソッドに散らばっているなら、それらは別の型である。
>   このとき、**分岐の片側だけを別ファイルへ切り出しても分岐は元の場所に残る**ので、何も解決しない。
> - **`kind` はあるが、切り替わるのが定数・アセット・初期値だけである** → **そのままでよい。**
>   どの値に固定しても通らなくなる行が無いなら、振る舞いは1つである。振る舞いが同じものを型で
>   分けると、責務の無いクラスが増える。データの差はフィールドで持つ。
> - **同じ関心の実装が単に多い** → 長いままでよい。分ける線が無いところに線を引かない。
>
> **`kind` フィールドがあること自体は違反ではない。** 外部形式(セーブデータ・アセット・通信)の
> タグとしては必要であり、振る舞いを伴わない区別にも使える。禁じるのは、**多態で書けるものを
> 分岐で手展開すること**である。天体の分類のように、その種類で閉じていることが主張そのもので
> あり網羅性検査が働くほうが正しい `switch` も、そのまま書いてよい(1.8 参照)。
>
> **診断せずに「長いから」というだけの理由で小さな関数を外へ出してはならない。** 数行の関数を
> 外に出しても、呼び出し側には委譲するメソッドと `import` 行が残り、行数はほとんど減らない。
> 減らないうえに、読むべきファイルが1本増える。
>
> **短すぎるモジュールが多数あることは、長すぎるモジュールと同格の違反である。** 500 行の基準は
> 上限であって目標ではない。行数の上限だけを警戒していると、責務の無い 20 行のファイルが並ぶ
> 状態に行き着く — こちらのほうが、どこに何が書いてあるかを追えなくなるぶん質が悪い。
> ファイル名から責務が言えないモジュール、`import` した先で1回しか呼ばれない関数だけを持つ
> モジュールは、呼び出し元へインライン展開して消す。
...
>
> - **静的側インターフェース。** 具象が自分のタグと構築シグネチャを静的に宣言する。基底は
>   `this.constructor` から自分の静的側を引く getter を持つ。
> - **クラス辞書は独立したモジュールに置く。** 基底のモジュールに置くと基底 → 具象 → 基底の
>   実行時循環になり、`extends` の評価時に落ちる。呼び出し元に `switch` を直書きするのも、
>   分岐を呼び出し元へ戻すだけなので不可。
> - **復元のコンストラクタ・シグネチャを全具象で揃える。** 具象ごとに違ってよいのは新規生成の
>   引数だけで、復元の腕(保存データ + 復元時刻)は共通にする。

**達成条件と検証.**

- `npm run typecheck`(文書のみだが、常に走らせる)。
- `DEVELOP/CODING-RULE.md` に上記 2 小節が入っている。
- `grep -c '^## 1\.' DEVELOP/CODING-RULE.md` が **13**(改定前と同じ)。新しい `##` 見出しを
  作っていないこと。
- `grep -c '^### ' DEVELOP/CODING-RULE.md` が **26**(改定前は 24)。
