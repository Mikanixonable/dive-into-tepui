...
1. 「窓を回して最小 TOI の1体を選ぶ」ループが `src/` に**1本だけ**である
   (`firstSurfaceContact` の中だけ)。
1b. **`attractor.ts` が接触に答えていない** — 掃引にも接触の幾何にも依存せず、
   `sphere-contact` / `collision-response` を import していない(`CODING-RULE 1.3`)。
2. `reachedBody` の綴りが `src/` `tests/` `tools/` に **0 件**。
3. 弧と実体の半径和が同じ式である — `firstSurfaceContact` の引数 `radius` に
   `GameEntity.radius` と `PredictedArc` の `radius` が入り、両者に別々の分岐が無い。
4. `resolveFixedSphereCollision` の綴りが 0 件で、掃引を2度掛ける経路が無い
   (`firstSurfaceContact` が返した `geometry` をそのまま `distributeFixedContact` へ渡す)。
5. `npm run typecheck` と `npm run test:physics` が通る。
6. **`tests/physics/attractor.test.ts` に、個体の半径ぶんだけ表面を跨ぐ配置で
   `firstSurfaceContact` が到達を返すテストがある** — 半径和が本当に効いていることを、
   天体半径だけの実装では落ちる形で固定する。
7. `?stage=debug-load` を ×131072 で 60 秒回して例外 0 件、自機と破片が残る
   (第9章の駆動手順)。

## 5-5. 手順

**全ステップ実施済み。** commit は `cc168a43`（SPEC）→ `145bfc45`（分配の切り出し）
→ `5297db89`（窓口の新設）→ `b6d00b59`（実体側の繋ぎ替え）→ `70c2fdd0`（弧側の繋ぎ替え）
→ `b51a0b75`（旧実装の削除）の 6 本。
...
| リスク | 影響 | それが露見する場所 |
|---|---|---|
| 表面ちょうどに置かれた個体が、半径和ちょうどの距離になり `startsInside` が真になる | 着地した艦の弧が毎フレーム打ち切られ、予測線が消えて点になる。**押し戻しは中心間距離を半径和ちょうどへ揃えるので、この状態は必ず起きる** | 第8章の実測で、静止した個体が表面へ収束したあと弧を伸ばせるか。着陸を実装するときの必須条件 |
| `sphereContactGeometry` の重なりフォールバックが弧にも効くようになる | いまの `reachedBody` は「跨ぎが無く始点で沈んでいる」ときだけ `prev` を返すが、新しい形では「区間終端で重なっている」ときも到達を返す。**弧が打ち切られる条件が広がる** | 表面すれすれを通過する弧が、いままで伸びていたのに打ち切られる。`attractor.test.ts` の「表面に触れない近傍通過は検出しない」で当たる |
| `alreadyInside` の到達状態が `prev` から `next` へ変わる | 天体の内側から始まった弧の到達マーカーが、区間の始点ではなく終点に出る。どちらも天体の内側なので見た目の差は無いが、テストの期待値は変わる | `attractor.test.ts`「開始時点で既に内部にいる場合もその天体を返す」 |
| `distributeFixedContact` を切り出したとき、`separation` 枝と `pushOut` 枝の場合分けを落とす | 掃引で解けた接触が押し戻し扱いになり、位置が半径和ちょうどに揃わない。**静かに間違う** | `collision-response.test.ts` の `resolveFixedSphereCollision` 由来の2件。切り出し前に、この2件が両方の枝を通っているか確かめてから割る |
| `PredictedArc` の `radius` を渡し忘れた呼び出しが 0 のまま通る | 弧だけが点判定に戻り、統一した意味が消える。**型が通るので気付かない** | 引数を省略可能にしないこと。既定値を持たせないので `typecheck` が落ちる |
| exp12 と `surface-candidates.test.ts` が `reachedBody` を「絞り込みの正しさの正本」として使っている | 置き換えを間違えると、絞り込みの回帰テストが何も守らなくなる | `chordDeviationBound` を 0 に潰したときに `surface-candidates.test.ts` の2件が落ちることを、置き換え後にもう一度確かめる |
