...
| 3 | 第7章(論点3) | SPEC から精度と性能を追い出し、そのうえで2つの実測を通す | 未着手 |

**第7章は SPEC の掃除(7-1)が先で、実測(7-2 / 7-3)は後。** 「性能のために判定を抜く」
「性能のために刻みを変える」は実装の裁量であって確定仕様ではない、と決めたので、消してよいか
どうかは実測の結論に依らない。実測が決めるのはコードの側だけになる。
...
1. `SPEC/README.md` の当該箇所を**「精度と性能は仕様化しない」**へ広げる。書いてよいのは
   ...
   実装の裁量である、と明記する。
2. その規則で `SPEC/` 全体を掃く。**着手時点(`572d6343`)で当たっている箇所**:

| ファイル | 箇所 | どうするか |
|---|---|---|
| `ORBIT.md`「時間刻みとタイムワープ」 | 「各サブステップの重力環境は…中間時刻において一度だけ評価し」 | 手段を削除。ただし**同じ区間内で相互作用するものは必ず同じ瞬間の位置関係で計算される**という不変条件は残す(これは精度ではなく整合性) |
| 同節 | 「サブステップの幅にも本数にも上限がある…計算コストが時間加速倍率に際限なく比例することはない」 | 削除。観測できる帰結(「倍率をどれだけ上げてもフレームが止まらない」)だけを残す |
| 同節 | 再突入域の細分化の3項(境界高度 200 km、基準面、対象を艦に限る理由と、細分化が上限に優先すること) | **手段なので削除。** 帰結「再突入した艦が失われるかどうかと、その文言は倍率に依らない」は `FLIGHT.md` が既に持つので、そちらを正とする |
| 同節 | 「**物体どうしの**剛体接触の解決は、時間加速倍率が一定以上になると行われない…接触判定に意味がなくなるため」 | 削除。「未確定の案」へ移す |
| `ORBIT.md`「物体どうしの接触」 | 「時間加速倍率が一定以上になると解決されなくなる…意味がなくなる」 | 削除 |
| `ORBIT.md`「数値的な限界」 | 「1フレームに刻めるサブステップの本数には上限があり…数値誤差由来の軌道減衰が生じる」 | 手段(本数の上限)を落とし、帰結(最高倍率の地球低軌道では数値誤差由来の減衰が生じる)だけ残す |
| `ORBIT.md`「未確定の案」 | — | **1件足す**: 物体どうしの剛体接触を高い倍率で解かないこと。費用のために解決を止めており、その倍率では物体どうしがすり抜ける |
| `GAME.md`「4. 時間加速」 | 「物体どうしの剛体接触(衝突)の解決もワープ×4以下でのみ働く」 / 「再突入域…の細分化はこの上限に優先する」 | どちらも削除。同じ行にある「天体の表面への接触はどの倍率でも解決され、…天体をすり抜けない」と「艦が焼ける時期が早まってはならない」は**残す** |
| `CONTROLS.md` キー表の後 | 「**物体どうしの**剛体接触は、タイムワープが×4を超えると働かなくなる」 / 「高度200 km以下…細分化が優先される」 | 同上。**推進・射撃・敵の射撃の×4は残す** — 操作の仕様であって性能の都合ではない |
| `FLIGHT.md`「弾薬ベルト」 | 「いずれの接触解決も、タイムワープが物理演算の上限(×4)を超えると行われなくなる」 | 削除。ベルトだけ実時間の1フレームごとに解く、という区別は**残す** |
| `COMBAT.md`「薬莢」 | 「**物体どうしの**剛体接触判定は、推進・射撃と同じくタイムワープ×4以下でのみ働く」 | 削除 |
| `AUDIO.md`「被弾・衝突・爆発」 | 「時間加速が高い段階まで上がっている間はその判定が働かなくなり、金属音も鳴らなくなる」 | 削除。「艦体と薬莢が接触すると金属音が鳴る」だけ残す |
...
3. `SPEC/ORBIT.md`「未確定の案」の「高いタイムワープ中に再突入する艦を『待たない』こと」は
   **残す** — 7-3 の実測で決まる。手段(細分化が本数の上限に優先すること)がこの節に書かれて
   いるのは規則違反ではない。未確定の案は確定仕様ではなく、判断のための材料だからである。

### 達成目標

1. `SPEC/README.md` の規則が「精度と性能は仕様化しない」の形になっている。
2. 上の表の各行が処理済みで、`SPEC/` の本文に**手段としての刻み幅・省略・計算量の記述が
   残っていない**。
3. 削除した記述が語っていた**観測できる帰結**が、どこかに残っている
   (削除ではなく置き換えになっている行を、1行ずつ突き合わせる)。
4. 「高い倍率では物体どうしがすり抜ける」が本文のどこにも無く、「未確定の案」に1件だけある。
...
- **セル一辺が倍率でほとんど伸びず、費用がフレーム時間の数 % に収まり、かつ乖離が
  ...
  `MAX_PHYS_SIM_SPEED` は自機の操作の可否だけを支配するようになり、7-1 で「未確定の案」へ
  移した項も消える。
- **どれか1つでも外れるなら、ゲートは残す。** その場合も SPEC は 7-1 のままで、ゲートは
  「未確定の案」に置かれたままになる — **残す判断もまた性能の判断であって、確定仕様ではない。**
  実測の文言は memos/ とこの文書に残す。
...
- **7-1**: SPEC 8 ファイル、30〜40 行の書き換え。
- **exp13**(7-2): exp12 の骨格(暦・積分・タイムライン生成)を流用できるので、新規は
  母集団の組み立てと narrow phase の計測で 150 行ほど。実行は exp12 と同じ桁(数分)。
- **exp14**(7-3): 熱と動圧の式の複製が 30 行、掃引が 4 通り × 数分。
- **是正の実装**(7-3 の本命案を採る場合): 熱・動圧の内側細分は `stepEnvironment` の中に
...
| リスク | 影響 | それが露見する場所 |
|---|---|---|
| SPEC から性能の記述を消すとき、**帰結まで一緒に消す** | 「天体はすり抜けない」「艦が焼ける時期が早まらない」はプレイヤーが見る事実で、消してはいけない。ゲートの記述と同じ行・同じ節に同居している | 7-1 の達成目標 3。削除した行を1行ずつ、残した帰結と突き合わせる |
| 「未確定の案」へ移した項が、そのまま忘れられる | 高倍率ですり抜けることの是非が誰にも問われなくなる | 7-2 の実測がこの項を必ず読み直す。実測を先送りするなら、項が残っていること自体が印になる |
...
| 7-3 の本命案(熱だけ内側細分)を入れたとき、`qdyn` の点サンプルを直さない | 動圧のピークだけ見逃しが残り、`MAX_DYN_PRESSURE` の判定が刻み依存のままになる。**温度だけ直って安心する** | 内側細分のループで `qdyn` の**最大値**を取ること。終端の点値を残さない |
...
| SPEC から根拠を消しすぎて、次に触る人が同じ議論を再発明する | 「なぜ ×4 なのか」が誰にも分からなくなる | **根拠は memos/ とこの文書に残す。** SPEC に書くのは帰結だけ、という分担そのものが 7-1 の目的である |
