// 実験15: 再突入で軌道 RK4 が発散する原因の切り分けと、大気の刻み規則の較正。
// exp14 は「熱と動圧の刻み感度」を測って壊れ方に行き当たったが、壊れているのが RK4 の側で
// あることまでしか分からなかった。ここでは
//   表1: どの力が壊しているか(抗力を切ると完走するか)
//   表2: 壊れる歩で RK4 の4段が実際に何を標本しているか
...
const DIVERGED = 2e4; // 対気速さ [m/s]。これを超えたら発散とみなす(脱出速度の 1.8 倍)
...
function descend(
  ...
  return { steps, maxTemp, maxQdyn, maxSpeed, minAlt };
}

// ---- 表2: RK4 の4段が標本する点 ----

// stepDynamics と同じ力を、段ごとの高度・重力・抗力に分けて返す。
function stageProbe(eph: Ephemeris, s: KinematicState, dt: number, body: CelestialBody): string {
  const t = s.t + dt / 2;
  const attractors = eph.gravityAttractorsAt(t);
  const at = (r: Vec3, v: Vec3): { a: Vec3; alt: number; drag: number } => {
    let g = v3();
    ...
    const d = dragAccel(sub(r, body.state.r), sub(v, body.state.v), SHIP_BCINV, body.atmosphere!);
    return { a: add(g, d), alt: ellipsoidAltitude(sub(r, body.state.r), body.atmosphere!), drag: len(d) };
  };
  const h = dt / 2;
  const k1 = at(s.r, s.v);
  ...
  const k4 = at(r4, v4);
  return [k1, k2, k3, k4]
    .map((k, i) => `k${i + 1} ${(k.alt / 1e3).toFixed(1)}km/${k.drag.toExponential(1)}`)
    .join('  ');
}
...
function descentRow(label: string, r: Descent, ref: Descent | null): readonly string[] {
  ...
  return [
    ...
    r.maxSpeed > DIVERGED ? `${(r.maxSpeed / 1e3).toExponential(1)} ★発散` : (r.maxSpeed / 1e3).toFixed(2),
...
const DESCENT_HEADER = ['刻みの決め方', '歩数', '対気速さ最大 [km/s]', '外殻温度の最大 [K]',
  '動圧の最大 [kPa]', '最低高度 [km]'];
...
function forceIsolation(eph: Ephemeris): void {
  console.log('## 表1: どの力が壊しているか(413 / 55 km、刻み固定)\n');
  const rows: string[][] = [];
...
function stageSampling(eph: Ephemeris): void {
  console.log('## 表2: 壊れる歩で RK4 の4段が標本する「高度 / |抗力|」\n');
  console.log('段の位置は現在の速度と加速度からの直線外挿なので、重力だけで動径方向に g·dt²/4 沈む。'
    + '剛性が全く問題ない高度でも、段が地面の下を標本して海面密度を拾う。\n');
  const rows: string[][] = [];
  for (const dt of [WARP_STEP, 34.13, 20]) {
    let s = apsisState(413e3, 55e3);
    let worst: { label: string; probe: string; lambdaDt: number; alt: number } | null = null;
    for (let i = 0; i < 500 && isFinite(s.r.x); i++) {
      const air = eph.atmosphereCelestialBodiesAt(s.t);
      const body = nearestAtmosphereBody(s.r, air)!;
      const { alt, rho, speed } = airflow(s, body);
      if (alt <= 0) break;
      const probe = stageProbe(eph, s, dt, body);
      ...
      // 「壊れた歩」= その1歩で対気速さが発散の域へ飛んだ歩。
      if (isFinite(next.r.x) && airflow(next, body).speed > DIVERGED) {
        worst = { label: `刻み ${dt.toFixed(2)} s`, probe, lambdaDt: 0.5 * rho * speed * SHIP_BCINV * dt, alt };
        break;
      }
      s = next;
    }
    rows.push(worst === null
      ? [`刻み ${dt.toFixed(2)} s`, '—(発散せず)', '—', '—']
      : [worst.label, `${(worst.alt / 1e3).toFixed(1)} km`, worst.lambdaDt.toFixed(3), worst.probe]);
  }
  table(['刻み', '壊れた歩の高度', 'λ·dt', '各段の 高度/|抗力| [m/s²]'], rows);
}

function calibration(eph: Ephemeris): void {
  console.log('## 表3: 軌道の族での較正\n');
  console.log('「剛性だけ」「沈み込みだけ」は片方の上限を外したもの。'
