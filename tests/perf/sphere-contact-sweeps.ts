...
function body(id: string, mu: number, radius: number, atmosphere: Atmosphere | null = null): CelestialMotion {
  return fixedMotion({
...
export function freeFall(central: CelestialMotion): Advance {
  return (s, dt) => stepDynamics(s, dt, [central], 0, [], 0, null, 0, 0, null);
}

export function withDrag(central: CelestialMotion, bcInv: number): Advance {
  return (s, dt) => stepDynamics(s, dt, [central], 0, [], 0, central, bcInv, 0, null);
}

export function withThrust(central: CelestialMotion, thrust: Vec3): Advance {
  return (s, dt) => stepDynamics(s, dt, [central], 0, [], 0, null, 0, 0, thrust);
}
...
export function againstBody(
  label: string, central: CelestialMotion, a0: KinematicState, dt: number, advanceA: Advance,
): Sweep {
  return sweepOf(label, a0, kinematicState<'eci'>(a0.t, central.stateAt(0).r, v3()), dt,
    advanceA, still, central.def.radius);
}

// 表面から alt の円軌道上の状態。x 軸上に置き、y 方向へ回る。
export function circular(central: CelestialMotion, alt: number): KinematicState {
  const r = central.def.radius + alt;
  return kinematicState<'eci'>(0, v3(r, 0, 0), v3(0, Math.sqrt(central.def.mu / r), 0));
}

// 半径 r の円軌道の周期。刻みを「1周あたり何歩か」で指定するための基準。
export function circularPeriod(central: CelestialMotion, r: number): number {
  return 2 * Math.PI * Math.sqrt(r ** 3 / central.def.mu);
}

// 近点 rp・離心率 e の軌道で、区間の割合 frac の位置に近点が来るように始点を戻した状態。
export function beforePerigee(
  central: CelestialMotion, rp: number, e: number, dt: number, frac: number,
): KinematicState {
  const atPerigee = kinematicState<'eci'>(0, v3(rp, 0, 0), v3(0, Math.sqrt(central.def.mu * (1 + e) / rp), 0));
