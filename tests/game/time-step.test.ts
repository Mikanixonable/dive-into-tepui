...
function body(atmosphere: Atmosphere | null, center = v3()): CelestialMotion {
  return fixedMotion({
    id: 'b', mu: 1e6, radius: 1000, state: kinematicState<'eci'>(0, center, v3()), accel: v3(),
    degree2: null, atmosphere,
  });
}
...
export function register(): void {
  ...
  test('time-step: 大気の上限は高度に対して単調で、しきい値による飛びを持たない', () => {
    ...
    let prev = atmosphericMaxStep(descending(0), 1e-3, bodies, 0);
    for (let alt = 1; alt <= 2000; alt++) {
      const step = atmosphericMaxStep(descending(alt), 1e-3, bodies, 0);
  ...
  test('time-step: 大気の上限が刻みを 0 へ潰すことはない', () => {
    ...
    for (let alt = -500; alt <= 3000; alt += 7) {
      for (const rate of [0, 1, 100, 10000]) {
        const step = atmosphericMaxStep(descending(alt, rate), 1, bodies, 0);
  ...
  test('time-step: 抗力を受けない物体と、大気の無いところに上限は無い', () => {
    ...
    assert.equal(atmosphericMaxStep(onSurface, 0, [body(UNIT_ATMOSPHERE)], 0), Infinity);
    assert.equal(atmosphericMaxStep(onSurface, 1, [body(null)], 0), Infinity);
    assert.equal(atmosphericMaxStep(onSurface, 1, [], 0), Infinity);
  });

  test('time-step: 濃い空気にいるほど、速いほど刻みは短い', () => {
    const bodies = [body(UNIT_ATMOSPHERE)];
    const low = atmosphericMaxStep(descending(0), 1e-3, bodies, 0);
    assert.ok(low < atmosphericMaxStep(descending(1000), 1e-3, bodies, 0));
    assert.ok(low < atmosphericMaxStep(descending(0, 1), 1e-3, bodies, 0));
  });
  ...
  test('time-step: 抵抗が大きい物体ほど刻みは短い', () => {
    ...
    assert.ok(atmosphericMaxStep(level, 1e-2, bodies, 0) < atmosphericMaxStep(level, 1e-3, bodies, 0));
  });
  ...
  test('time-step: 高度は ECI 原点ではなく、その大気天体の中心から測る', () => {
    ...
    assert.equal(
      atmosphericMaxStep(inside, 1e-3, [body(UNIT_ATMOSPHERE, center)], 0),
      atmosphericMaxStep(atCenter, 1e-3, [body(UNIT_ATMOSPHERE)], 0));
  });
  ...
  test('time-step: 上昇中でも、濃い空気の中なら刻みは縮む', () => {
    ...
    assert.ok(atmosphericMaxStep(climbing, 1e-3, [body(UNIT_ATMOSPHERE)], 0) < 20);
  });

  test('time-step: 抵抗を受けない相手・大気の無い相手は、どんな刻みでも奪い切られない', () => {
    const bodies = [body(UNIT_ATMOSPHERE)];
    assert.equal(dragTakesFullAirspeed(descending(0), 0, bodies, 0, 1e9), false, '抵抗を受けない');
    assert.equal(dragTakesFullAirspeed(descending(0), 1, [body(null)], 0, 1e9), false, '大気が無い');
    assert.equal(dragTakesFullAirspeed(descending(0), 1, [], 0, 1e9), false, '相手がいない');
    assert.equal(dragTakesFullAirspeed(descending(0), 1, bodies, 0, 0), false, '刻みが 0');
  });
  ...
  test('time-step: 刻みを広げるほど、奪い切られる高度は上がる', () => {
    ...
    const boundary = (dt: number): number => {
      ...
      for (let i = 0; i < 60; i++) {
        const mid = (lo + hi) / 2;
        if (dragTakesFullAirspeed(descending(mid), 1e-3, bodies, 0, dt)) lo = mid; else hi = mid;
      }
      return (lo + hi) / 2;
    };
    ...
    // 境界のすぐ外側では奪い切られない。
    assert.equal(dragTakesFullAirspeed(descending(wide + 1), 1e-3, bodies, 0, 200), false);
    assert.equal(dragTakesFullAirspeed(descending(wide - 1), 1e-3, bodies, 0, 200), true);
  });
  ...
  test('time-step: 奪い切りの判定は、沈み込みの上限に引きずられない', () => {
    ...
    assert.ok(
      atmosphericMaxStep(farAbove, 1e-3, bodies, 0) < 1e4,
      '前提: 沈み込みの上限が有限の刻みを要求している');
    assert.equal(
      dragTakesFullAirspeed(farAbove, 1e-3, bodies, 0, 1e4), false,
      '空気が無いのに奪い切られている');
  });
}
