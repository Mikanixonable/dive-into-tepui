...
export function register(): void {
  ...
  test('thermal: 太陽光の受熱は1天文単位で太陽定数そのもので、距離の2乗に反比例する', () => {
    ...
    const at1AU = solarHeating(SUN.radiantIntensity, AU, 1, area);
    assert.ok(Math.abs(at1AU - SOLAR_CONSTANT * area) < 1e-12, `1AU で ${at1AU} W/kg`);
    for (const ratio of [0.5, 2, 5.2, 30]) {
      const far = solarHeating(SUN.radiantIntensity, ratio * AU, 1, area);
  ...
  test('thermal: 太陽光の受熱は日照率に比例し、本影では入らない', () => {
    ...
    const full = solarHeating(SUN.radiantIntensity, AU, 1, area);
    assert.ok(Math.abs(solarHeating(SUN.radiantIntensity, AU, 0.25, area) - full / 4) < 1e-12 * full);
    assert.equal(solarHeating(SUN.radiantIntensity, AU, 0, area), 0, '本影では入らない');
  });
  ...
  test('thermal: 地球軌道の日照だけでは、破片も艦も限界温度に届かない', () => {
    ...
    for (const c of cases) {
      const heating = solarHeating(
        SUN.radiantIntensity, AU, 1, solarAbsorbAreaPerMass(c.bcInv));
