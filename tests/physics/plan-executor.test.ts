...
export function register(): void {
  test('PlanExecutor: 目標Δvがほぼ0でノード時刻が目前ならその場で完了しthrustはnull', () => {
    ...
    executor.update(ship, 0.1, 0, openGate, true);
  ...
  test('PlanExecutor: バーン中にノード(Δv編集)が差し替わるとthrustが残らない(regression C)', () => {
    ...
    executor.update(ship, 0.1, 49, openGate, true);
    // 点火予定時刻(dv=200, accel=100 -> 燃焼2秒、点火は node.t-1=49)に到達させて点火する。
    executor.applyIgnitionAndCutoff(ship, 49, openGate, true);
  ...
  test('PlanExecutor: 純ラジアル方向のΔvでもslewを抜けてarmedへ進む(regression D)', () => {
    ...
    assert.notEqual(executor.nextEventTime(ship, 0, openGate, true), null);
  });
  ...
  test('PlanExecutor: ノード時刻がはるか先なら現在のΔvが小さくても点火・完了しない(regression F)', () => {
    ...
    assert.equal(executor.nextEventTime(ship, 0, openGate, true), null, 'armed に入っていないこと');
  });
  ...
  test('PlanExecutor: 高warpでゲートが閉じている間はnextEventTimeがnull(regression H)', () => {
    ...
    executor.update(ship, 0.1, 49, openGate, true);
    assert.notEqual(executor.nextEventTime(ship, 49, openGate, true), null, 'test setup: ゲートが開いていればarmed');

    executor.update(ship, 0.1, 49, closedGate, true);
    assert.equal(executor.nextEventTime(ship, 49, closedGate, true), null);
  });
  ...
  test('PlanExecutor: ゲートが閉じている間はapplyIgnitionAndCutoffの遮断判定も走らない(regression I)', () => {
    ...
    executor.update(ship, 0.1, 49, closedGate, true);
    executor.applyIgnitionAndCutoff(ship, 49, closedGate, true);
  ...
  test('PlanExecutor: 圏外へ出ても実行中の燃焼は完遂する(§13-6)', () => {
    const ship = makeShip();
    const dv = v3(0, 0, 200);
    const node = kinematicState(50, ship.state.r, add(ship.state.v, dv));
    ship.plan.addNode(node, ship.state);
    ...
    // ここで通信が切れる。噴射シーケンスは機上で完結するので推力は続く。
    executor.update(ship, 0.1, 49.5, openGate, false);
    assert.ok(ship.thrust !== null, '圏外でも燃焼が続くこと');
    assert.notEqual(executor.nextEventTime(ship, 49.5, openGate, false), null, '遮断予定は立ち続けること');

    // 目標速度へ到達すれば、圏外のまま遮断してノードを消化する。
    ship.state = kinematicState(50, ship.state.r, node.v);
    executor.applyIgnitionAndCutoff(ship, 50, openGate, false);
    assert.equal(ship.thrust, null, '遮断されること');
    assert.equal(ship.plan.nodes.length, 0, 'ノードが消化されていること');
  });

  test('PlanExecutor: 圏外では点火せず、ノードは残ったままになる(§13-6)', () => {
    const ship = makeShip();
    const dv = v3(0, 0, 200);
    ship.plan.addNode(kinematicState(50, ship.state.r, add(ship.state.v, dv)), ship.state);
    const executor = new PlanExecutor(hud);
    ...
    alignExactly(ship, v3(0, 0, 1), ship.state.r);
    executor.update(ship, 0.1, 49, openGate, false);
    assert.equal(ship.thrust, null, '圏外では姿勢整列も点火もしないこと');
    assert.equal(executor.nextEventTime(ship, 49, openGate, false), null);
    executor.applyIgnitionAndCutoff(ship, 50, openGate, false);
    assert.equal(ship.thrust, null, '点火予定時刻を跨いでも点火しないこと');
    assert.equal(ship.plan.nodes.length, 1, 'ノードは消えないこと');
  });

  test('PlanExecutor: 圏外で通過したノードは破棄され、残りのノードから再開する(§13-6)', () => {
    const ship = makeShip();
    const dv = v3(0, 0, 200);
    const missed = kinematicState(50, ship.state.r, add(ship.state.v, dv));
    const later = kinematicState(500, ship.state.r, add(ship.state.v, dv));
    ship.plan.addNode(missed, ship.state);
    ship.plan.addNode(later, ship.state);
    const executor = new PlanExecutor(hud);

    // 圏外のまま最初のノードの実行時刻を通過させる。
    executor.update(ship, 0.1, 49, openGate, false);
    executor.update(ship, 0.1, 60, openGate, false);
    assert.equal(ship.plan.nodes.length, 2, '圏外の間はノードが残ること');

    // 圏内へ戻ると、通過してしまったノードだけが捨てられる。
    executor.update(ship, 0.1, 61, openGate, true);
    assert.equal(ship.plan.nodes.length, 1, '通過したノードだけが破棄されること');
    assert.equal(ship.plan.firstNode(), later, '残りのノードから再開すること');

    // 残ったノードは通常どおり実行される。
    alignExactly(ship, v3(0, 0, 1), ship.state.r);
    executor.update(ship, 0.1, 499, openGate, true);
    assert.notEqual(executor.nextEventTime(ship, 499, openGate, true), null, 'armed に入り直すこと');
  });
