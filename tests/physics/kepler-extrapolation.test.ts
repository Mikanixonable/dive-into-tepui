...
function relativeTip(tip: KinematicState): KinematicState<'primaryRel'> {
  return toPrimaryRelative(tip.t, tip, EARTH.stateAt(0));
}
...
export function register(): void {
  test('kepler-extrapolation: extrapolatedRelativeState は円軌道を1周期ぶん外挿すると元の位置に戻る', () => {
    ...
    const back = extrapolatedRelativeState(tip, EARTH, 0, tip.t + period);
  ...
  test('kepler-extrapolation: extrapolatedRelativeState は離心軌道(e=0.6)で密な数値積分と一致する', () => {
    ...
    const extrapolated = extrapolatedRelativeState(tip, EARTH, 0, target);
  ...
  test('kepler-extrapolation: extrapolatedRelativeStates は時刻昇順で、最後の要素がちょうど untilT', () => {
    ...
    const states = extrapolatedRelativeStates(tip, EARTH, 0, untilT, 12);
  ...
  test('kepler-extrapolation: extrapolatedRelativeStates は複数周(3.5周)にまたがっても時刻が単調増加し、位置が飛ばない', () => {
    ...
    const states = extrapolatedRelativeStates(tip, EARTH, 0, untilT, count);
  ...
  test('kepler-extrapolation: extrapolatedRelativeStates は等時間刻みより弧長のばらつきが小さい(離心率0.8で1周)', () => {
    ...
    const byE = extrapolatedRelativeStates(tip, EARTH, 0, untilT, count);
    const byTime: KinematicState<'primaryRel'>[] = [];
    for (let i = 1; i <= count; i++) {
      const t = tip.t + (period * i) / count;
      byTime.push(extrapolatedRelativeState(tip, EARTH, 0, t)!);
    }
  ...
  test('kepler-extrapolation: 離心率が高すぎる(e>=0.98)/双曲線/放物線(a非有限)/mu<=0 では外挿できない', () => {
    ...
    assert.equal(extrapolatedRelativeState(nearParabolic, EARTH, 0, 1000), null);
    assert.deepEqual(extrapolatedRelativeStates(nearParabolic, EARTH, 0, 1000, 5), []);

    // 双曲線(e>1、a<0)。
    const rp = R_EARTH + 500e3;
    const e = 1.2;
    const hyperbolic = stateFromOrbitalElements(0, rp / (1 - e), e, 0, 0, 0, 0, MU_EARTH);
    assert.equal(extrapolatedRelativeState(hyperbolic, EARTH, 0, 1000), null);
    assert.deepEqual(extrapolatedRelativeStates(hyperbolic, EARTH, 0, 1000, 5), []);
    ...
    assert.equal(extrapolatedRelativeState(parabolic, EARTH, 0, 1000), null);
    assert.deepEqual(extrapolatedRelativeStates(parabolic, EARTH, 0, 1000, 5), []);

    const noMu: CelestialMotion = fixedMotion({
      id: 'earth', mu: 0, radius: R_EARTH, state: kinematicState<'eci'>(0, v3(), v3()),
    });
    const circular = kinematicState<'eci'>(0, v3(r0, 0, 0), v3(0, 7000, 0));
    assert.equal(extrapolatedRelativeState(circular, noMu, 0, 1000), null);
    assert.deepEqual(extrapolatedRelativeStates(circular, noMu, 0, 1000, 5), []);
  });

  test('kepler-extrapolation: count<=0 や untilT<=tip.t では空配列', () => {
    const tip = kinematicState<'eci'>(1000, v3(R_EARTH + 500e3, 0, 0), v3(0, 7600, 0));
    assert.deepEqual(extrapolatedRelativeStates(tip, EARTH, 0, 5000, 0), []);
    assert.deepEqual(extrapolatedRelativeStates(tip, EARTH, 0, 5000, -3), []);
    assert.deepEqual(extrapolatedRelativeStates(tip, EARTH, 0, tip.t, 5), []);
    assert.deepEqual(extrapolatedRelativeStates(tip, EARTH, 0, tip.t - 100, 5), []);
  });
}
