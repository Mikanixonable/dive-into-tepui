...
function sunAt(distance: number): Attractor {
  return { id: 'sun', mu: MU_SUN, radius: R_SUN, state: kinematicState(0, v3(distance, 0, 0), ZERO), degree2: null, isStar: true };
}

export function register(): void {
  test('srp: srpCoeff = 0 gives exactly zero acceleration', () => {
    assert.deepEqual(srpAccel(v3(7e6, 0, 0), sunAt(ASTRONOMICAL_UNIT), 0, 1), ZERO);
  });

  test('srp: a fully shadowed object gets exactly zero acceleration', () => {
    assert.deepEqual(srpAccel(v3(7e6, 0, 0), sunAt(ASTRONOMICAL_UNIT), 1e-2, 0), ZERO);
  });

  test('srp: the acceleration points from the sun toward the object', () => {
    const sun = sunAt(ASTRONOMICAL_UNIT);
  ...
  test('srp: at 1 AU with a unit coefficient the magnitude equals the reference solar pressure', () => {
    ...
    const sun = sunAt(0);
    const mag = len(srpAccel(v3(ASTRONOMICAL_UNIT, 0, 0), sun, 1, 1));
  ...
  test('srp: doubling the sun distance quarters the acceleration', () => {
    const sun = sunAt(0);
    const near = len(srpAccel(v3(ASTRONOMICAL_UNIT, 0, 0), sun, 1e-2, 1));
    const far = len(srpAccel(v3(2 * ASTRONOMICAL_UNIT, 0, 0), sun, 1e-2, 1));
  ...
  test('srp: a ship-scale coefficient gives about 7e-8 m/s^2 near Earth', () => {
    ...
    const mag = len(srpAccel(v3(-6.8e6, 0, 0), sunAt(ASTRONOMICAL_UNIT), 1.56e-2, 1));
