...
export function register(): void {
  test('srp: srpCoeff = 0 gives exactly zero acceleration', () => {
    assert.deepEqual(srpAccel(v3(7e6, 0, 0), sunAt(AU), 0, 0, 1), ZERO);
  });

  test('srp: a fully shadowed object gets exactly zero acceleration', () => {
    assert.deepEqual(srpAccel(v3(7e6, 0, 0), sunAt(AU), 0, 1e-2, 0), ZERO);
  });

  test('srp: the acceleration points from the sun toward the object', () => {
    const sun = sunAt(AU);
    const r = v3(-7e6, 2e6, 1e6); // 太陽と反対側
    const a = srpAccel(r, sun, 0, 1e-2, 1);
    const away = norm(sub(r, sun.stateAt(0).r));
  ...
  test('srp: doubling the sun distance quarters the acceleration', () => {
    const sun = sunAt(0);
    const near = len(srpAccel(v3(AU, 0, 0), sun, 0, 1e-2, 1));
    const far = len(srpAccel(v3(2 * AU, 0, 0), sun, 0, 1e-2, 1));
  ...
  test('srp: the shadow factor scales the acceleration linearly', () => {
    const sun = sunAt(AU);
    const r = v3(-7e6, 0, 0);
    const full = len(srpAccel(r, sun, 0, 1e-2, 1));
    const half = len(srpAccel(r, sun, 0, 1e-2, 0.5));
  ...
  test('srp: a ship-scale coefficient gives about 7e-8 m/s^2 near Earth', () => {
    ...
    const mag = len(srpAccel(v3(-6.8e6, 0, 0), sunAt(AU), 0, 1.56e-2, 1));
