...
export function register(): void {
  ...
  test('firstSurfaceContact: 1ステップで小天体を貫通する経路を捉える(点判定は見逃す)', () => {
    ...
    assert.equal(firstSurfaceContact(prev, next, 0, [rock])?.body, rock);
  });
  ...
  test('firstSurfaceContact: 掃引で捉えた toi は区間の途中で、補間すると天体表面上に載る', () => {
    ...
    const hit = firstSurfaceContact(prev, next, 0, [rock]);
    ...
    const dist = len(sub(at.r, rock.state.r));
    assert.ok(Math.abs(dist - rock.radius) < 1, `impact should sit on the surface, ${dist} m vs ${rock.radius} m`);
  ...
  test('firstSurfaceContact: 表面に触れない近傍通過は検出しない', () => {
    ...
    const prev = kinematicState(0, v3(-78e3, 0, 0), vel);
    const next = kinematicState(20, v3(78e3, 0, 0), vel);
    assert.equal(firstSurfaceContact(prev, next, 0, [rock]), null);
  });
  ...
  test('firstSurfaceContact: 天体自身が区間の間に動いて当たる経路を捉える', () => {
    ...
    assert.equal(firstSurfaceContact(prev, next, 0, [still]), null);
    const hit = firstSurfaceContact(prev, next, 0, [moving]);
  ...
  test('firstSurfaceContact: 動いている天体の表面に触れない通過は検出しない', () => {
    ...
    assert.equal(
      firstSurfaceContact(kinematicState(0, ZERO, ZERO), kinematicState(20, ZERO, ZERO), 0, [moving]),
      null);
  });
  ...
  test('firstSurfaceContact: 開始時点で既に内部にいる場合もその天体を返す', () => {
    const inside = kinematicState(0, v3(R_EARTH - 1e3, 0, 0), ZERO);
    const later = kinematicState(20, v3(R_EARTH - 2e3, 0, 0), ZERO);
    const hit = firstSurfaceContact(inside, later, 0, [EARTH]);
  ...
  test('firstSurfaceContact: 区間の無い(prev === next)入力は区間終端の重なりだけを見る', () => {
    ...
    assert.equal(firstSurfaceContact(outside, outside, 0, [EARTH]), null);
    const inside = kinematicState(0, v3(R_EARTH - 1, 0, 0), ZERO);
    assert.equal(firstSurfaceContact(inside, inside, 0, [EARTH])?.body, EARTH);
  });
  ...
  test('firstSurfaceContact: 非有限な入力は接触なしとして扱う', () => {
    ...
    const vel = v3(7800, 0, 0);
    const next = kinematicState(20, v3(78e3, 0, 0), vel);
    assert.equal(firstSurfaceContact(kinematicState(0, v3(NaN, 0, 0), vel), next, 0, [rock]), null);
    const prev = kinematicState(0, v3(-78e3, 0, 0), vel);
    assert.equal(
  ...
  test('firstSurfaceContact: 複数が触れうるときは最も早い1体を選ぶ', () => {
    ...
    assert.equal(firstSurfaceContact(prev, next, 0, [near, far])?.body, near);
    assert.equal(firstSurfaceContact(prev, next, 0, [far, near])?.body, near);
  });
}
