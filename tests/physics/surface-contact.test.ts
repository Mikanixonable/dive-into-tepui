...
export function register(): void {
  ...
  test('firstSurfaceContact: 動いている天体の表面に触れない通過は検出しない', () => {
    ...
    assert.equal(
      firstSurfaceContact(kinematicState<'eci'>(0, ZERO, ZERO), kinematicState<'eci'>(20, ZERO, ZERO), 0, [moving]),
      null);
  });
  ...
  test('firstSurfaceContact: 非有限な入力は接触なしとして扱う', () => {
    ...
    const vel = v3(7800, 0, 0);
    const next = kinematicState<'eci'>(20, v3(78e3, 0, 0), vel);
    assert.equal(firstSurfaceContact(kinematicState<'eci'>(0, v3(NaN, 0, 0), vel), next, 0, [rock]), null);
    const prev = kinematicState<'eci'>(0, v3(-78e3, 0, 0), vel);
    assert.equal(
      firstSurfaceContact(prev, kinematicState<'eci'>(20, v3(NaN, 0, 0), vel), 0, [rock]), null);
  });
