...
function star(x: number): CelestialMotion {
  return fixedMotion({ id: 'sun', mu: MU_SUN, radius: R_SUN, state: kinematicState<'eci'>(0, v3(x, 0, 0), v3(0, 0, 0)), accel: v3(), degree2: null, atmosphere: null, kind: 'star' });
}

function body(id: string, mu: number, radius: number, x: number): CelestialMotion {
  return fixedMotion({ id, mu, radius, state: kinematicState<'eci'>(0, v3(x, 0, 0), v3(0, 0, 0)), accel: v3(), degree2: null, atmosphere: null });
}
