...
import {
  EphemerisPackFormatError,
  buildEphemerisPackData,
  canonicalJson,
  decodeEphemerisPack,
  encodeEphemerisPack,
  toEvaluatorEphemerisPack,
  validateManifest,
...
function throwsFormat(fn: () => unknown): void {
  assert.throws(fn, (error: unknown) => error instanceof EphemerisPackFormatError);
}
...
export function register(): void {
  test('ephemeris pack: deterministic Float64 Chebyshev roundtrip', () => {
    ...
    assert.deepEqual(bytes, encodeEphemerisPack({ ...data.manifest }, data.payload));
  });
  ...
  test('ephemeris pack: truncated, bad-header, and non-finite payload corruption is rejected', () => {
    ...
    throwsFormat(() => decodeEphemerisPack(bytes.subarray(0, bytes.length - 1)));
    const badMagic = bytes.slice();
    badMagic[0] ^= 0xff;
    throwsFormat(() => decodeEphemerisPack(badMagic));
    const badLength = bytes.slice();
    badLength[20] += 8;
    throwsFormat(() => decodeEphemerisPack(badLength));
  ...
  test('ephemeris pack: decoded data adapts to the existing evaluator pack shape', () => {
    const data = buildEphemerisPackData(BASE, SEGMENTS);
    const evaluatorPack = toEvaluatorEphemerisPack({
