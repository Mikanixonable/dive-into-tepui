...
export function register(): void {
  test('orbit-entity: step only records a history sample once sampleInterval has elapsed', () => {
    ...
    for (let i = 0; i < 100; i++) {
      e.step(dt, EPH, 0, null, sampleInterval, 100000);
    }
  ...
  test('orbit-entity: keepDuration bounds how many history samples accumulate', () => {
    ...
    for (let i = 0; i < 500; i++) { // 5000秒ぶん進める(保持窓の25倍)
      e.step(dt, EPH, 0, null, sampleInterval, keepDuration);
    }
  ...
  test('orbit-entity: at() matches direct re-integration within interpolation error', () => {
    ...
    for (let i = 0; i < 200; i++) {
      dense.step(dt, EPH, 0, null, dt, 1e6); // sampleInterval=dt → 実質毎ステップ記録
      sparse.step(dt, EPH, 0, null, sampleInterval, 1e6);
  ...
  test('orbit-entity: reset discards history samples at or after the new state time', () => {
    ...
    const dt = 10;
    for (let i = 0; i < 10; i++) {
      e.step(dt, EPH, 0, null, dt, 100000); // 毎ステップ記録: history = t=0..80, state.t=90
    }
  ...
  test('orbit-entity: prevState always reflects the state immediately before the last step/reset', () => {
    ...
    e.step(10, EPH, 0, null, 1000, 0); // keepDuration=0(history 不使用)でも prevState は更新される
    assert.equal(e.prevState.t, 0);
    assert.equal(e.state.t, 10);

    e.step(10, EPH, 0, null, 1000, 0);
  ...
  test('orbit-entity: samplesOldestFirst is ordered oldest-to-newest and ends at state', () => {
    ...
    const sampleInterval = 10; // 毎ステップ記録
    for (let i = 0; i < 20; i++) {
      e.step(dt, EPH, 0, null, sampleInterval, 1e6);
    }
  ...
  test('orbit-entity: samplesOldestFirst is just [state] when history is empty', () => {
    ...
    for (let i = 0; i < 10; i++) {
      noHistory.step(1, EPH, 0, null, 1, 0); // keepDuration=0 なので history は空のまま
    }
