// physics/orbit-entity.ts の回帰テスト。game/game-entity/game-entity.ts の GameEntity から
...
const SUN_POS = v3(1.5e11, 0, 0);
const MOON_POS = v3(3.8e8, 0, 0);
...
export function register(): void {
  test('orbit-entity: step only records a history sample once sampleInterval has elapsed', () => {
    const e = new OrbitEntity(circularState());
    const dt = 5;
    const sampleInterval = 23; // dt では割り切れない値にして端数の丸まり方を確認する
    for (let i = 0; i < 100; i++) {
      e.step(dt, SUN_POS, MOON_POS, 0, null, sampleInterval, 100000);
    }
  ...
  test('orbit-entity: step never touches history when keepDuration is 0', () => {
    const e = new OrbitEntity(circularState());
    for (let i = 0; i < 50; i++) {
      e.step(1, SUN_POS, MOON_POS, 0, null, 1, 0);
    }
    assert.equal(e.history.size, 0);
  });

  test('orbit-entity: keepDuration bounds how many history samples accumulate', () => {
    const e = new OrbitEntity(circularState());
    const dt = 10;
    ...
    for (let i = 0; i < 500; i++) { // 5000秒ぶん進める(保持窓の25倍)
      e.step(dt, SUN_POS, MOON_POS, 0, null, sampleInterval, keepDuration);
    }
    assert.ok(e.history.size < 30, `history should stay bounded by keepDuration, got ${e.history.size} samples`);
  });

  test('orbit-entity: at() matches direct re-integration within interpolation error', () => {
    // 密なステップ(sampleInterval を実質無視)で真値の軌跡を作りつつ、同時に大きい
    // sampleInterval で間引いた OrbitEntity を並行して進め、任意時刻の at() が密な真値と
    // 近い(better_predict.md §4 実測の補間誤差 30m 程度)ことを確認する。
    const dense = new OrbitEntity(circularState());
    const sparse = new OrbitEntity(circularState());
    const dt = 5;
    const sampleInterval = 174; // LEO 1周32点相当の実測値
    const denseStates: OrbitState[] = [];
    for (let i = 0; i < 200; i++) {
      dense.step(dt, SUN_POS, MOON_POS, 0, null, dt, 1e6); // sampleInterval=dt → 実質毎ステップ記録
      sparse.step(dt, SUN_POS, MOON_POS, 0, null, sampleInterval, 1e6);
  ...
  test('orbit-entity: reset discards history samples at or after the new state time', () => {
    const e = new OrbitEntity(circularState());
    const dt = 10;
    for (let i = 0; i < 10; i++) {
      e.step(dt, SUN_POS, MOON_POS, 0, null, dt, 100000); // 毎ステップ記録: history = t=0..80, state.t=90
    }
  ...
  test('orbit-entity: prevState always reflects the state immediately before the last step/reset', () => {
    const e = new OrbitEntity(circularState());
    assert.equal(e.prevState.t, 0); // 初期状態では自分自身に退化(まだ一度も進んでいない)

    e.step(10, SUN_POS, MOON_POS, 0, null, 1000, 0); // keepDuration=0(history 不使用)でも prevState は更新される
    assert.equal(e.prevState.t, 0);
    assert.equal(e.state.t, 10);

    e.step(10, SUN_POS, MOON_POS, 0, null, 1000, 0);
  ...
  test('orbit-entity: at() returns state itself at t === state.t and null beyond it', () => {
    const e = new OrbitEntity(circularState());
    e.step(10, SUN_POS, MOON_POS, 0, null, 5, 100000);
