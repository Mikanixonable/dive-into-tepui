...
import { secondaryFrameFor, solarSystemParts } from './test-helpers';
...
export function register(): void {
  ...
  for (const secondaryId of SECONDARIES) {
    for (const point of POINTS) {
      ...
      const halo: HaloParams = { point, az: 1e7, phase: 0.3 };

      test(`halo: ${label} lissajousState is finite`, () => {
        const s = lissajousState(secondaryFrameFor(PARTS, secondaryId, t), lissajous);
      ...
      test(`halo: ${label} lissajous with az=0 stays in the L-point orbital plane`, () => {
        const frame = collinearFrame(secondaryFrameFor(PARTS, secondaryId, t), point);
        const s = lissajousState(secondaryFrameFor(PARTS, secondaryId, t), { ...lissajous, az: 0 });
        ...
        const tol = secondaryId === 'moon' ? 30 : 1e-1;
      ...
      test(`halo: ${label} haloState is finite and near the L-point`, () => {
        const s = haloState(secondaryFrameFor(PARTS, secondaryId, t), halo);
  ...
  test('halo: Sun-Earth L1 linear parameters match the published values', () => {
    const frame = collinearFrame(secondaryFrameFor(PARTS, 'earth', t), 'L1');
  ...
  test('halo: Earth-Moon L1 linear parameters match the published values', () => {
    const frame = collinearFrame(secondaryFrameFor(PARTS, 'moon', t), 'L1');
