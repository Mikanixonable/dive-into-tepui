...
地球・月・太陽系の天体を舞台に、宇宙船の姿勢と並進を操作しながら敵機と戦います。時間加速で軌道上の移動を短縮し、必要ならマップビューでマニューバーを計画します。

## ゲームの全体像

このゲームでは、敵機へまっすぐ飛ぶだけでは戦えません。機体の向き、推進方向、軌道上の位置と速度を考えながら、接近・離脱・軌道変更・射撃を行います。

- 機体座標系の並進と RCS による姿勢制御
- 時間加速を使った軌道上の移動
- 敵機の軌道と相対速度を考えた迎撃
- マップビューでの軌道表示とマニューバーノード編集
- 大気抵抗や重力場を含む軌道シミュレーション
...
`stage 2` では、マップビューで軌道遷移を計画することが重要になります。`CREATIVE` では、補給・燃料補給・敵の波状攻撃を個別に切り替えられます。

## 最初の 1 周

1. `T` で敵をターゲットに選択する
2. `F` で機首をプログレード方向へ向ける
3. `W/S/A/D/Q/E` で機体を敵の軌道へ近づける
4. `,` と `.` で時間加速を調整し、会合までの時間を短縮する
5. 必要に応じて `M` でマップビューを開き、計画軌道を編集する
6. 射撃可能な時間倍率まで戻し、照準を合わせて `Space` で射撃する
7. `H` で詳細な操作説明、`ESC` でポーズ・設定を開く
...
マップビューでは、計画軌道をクリックしてノードを配置できます。ノードの丸ハンドルをドラッグすると実行時刻を移動でき、Δv ハンドルをドラッグすると成分を直接調整できます。
...
- `SHIP STATUS`: ミッション時間、時間倍率、推進出力、RCS 状態など
- `ORBIT`: 高度、速度、近地点・遠地点、軌道傾斜角、周期など
- `TARGET`: ターゲットまでの距離、接近速度、相対速度、耐久値など
- `CONTACTS`: 周囲の敵機と選択中のターゲット
- 画面中央のマーカー: 機首方向、進行方向、ターゲット方向、リード照準

マップビューでは、天体・機体・軌道・予測軌道を一覧し、表示する座標系やフレームを切り替えられます。計画したノードは `N` で実行地点まで自動ワープできます。

## セーブと設定

セーブデータはブラウザ内のストレージに保存されます。ラン開始時に保存され、プレイ中は約 60 秒ごとにオートセーブされます。`F5` で手動セーブ、`F9` でセーブデータの管理画面を開けます。
